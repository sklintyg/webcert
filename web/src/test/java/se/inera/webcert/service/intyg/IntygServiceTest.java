...
public class IntygServiceTest extends AbstractIntygServiceTest {
    ...
    public void testFetchIntyg() throws Exception {
        ...
        when(getCertificateService.getCertificateForCare(LOGICAL_ADDRESS, request)).thenReturn(
                intygtjanstResponse);
        ...
        verify(getCertificateService).getCertificateForCare(LOGICAL_ADDRESS, request);
    ...
    public void testListIntyg() {
        ...
        when(listCertificateService.listCertificatesForCare(LOGICAL_ADDRESS, request)).thenReturn(
                listResponse);

        List<IntygItem> list = intygService.listIntyg(Collections.singletonList("enhet-1"), "19121212-1212");

        verify(listCertificateService).listCertificatesForCare(LOGICAL_ADDRESS, request);
