...
class CertificateForPatientServiceImplTest {
  ...
  void setUp() {
    ...
    certificateForPatient =
        new CertificateForPatientServiceImpl(
      ...
      final var actualResult =
          (List<ListIntygEntry>)
              objectMapper.readValue(
                  ...
                  objectMapper
                      .getTypeFactory()
                      .constructCollectionType(List.class, ListIntygEntry.class));
    ...
    void setUp() throws JsonProcessingException {
      ...
      final var cachedData =
          objectMapper.writeValueAsString(
              List.of(
                  intygListItemFromIT(
                      "3", "2014-01-02T10:11:23", CertificateState.CANCELLED.name())));
      certificatesForPatientCache.put(CACHE_KEY, cachedData);
    }
  ...
  class IncludeRelationsOnListIntygEntry {
    ...
    void shouldAddRelationIfReplacedBySignedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.ERSATT), CertificateState.RECEIVED);
    ...
    void shouldNotAddRelationIfReplacedByUnsignedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.ERSATT), CertificateState.IN_PROGRESS);
    ...
    void shouldNotAddRelationIfReplacedByRevokedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.ERSATT), CertificateState.CANCELLED);
    ...
    void shouldAddRelationIfComplementedBySignedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.KOMPLT), CertificateState.RECEIVED);
    ...
    void shouldNotAddRelationIfComplementedByUnsignedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.KOMPLT), CertificateState.IN_PROGRESS);
    ...
    void shouldNotAddRelationIfComplementedByRevokedCertificate() {
      final var expectedRelations =
          prepareCertificates(List.of(RelationKod.KOMPLT), CertificateState.CANCELLED);
    ...
    void shouldAddRelationsIfComplementedAndReplacedBySignedCertificate() {
      final var expectedRelations =
          prepareCertificates(
              List.of(RelationKod.KOMPLT, RelationKod.ERSATT), CertificateState.RECEIVED);
    ...
    private Relations prepareCertificates(
        List<RelationKod> relationKodList, CertificateState certificateState) {
      if (relationKodList != null) {
        final var fromWebcert =
            relationKodList.stream()
                .map(
                    relationKod -> {
                      final var certificateWithRelation =
                          utkastFromWC("4", "2014-01-03T12:12:18", certificateState.name());
  ...
  private ListIntygEntry intygListItemFromIT(String id, String localDateTimeAsStr, String status) {
    final var intygItem =
        TestIntygFactory.createIntygItem(
            id, LocalDateTime.parse(localDateTimeAsStr), IntygSource.IT);
