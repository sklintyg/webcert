...
public class FmbSjukfallServiceImplTest {
  ...
  @Mock private ListActiveSickLeavesForCareUnitResponderInterface sickLeavesForCareUnit;

  @Mock private SjukfallEngineService sjukfallEngineService;

  @Mock private WebCertUserService webCertUserService;

  @Captor private ArgumentCaptor<ListActiveSickLeavesForCareUnitType> requestCaptor;

  @InjectMocks private FmbSjukfallServiceImpl fmbSjukfallService;
  ...
  public void totalSjukskrivningstidForPatientAndCareUnit() {
    ...
    doReturn(response)
        ...
        .listActiveSickLeavesForCareUnit(
            anyString(), any(ListActiveSickLeavesForCareUnitType.class));
    ...
    final int tid =
        fmbSjukfallService.totalSjukskrivningstidForPatientAndCareUnit(
            Personnummer.createPersonnummer(PERSON_NUMMER).get(), PERIODS);

    Mockito.verify(sickLeavesForCareUnit, times(1))
        .listActiveSickLeavesForCareUnit(anyString(), requestCaptor.capture());
  ...
  public void totalSjukskrivningstidForPatientAndCareUnitWithoutPreviousSickLeaves() {
    ...
    final int tid =
        fmbSjukfallService.totalSjukskrivningstidForPatientAndCareUnit(
            Personnummer.createPersonnummer(PERSON_NUMMER).get(), Arrays.asList(period));
  ...
  public void totalSjukskrivningstidForPatientAndCareUnitMottagning() {
    ...
    user.setValdVardenhet(
        user.getVardgivare().get(0).getVardenheter().get(0).getMottagningar().get(0));
    final ListActiveSickLeavesForCareUnitResponseType response = createResponse();
  ...
  private ListActiveSickLeavesForCareUnitResponseType createEmptyResponse() {
    ListActiveSickLeavesForCareUnitResponseType response =
        new ListActiveSickLeavesForCareUnitResponseType();
