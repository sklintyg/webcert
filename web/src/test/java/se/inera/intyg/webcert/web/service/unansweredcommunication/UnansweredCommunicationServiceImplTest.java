...
class UnansweredCommunicationServiceImplTest {
  ...
  @InjectMocks private UnansweredCommunicationServiceImpl unansweredCommunicationService;
  ...
  void shouldIncrementComplemented() {
    ...
    Assertions.assertEquals(
        1, result.getUnansweredQAsMap().get(expecteCertificateId).getComplement());
  }
  ...
  void shouldIncrementMultipleComplemented() {
    ...
    final var result = unansweredCommunicationService.get(request);
    Assertions.assertEquals(
        3, result.getUnansweredQAsMap().get(expecteCertificateId).getComplement());
  }
  ...
  void shouldIncrementComplementAndOther() {
    ...
    final var notValidArende =
        getArende(ArendeAmne.PAMINN, Status.PENDING_INTERNAL_ACTION, expecteCertificateId, FK);

    doReturn(List.of(arendeOther, arendeComplement, notValidArende))
        .when(arendeService)
        .getArendenForPatientsWithTimestampAfterDate(eq(request.getPatientIds()), any());
