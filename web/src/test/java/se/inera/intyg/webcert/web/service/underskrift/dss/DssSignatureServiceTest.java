...
public class DssSignatureServiceTest {
  ...
  @Mock DssMetadataService dssMetadataService;

  @Mock WebCertUserService webCertUserService;

  @Mock DssSignMessageService dssSignMessageService;

  @Mock DssSignMessageIdpProvider dssSignMessageIdpProvider;

  @Mock WebCertUser user;

  @Mock UtkastRepository utkastRepository;

  @Mock Optional<Utkast> utkastOptional;

  @Mock Utkast utkast;

  @Captor ArgumentCaptor<SignRequest> signRequestCaptor;

  @Mock Personnummer patient;

  @Mock UnderskriftService underskriftService;
  ...
  @Captor ArgumentCaptor<String> ticketIdCaptor;

  @Captor ArgumentCaptor<String> certificateCaptor;

  @Captor ArgumentCaptor<byte[]> signatureCaptor;

  @Captor ArgumentCaptor<SignaturStatus> statusCaptor;
  ...
  public DssSignatureServiceTest() {
    ...
    dssSignatureService =
        new DssSignatureService(
            dssMetadataService,
            dssSignMessageService,
            webCertUserService,
            utkastRepository,
            dssSignMessageIdpProvider,
            underskriftService,
            redisTicketTracker,
  ...
  public void createSignatureRequestDTO() throws ModuleNotFoundException {
    ...
    ReflectionTestUtils.setField(
        dssSignatureService, "dssClientEntityHostUrl", "https://wc.localtest.me:8020");
    ReflectionTestUtils.setField(
        dssSignatureService, "dssClientResponseHostUrl", "https://wc.localtest.me:8020");
    ReflectionTestUtils.setField(
        dssSignatureService, "customerId", "AnUn3ss3c@ary_Long--$@ClientIDwithr4nd0mCharacters");
    ReflectionTestUtils.setField(
        dssSignatureService, "applicationId", "App/ID\\With--w€|rd__CH@r$");
    ReflectionTestUtils.setField(
        dssSignatureService,
        "serviceUrl",
        "https://esign.v2.st.signatureservice.se/signservice-frontend/metadata/4321a111111");
    var signMessageTemplate =
        "Härmed skriver jag under {intygsTyp} utfärdat för {patientPnr}<br/><br/>Intygs-id: {intygsId}";
    ReflectionTestUtils.setField(dssSignatureService, "signMessage", signMessageTemplate);
    ReflectionTestUtils.setField(
        dssSignatureService,
        "approvedLoaList",
        Arrays.asList("http://id.sambi.se/loa/loa2", "http://id.sambi.se/loa/loa3"));
    final var signRequestValidityInMinutes = 8;
    ReflectionTestUtils.setField(
        dssSignatureService, "signRequestValidityInMinutes", signRequestValidityInMinutes);

    var sb =
        SignaturBiljettBuilder.aSignaturBiljett(
                "ticketId", SignaturTyp.XMLDSIG, SignMethod.SIGN_SERVICE)
    ...
    // Check signMessage
    var signRequestExtensionTypeJAXBElement =
        (JAXBElement<SignRequestExtensionType>)
            capturedSignRequest.getOptionalInputs().getAny().getFirst();
    assertNotNull(signRequestExtensionTypeJAXBElement);

    var actualSignMessageBytes =
        signRequestExtensionTypeJAXBElement.getValue().getSignMessage().getMessage();
    assertNotNull(actualSignMessageBytes);

    var actualSignMessage = new String(actualSignMessageBytes, StandardCharsets.UTF_8);
    var expectedSignMessage =
        signMessageTemplate
            .replace("{intygsTyp}", "intygsTyp")
            .replace("{patientPnr}", "19121212-1212")
            .replace("{intygsId}", "intygsId");
    assertEquals(expectedSignMessage, actualSignMessage);

    final var notBefore =
        signRequestExtensionTypeJAXBElement
            .getValue()
            .getConditions()
            .getNotBefore()
            .toGregorianCalendar()
  ...
  public void receiveSignatureResponse()
    ...
    var signByteArray =
        Base64.getDecoder()
            .decode(
                "MEYCIQDexXhdoTJWwjcXnQUGR2QAXcEbP+5N1f8QPghNeGb2dgIhAOvoNNfTlZqPyhuuYwLryVirmx/90NHsWf+oefZEQjBd");
  ...
  public void findReturnUrl() {
    ...
    assertEquals(
        String.format("https://wc.localtest.me:8020/#/intyg/lisjp/1.1/%s/?signed", intygsId),
        returnUrl);
  }
  ...
  public void findReturnErrorUrl() {
    ...
    assertEquals(
        String.format(
            "https://wc.localtest.me:8020/#/lisjp/1.1/edit/%s/?error&ticket=123", intygsId),
        returnUrl);
  }
  ...
  public void isUnitInIeWhitelist() {
    ...
    ReflectionTestUtils.setField(dssSignatureService, "dssUnitWhitelistForIe", List.of(""));
    assertTrue(dssSignatureService.shouldUseSigningService("TSTNMT2321000156-1077"));
    ...
    ReflectionTestUtils.setField(
        dssSignatureService,
        "dssUnitWhitelistForIe",
        Arrays.asList("TSTNMT2321000156-1077", "TSTNMT23210001512-*"));
    assertFalse(dssSignatureService.shouldUseSigningService("TSTNMT2321000156-1077"));
    ...
    ReflectionTestUtils.setField(
        dssSignatureService, "dssUnitWhitelistForIe", List.of("TSTNMT23210001512*"));
    assertTrue(dssSignatureService.shouldUseSigningService("TSTNMT2321000156-1077"));
    assertFalse(dssSignatureService.shouldUseSigningService("TSTNMT23210001512-WILDCARD"));
    assertFalse(dssSignatureService.shouldUseSigningService("TSTNMT23210001512WILDCARD"));
    assertTrue(dssSignatureService.shouldUseSigningService("FINNS_INTE"));
