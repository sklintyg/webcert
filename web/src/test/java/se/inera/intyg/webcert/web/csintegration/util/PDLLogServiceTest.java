...
class PDLLogServiceTest {
  ...
  @Mock LogRequestFactory logRequestFactory;

  @InjectMocks PDLLogService pdlLogService;
  ...
  void setUp() {
    CERTIFICATE.setMetadata(
        CertificateMetadata.builder()
            ...
            .recipient(CertificateRecipient.builder().name(RECIPIENT_NAME).build())
            .build());
  }
  ...
  void shouldLogCreateCertificate() {
    ...
    doReturn(expectedLogRequest)
        .when(logRequestFactory)
  ...
  void shouldLogCreateCertificateWithAdditionalInfo() {
    ...
    doReturn(expectedLogRequest)
        .when(logRequestFactory)
        .createLogRequestFromCertificate(CERTIFICATE, ADDITIONAL_INFO);
  ...
  class Print {
    ...
    void shouldLogForDraft() {
      final var expectedLogRequest = LogRequest.builder().build();
      final var captor = ArgumentCaptor.forClass(LogRequest.class);
      final var metadata = CertificateMetadata.builder().status(CertificateStatus.UNSIGNED).build();
    ...
    void shouldLogForLockedDraft() {
      final var expectedLogRequest = LogRequest.builder().build();
      final var captor = ArgumentCaptor.forClass(LogRequest.class);
      final var metadata = CertificateMetadata.builder().status(CertificateStatus.LOCKED).build();
      CERTIFICATE.setMetadata(metadata);
      doReturn(expectedLogRequest)
          .when(logRequestFactory)
          .createLogRequestFromCertificate(CERTIFICATE, null);
    ...
    void shouldLogForSigned() {
      final var expectedLogRequest = LogRequest.builder().build();
      final var captor = ArgumentCaptor.forClass(LogRequest.class);
      final var metadata = CertificateMetadata.builder().status(CertificateStatus.SIGNED).build();
      CERTIFICATE.setMetadata(metadata);
      doReturn(expectedLogRequest)
          .when(logRequestFactory)
          .createLogRequestFromCertificate(CERTIFICATE, null);
    ...
    void shouldLogForRevoked() {
      final var expectedLogRequest = LogRequest.builder().build();
      final var captor = ArgumentCaptor.forClass(LogRequest.class);
      final var metadata = CertificateMetadata.builder().status(CertificateStatus.REVOKED).build();
      CERTIFICATE.setMetadata(metadata);
      doReturn(expectedLogRequest)
          .when(logRequestFactory)
          .createLogRequestFromCertificate(CERTIFICATE, null);
  ...
  void shouldLogSentCertificate() {
    ...
    doReturn(expectedLogRequest)
        ...
        .createLogRequestFromCertificate(
            CERTIFICATE, ADDITIONAL_WITH_RECIPIENT_MESSAGE + RECIPIENT_NAME);
  ...
  void shouldLogSentCertificateWithAdditionalInfo() {
    ...
    doReturn(expectedLogRequest)
        ...
        .createLogRequestFromCertificate(
            CERTIFICATE,
            ADDITIONAL_INFO + ". " + ADDITIONAL_WITH_RECIPIENT_MESSAGE + RECIPIENT_NAME);
    doReturn(mockedUser).when(webCertUserService).getUser();
    doReturn(additionalInfoIntegrationParameter()).when(mockedUser).getParameters();
    doReturn(true).when(webCertUserService).hasAuthenticationContext();
  ...
  void shouldLogSentCertificateWithAdditionalInfoWithRecipient() {
    final var expectedLogRequest =
        "Läsning i enlighet med sammanhållen journalföring. Intyg skickat till mottagare recipientName";
    final var captor = ArgumentCaptor.forClass(String.class);
    final var mockedUser = mock(WebCertUser.class);

    doReturn(LogRequest.builder().build())
        .when(logRequestFactory)
        .createLogRequestFromCertificate(
            CERTIFICATE,
            ADDITIONAL_INFO + ". " + ADDITIONAL_WITH_RECIPIENT_MESSAGE + RECIPIENT_NAME);
  ...
  void shouldLogCreateMessage() {
    ...
    pdlLogService.logCreateMessage(PERSON_ID, CERTIFICATE_ID);
  ...
  private IntegrationParameters additionalInfoIntegrationParameter() {
    return IntegrationParameters.of(
        null, null, null, null, null, null, null, null, null, true, false, false, false);
  }
}
