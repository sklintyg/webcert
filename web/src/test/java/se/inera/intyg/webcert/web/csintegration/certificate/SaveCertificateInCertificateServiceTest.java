...
class SaveCertificateInCertificateServiceTest {
  ...
  @InjectMocks SaveCertificateInCertificateService saveCertificateInCertificateService;
  ...
  private static final SaveCertificateRequestDTO REQUEST =
  ...
  static {
    CERTIFICATE.setMetadata(
        CertificateMetadata.builder()
            ...
            .patient(Patient.builder().personId(PersonId.builder().id(ID).build()).build())
            .build());
  }

  @Test
  void shouldReturnNullIfCertificateDoesNotExistInCS() {
    assertEquals(-1L, saveCertificateInCertificateService.saveCertificate(CERTIFICATE, PDL_LOG));
  }
  ...
  class CertificateServiceHasCertificate {
    ...
    void setup() {
      ...
      when(csIntegrationService.saveCertificate(REQUEST)).thenReturn(CERTIFICATE);
      when(csIntegrationRequestFactory.saveRequest(CERTIFICATE, ID)).thenReturn(REQUEST);
    }
    ...
    void shouldPublishCertificateStatusUpdate() {
      ...
      verify(publishCertificateStatusUpdateService, times(1))
          .publish(CERTIFICATE, HandelsekodEnum.ANDRAT);
    }

    @Test
    void shouldPublishAnalyticsMessage() {
      final var analyticsMessage = CertificateAnalyticsMessage.builder().build();
      when(certificateAnalyticsMessageFactory.draftUpdated(CERTIFICATE))
