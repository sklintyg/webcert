...
class IntegratedUnitRegistryHelperTest {
  ...
  @Mock IntegreradeEnheterRegistry enheterRegistry;

  @Mock WebCertUser webCertUser;

  @InjectMocks IntegratedUnitRegistryHelper integratedUnitRegistryHelper;
  ...
  class AddUnit {
    ...
    class HasUser {
      ...
      @Test
      void shouldAdd() {
        when(webCertUser.getValdVardenhet()).thenReturn(new Vardenhet(UNIT_ID, UNIT_NAME));
        when(webCertUser.getValdVardgivare())
            .thenReturn(new Vardenhet(CARE_PROVIDER_ID, CARE_PROVIDER_NAME));
        final var expectedEntry =
            new IntegreradEnhetEntry(UNIT_ID, UNIT_NAME, CARE_PROVIDER_ID, CARE_PROVIDER_NAME);
  ...
  class AddUnitForCopy {
    ...
    void shouldAddUnitDataFromCopyUsingRegistryIfDeepIntegration() {
      ...
      final var expectedEntry =
          new IntegreradEnhetEntry(
              COPY_UNIT_ID, COPY_UNIT_NAME, COPY_CARE_PROVIDER_ID, COPY_CARE_PROVIDER_NAME);
    ...
    void shouldNotAddUnitDataFromCopyUsingRegistryIfNotDeepIntegration() {
      ...
      verify(enheterRegistry, times(0))
          .addIfSameVardgivareButDifferentUnits(anyString(), any(), anyString());
    }
  }
}
