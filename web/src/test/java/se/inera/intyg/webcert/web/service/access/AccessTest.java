...
public abstract class AccessTest {
  ...
  @Mock protected UtkastService utkastService;
  ...
  public static final Personnummer PERSONNUMMER =
      Personnummer.createPersonnummer(PERSONNUMMER_TOLVAN).get();
  ...
  protected void setupMocksForNoConditionsDifferentUnit() {
    final WebCertUser user =
        AccessServiceTestToolkit.createUserDifferentUnit(intygsTyp, accessServiceTestData);
    doReturn(user).when(webCertUserService).getUser();
    doReturn(false).when(patientDetailsResolver).isAvliden(PERSONNUMMER);
    doReturn(SekretessStatus.FALSE).when(patientDetailsResolver).getSekretessStatus(PERSONNUMMER);
    doReturn(false).when(webCertUserService).isUserAllowedAccessToUnit(enhetsId);
    doReturn(AccessServiceTestToolkit.createEmptyPreviousForUtkast(intygsTyp))
        .when(utkastService)
        .checkIfPersonHasExistingIntyg(PERSONNUMMER, user, null);
    doReturn(selectedVardgivare).when(user).getValdVardgivare();
  ...
  protected void setupMocksForDeceasedPatient() {
    final WebCertUser user = AccessServiceTestToolkit.createUser(intygsTyp, accessServiceTestData);
    doReturn(user).when(webCertUserService).getUser();
    doReturn(true).when(patientDetailsResolver).isAvliden(PERSONNUMMER);
    doReturn(SekretessStatus.FALSE).when(patientDetailsResolver).getSekretessStatus(PERSONNUMMER);
    doReturn(true).when(webCertUserService).isUserAllowedAccessToUnit(enhetsId);
    doReturn(AccessServiceTestToolkit.createEmptyPreviousForUtkast(intygsTyp))
        .when(utkastService)
  ...
  protected void setupMocksForInactiveUnit() {
    final WebCertUser user =
        AccessServiceTestToolkit.createUser(intygsTyp, accessServiceTestData, true);
    doReturn(user).when(webCertUserService).getUser();
    doReturn(false).when(patientDetailsResolver).isAvliden(PERSONNUMMER);
    doReturn(SekretessStatus.FALSE).when(patientDetailsResolver).getSekretessStatus(PERSONNUMMER);
    doReturn(true).when(webCertUserService).isUserAllowedAccessToUnit(enhetsId);
  ...
  protected void setupMocksForOnRenewFalse() {
    final WebCertUser user =
        AccessServiceTestToolkit.createUser(intygsTyp, accessServiceTestData, false, false);
    doReturn(user).when(webCertUserService).getUser();
    doReturn(false).when(patientDetailsResolver).isAvliden(PERSONNUMMER);
    doReturn(SekretessStatus.FALSE).when(patientDetailsResolver).getSekretessStatus(PERSONNUMMER);
    doReturn(true).when(webCertUserService).isUserAllowedAccessToUnit(enhetsId);
  ...
  protected void setupMocksForOnSekretessPatient() {
    ...
    doReturn(user).when(webCertUserService).getUser();
    doReturn(false).when(patientDetailsResolver).isAvliden(PERSONNUMMER);
    doReturn(SekretessStatus.TRUE).when(patientDetailsResolver).getSekretessStatus(PERSONNUMMER);
    doReturn(true).when(webCertUserService).isUserAllowedAccessToUnit(enhetsId);
    doReturn(AccessServiceTestToolkit.createEmptyPreviousForUtkast(intygsTyp))
        .when(utkastService)
        .checkIfPersonHasExistingIntyg(PERSONNUMMER, user, null);
