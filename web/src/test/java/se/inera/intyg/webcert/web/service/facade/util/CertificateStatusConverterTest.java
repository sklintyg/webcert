...
class CertificateStatusConverterTest {
  ...
  class GetStatusFromIntygStatusList {
    ...
    private final Status RECIEVED_STATUS =
        new Status(CertificateState.RECEIVED, "HSVARD", LocalDateTime.now());
    private final Status SENT_STATUS = new Status(CertificateState.SENT, "FK", LocalDateTime.now());
    private final Status CANCELLED_STATUS =
        new Status(CertificateState.CANCELLED, "HSVARD", LocalDateTime.now());

    @Test
    void shallReturnSignedIfListMissingCancelled() {
      assertEquals(
          CertificateStatus.SIGNED, getStatus(Arrays.asList(RECIEVED_STATUS, SENT_STATUS)));
    }
    ...
    void shallReturnRevokedIfListContainsCancelled() {
      assertEquals(
          ...
          getStatus(Arrays.asList(RECIEVED_STATUS, SENT_STATUS, CANCELLED_STATUS)));
    }
  }
