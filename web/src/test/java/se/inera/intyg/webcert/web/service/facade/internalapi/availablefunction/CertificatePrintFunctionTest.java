...
class CertificatePrintFunctionTest {
  ...
  private static final String NOT_QUESTION_SMITTBARAR_PENNING =
      "NOT_AVSTANGNING_SMITTSKYDD_SVAR_ID_27";
  ...
  private static final AvailableFunctionDTO EXPECTED_CUSTOMIZE_FUNCTION =
      AvailableFunctionFactory.customizePrint(true, FILE_NAME);
  private static final AvailableFunctionDTO EXPECTED_PRINT_FUNCTION =
      AvailableFunctionFactory.print(true, FILE_NAME);
  private static final AvailableFunctionDTO EXPECTED_SMITTBARAR_PENNING_FUNCTION =
      AvailableFunctionFactory.avstangningSmittskydd(true);

  @Mock AuthoritiesHelper authoritiesHelper;

  @InjectMocks private CertificatePrintFunction certificatePrintFunction;
  ...
  class ActiveFeature {
    ...
    void shouldOnlyReturnPrintCertificateIfTypeIsNotAg7804OrAg114() {
      final var certificate =
          buildCertificate(new Certificate(), FK7263, QUESTION_SMITTBARAR_PENNING, null);
    ...
    @Test
    void
        shoulReturnPrintCertificateAndSmittbarareInfoIfTypeIsAg7804ButQuestionSmittbararpenningIsTrue() {
      final var certificate =
          buildCertificate(new Certificate(), AG7804, QUESTION_SMITTBARAR_PENNING, true);
      final var result = certificatePrintFunction.get(certificate);
    ...
    void shouldOnlyReturnPrintCertificateIfTypeIsAg7804ButQuestionSmittbararpenningIsMissing() {
      final var certificate =
          buildCertificate(new Certificate(), AG7804, NOT_QUESTION_SMITTBARAR_PENNING, false);
      final var result = certificatePrintFunction.get(certificate);
      assertEquals(List.of(EXPECTED_PRINT_FUNCTION), result);
    }
    ...
    void shouldReturnCustomizePrintCertificateIfTypeIsAg7804AndQuestionSmittbararpenningIsFalse() {
      final var certificate =
          buildCertificate(new Certificate(), AG7804, QUESTION_SMITTBARAR_PENNING, false);
      final var result = certificatePrintFunction.get(certificate);
    ...
    void shouldReturnCustomizePrintCertificateIfTypeIsAg7804AndQuestionSmittbararpenningIsNull() {
      final var certificate =
          buildCertificate(new Certificate(), AG7804, QUESTION_SMITTBARAR_PENNING, null);
      final var result = certificatePrintFunction.get(certificate);
      assertEquals(List.of(EXPECTED_CUSTOMIZE_FUNCTION), result);
    }
    ...
    void shouldReturnCustomizePrintCertificateIfTypeIsAg114() {
      final var certificate =
          buildCertificate(new Certificate(), AG114, NOT_QUESTION_SMITTBARAR_PENNING, false);
      final var result = certificatePrintFunction.get(certificate);
      assertEquals(List.of(EXPECTED_CUSTOMIZE_FUNCTION), result);
    }
    ...
    void shouldReturnEmptyCertificateIsReplaced() {
      final var certificate =
          buildCertificateWithRelation(new Certificate(), CertificateRelationType.REPLACED);
      final var result = certificatePrintFunction.get(certificate);
    ...
    void shouldReturnEmptyCertificateIsComplemented() {
      final var certificate =
          buildCertificateWithRelation(new Certificate(), CertificateRelationType.COMPLEMENTED);
      final var result = certificatePrintFunction.get(certificate);
      assertEquals(Collections.emptyList(), result);
    }
  }
  ...
  private static Certificate buildCertificateWithRelation(
      Certificate certificate, CertificateRelationType type) {
    CertificateRelation[] relationChildren = {
      CertificateRelation.builder().type(type).status(CertificateStatus.SIGNED).build()
    };
    final var relations = CertificateRelations.builder().children(relationChildren).build();
  ...
  private static Certificate buildCertificate(
      Certificate certificate, String type, String questionId, Boolean selected) {
  ...
  private static Certificate buildCertificate(
      ...
      String questionId,
      Boolean selected,
      CertificateRelations certificateRelations) {
    ...
    certificate.setData(
        Map.of(
            ...
            CertificateDataElement.builder()
                .value(CertificateDataValueBoolean.builder().selected(selected).build())
                .build()));
    return certificate;
  }
}
