...
class ResourceLinkListHelperImplTest {
  ...
  @Mock CertificateRelationsConverter certificateRelationsConverter;
  @Mock AuthoritiesHelper authoritiesHelper;
  @InjectMocks private ResourceLinkListHelperImpl resourceLinkListHelper;
  ...
  public void setup(boolean renew, boolean read) {
    ...
    when(certificateAccessServiceHelper.isAllowToRenew(any(AccessEvaluationParameters.class)))
        .thenReturn(renew);
    when(certificateAccessServiceHelper.isAllowToRead(any(AccessEvaluationParameters.class)))
        .thenReturn(read);
  ...
  class RenewCertificate {
    ...
    public void shouldIncludeRenewResourceLinkIfActionLinkExists() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.SIGNED.toString(), ActionLinkType.FORNYA_INTYG);
    ...
    public void shouldNotIncludeRenewResourceLinkIfDraft() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.INCOMPLETE.toString(), ActionLinkType.FORNYA_INTYG);
      final var resourceLinks =
          resourceLinkListHelper.get(entry, CertificateListItemStatus.INCOMPLETE);
      assertEquals(0, resourceLinks.size());
    }
    ...
    void shouldIncludeRenewResourceLinkIfFromCertificateService() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.INCOMPLETE.toString(),
  ...
  class ForwardCertificate {
    ...
    public void shouldIncludeForwardResourceLinkIfActionLinkExists() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.INCOMPLETE.toString(),
              ActionLinkType.VIDAREBEFORDRA_UTKAST);
      final var resourceLinks =
          resourceLinkListHelper.get(entry, CertificateListItemStatus.INCOMPLETE);
      assertEquals(1, resourceLinks.size());
    ...
    public void shouldNotIncludeForwardResourceLinkIfLockedDraft() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.LOCKED.toString(), ActionLinkType.VIDAREBEFORDRA_UTKAST);
    ...
    public void shouldNotIncludeForwardResourceLinkIfSigned() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.SIGNED.toString(), ActionLinkType.VIDAREBEFORDRA_UTKAST);
      final var resourceLinks = resourceLinkListHelper.get(entry, CertificateListItemStatus.SIGNED);
      assertEquals(0, resourceLinks.size());
    }
    ...
    public void shouldIncludeForwardResourceLinkIfCompleteDraft() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.COMPLETE.toString(), ActionLinkType.VIDAREBEFORDRA_UTKAST);
      final var resourceLinks =
          resourceLinkListHelper.get(entry, CertificateListItemStatus.COMPLETE);
      assertEquals(1, resourceLinks.size());
      assertEquals(ResourceLinkTypeDTO.FORWARD_CERTIFICATE, resourceLinks.get(0).getType());
    }
  ...
  class ReadCertificate {
    ...
    public void shouldIncludeReadResourceLinkIfActionLinkExists() {
      ...
      final var entry =
          setupListIntygEntry(
              CertificateListItemStatus.INCOMPLETE.toString(), ActionLinkType.LASA_INTYG);
      final var resourceLinks =
          resourceLinkListHelper.get(entry, CertificateListItemStatus.INCOMPLETE);
      assertEquals(1, resourceLinks.size());
