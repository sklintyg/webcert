...
public class IntygServicePatientTest {
    ...
    private static final String CERTIFICATE_TYPE_VERSION = "6.8";
    ...
    public void setupIntygstjanstResponse() throws Exception {
        ...
        TsBasUtlatandeV6 utlatande = objectMapper.readValue(json, TsBasUtlatandeV6.class);

        CertificateResponse certificateResponse = new CertificateResponse(json, utlatande, buildCertificateMetaData(), false);
        when(moduleFacade.getCertificate(anyString(), anyString(), anyString())).thenReturn(certificateResponse);
    ...
    public void setupLookForIntygTypeInfo() throws IOException {
        ...
        typeInfo.setTypVersion(CERTIFICATE_TYPE_VERSION);
    }

    @Before
    public void IntygServiceConverter() throws Exception {
        when(moduleRegistry.getModuleApi(or(isNull(),any(String.class)), or(isNull(),any(String.class)))).thenReturn(moduleApi);
        json = FileUtils.getStringFromFile(new ClassPathResource("IntygServiceTest/utlatande-address.json").getFile());
        TsBasUtlatandeV6 utlatande = objectMapper.readValue(json, TsBasUtlatandeV6.class);
