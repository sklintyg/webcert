...
/**
 * Helper class for taking a list of arbitrary "id", "unitId", "personnummer", filter out any
 * personnummer being sekretessmarkerad and returning the result as a map of unitId -> count(id).
 *
 * <p>Used for getting counters of fråga/svar, ärenden and ej signerade utkast correct.
 *
...
public class StatisticsGroupByUtil {
  ...
  /**
   * Takes a list of object[] where each object[] is one of utkast, fraga/svar or arende represented
   * as:
   *
   * <p>[0] id (unique, this is what we want to count per enhetsId) [1] enhetsId [2] personnummer
   *
   * <p>This method will filter out any items belonging to a patient having sekretessmarkering and
   * return the result as a map: EnhetsId -> number of id for that unit.
   *
  ...
  public Map<String, Long> toSekretessFilteredMap(List<GroupableItem> groupableItems) {
    ...
    Map<Personnummer, SekretessStatus> sekretessStatusMap =
        patientDetailsResolver.getSekretessStatusForList(
            getPersonummerList(filteredGroupableItems));

    // update sekretess status
    filteredGroupableItems.forEach(
        item -> item.setSekretessStatus(sekretessStatusMap.get(createPnr(item.getPersonnummer()))));
