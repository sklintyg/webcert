...
public class UnderskriftServiceImpl implements UnderskriftService {
  ...
  public SignaturBiljett startSigningProcess(
    ...
    // Determine which method to use for creating the SignaturBiljett that contains the payload to
    // sign etc.
    ...
    switch (user.getAuthenticationMethod()) {
      ...
      case NET_ID:
      case FAKE:
        final var registerCertificateXml =
            utkastModelToXMLConverter.utkastToXml(updatedJson, intygsTyp);
        signaturBiljett =
            xmlUnderskriftService.skapaSigneringsBiljettMedDigest(
                intygsId,
                intygsTyp,
                version,
                Optional.of(updatedJson),
                signMethod,
                ticketId,
                userIpAddress,
      ...
      case MOBILT_BANK_ID:
        signaturBiljett =
            grpUnderskriftService.skapaSigneringsBiljettMedDigest(
                intygsId,
                intygsTyp,
                version,
                Optional.of(updatedJson),
  ...
  @Override
  public SignaturBiljett fakeSignature(
      String intygsId, String intygsTyp, long version, String ticketId) {
    WebCertUser user = webCertUserService.getUser();
    Utkast utkast = getUtkastForSignering(intygsId, version, user);
  ...
  /**
   * Called either when: - the /api/signature endpoint when the NetiD plugin has signed the
   * Base64-encoded SignedInfo XML.
   */
  @Override
  public SignaturBiljett netidSignature(String biljettId, byte[] signatur, String certifikat) {
    ...
    // Highly unlikely, but if the redis cache have crashed in between or similar, we need to handle
    // that the there's
    // no SignaturBiljett to be had.
    if (signaturBiljett == null) {
      String errMsg =
          "No SignaturBiljett found for ticketId '{}' when finalizing signature. "
    ...
    WebCertUser user = webCertUserService.getUser();
    Utkast utkast =
        getUtkastForSignering(signaturBiljett.getIntygsId(), signaturBiljett.getVersion(), user);

    SignaturBiljett finishedSignaturBiljett =
        xmlUnderskriftService.finalizeSignature(
            signaturBiljett, signatur, certifikat, utkast, user);
    finalizeSignature(utkast, user);
  ...
  /**
   * Makes sure the specified Utkast is ready for signing, and then returns it. If Utkast is not
   * ready for signing, a WebCertServiceException is thrown.
   *
  ...
  private Utkast getUtkastForSignering(String intygId, long version, WebCertUser user) {
    ...
    if (utkast == null) {
      ...
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.DATA_NOT_FOUND,
          "Internal error signing utkast, the utkast '" + intygId + "' could not be found");
    }
    ...
    if (utkast.getVersion() != version) {
      ...
      LOG.warn(
          "Utkast '{}' med status '{}' kunde inte signeras. Måste vara i status {}",
          intygId,
          utkast.getStatus(),
          UtkastStatus.DRAFT_COMPLETE);
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.INVALID_STATE,
          "Internal error signing utkast, the utkast '"
              + intygId
              + "' was not in state "
              + UtkastStatus.DRAFT_COMPLETE);
    }
  ...
  private String updateJsonWithVardpersonAndSigneringsTid(Utkast utkast, WebCertUser user) {
    ...
    try {
      // VardpersonReferens vardpersonReferens =
      // UpdateUserUtil.createVardpersonFromWebCertUser(user);
      ModuleApi moduleApi =
          moduleRegistry.getModuleApi(utkast.getIntygsTyp(), utkast.getIntygTypeVersion());
      Vardenhet vardenhetFromJson =
          moduleApi
              .getUtlatandeFromJson(utkast.getModel())
              .getGrundData()
              .getSkapadAv()
              .getVardenhet();
      return moduleApi.updateBeforeSigning(
          utkast.getModel(),
  ...
  private void verifyAccessToSignDraft(Utkast utkast) {
    final AccessResult accessResult =
        draftAccessService.allowToSignDraft(
            utkast.getIntygsTyp(),
