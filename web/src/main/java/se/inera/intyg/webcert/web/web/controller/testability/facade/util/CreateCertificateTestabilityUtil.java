...
public class CreateCertificateTestabilityUtil {
  ...
  public String createNewCertificate(
    ...
    final var hosPersonal =
        getHoSPerson(createCertificateRequest.getPersonId(), createCertificateRequest.getUnitId());
    ...
    if (activeAndSupportsType(
        createCertificateRequest.getCertificateType(),
        createCertificateRequest.getCertificateTypeVersion())) {

      final var modelIdDTO =
    ...
    final var createNewDraftRequest =
    ...
    updateIntygstjanstTestabilityUtil.update(
        utkast, hosPersonal, createCertificateRequest.getPersonId());
  ...
  private String getUpdateJsonModel(
      Utkast utkast, CreateCertificateRequestDTO createCertificateRequest) {
    ...
    try {
      ...
      final var fillType =
          MINIMAL.equals(createCertificateRequest.getFillType())
              ? FillType.MINIMAL
              : FillType.MAXIMAL;
  ...
  private void downgradeMinorVersionIfNecessary(
      CreateNewDraftRequest createNewDraftRequest, Utkast utkast) {
    final var latestVersionForSameMajorVersion =
        intygTextsService.getLatestVersionForSameMajorVersion(
            createNewDraftRequest.getIntygType(), createNewDraftRequest.getIntygTypeVersion());
    if (latestVersionForSameMajorVersion != null) {
      utkast.setModel(
          utkast
              .getModel()
              .replace(
                  latestVersionForSameMajorVersion, createNewDraftRequest.getIntygTypeVersion()));
    }
  }
  ...
  private void updateCertificateWithRequestedStatus(
    ...
    if (createCertificateRequest.getStatus() == CertificateStatus.UNSIGNED) {
      ...
      throw new IllegalArgumentException(
          String.format(
              "Status '%s' not supported when creating certificate!",
  ...
  private Patient getPatient(String patientId, String type, String typeVersion) {
    ...
    final var patient =
        csIntegrationService.certificateTypeExists(type).isPresent()
            ? new Patient()
            : patientDetailsResolver.resolvePatient(personnummer, type, typeVersion);
