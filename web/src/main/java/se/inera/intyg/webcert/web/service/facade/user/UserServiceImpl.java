...
public class UserServiceImpl implements UserService {
  ...
  public User getLoggedInUser() {
    ...
    return User.builder()
        ...
        .loggedInUnit(
            Unit.builder()
                .unitName(loggedInUnit.getNamn())
                .unitId(loggedInUnit.getId())
                .isInactive(isInactiveUnit)
                .build())
        .loggedInCareUnit(
            Unit.builder()
                .unitName(loggedInCareUnit.getNamn())
                .unitId(loggedInCareUnit.getId())
                .build())
        .loggedInCareProvider(
            Unit.builder()
                .unitName(loggedInCareProvider.getNamn())
                .unitId(loggedInCareProvider.getId())
                .build())
  ...
  private static Optional<Personnummer> getPersonId(WebCertUser webCertUser) {
    return webCertUser.getPersonId() == null
        ? Optional.empty()
        : Personnummer.createPersonnummer(webCertUser.getPersonId());
  }
  ...
  private List<Unit> getUnits(List<Mottagning> mottagningar) {
    return mottagningar.stream()
        .map(
            mottagning ->
                Unit.builder().unitId(mottagning.getId()).unitName(mottagning.getNamn()).build())
  ...
  public Vardenhet getLoggedInCareUnit(WebCertUser webCertUser) {
    ...
    final var currentCareProvider =
        webCertUser.getVardgivare().stream()
  ...
  private Vardenhet findCareUnit(List<Vardenhet> careUnits, String loggedInUnitId) {
    return careUnits.stream()
        .filter(
            careUnit ->
                careUnit.getId().equalsIgnoreCase(loggedInUnitId)
                    || isSubunit(careUnit.getMottagningar(), loggedInUnitId))
        .findFirst()
        .orElseThrow();
  }
