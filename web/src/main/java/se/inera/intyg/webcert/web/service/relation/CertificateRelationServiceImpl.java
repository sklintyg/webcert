...
public class CertificateRelationServiceImpl implements CertificateRelationService {
  ...
  private Relations.FrontendRelations prepareChildRelationDataForFrontend(
      List<WebcertCertificateRelation> childRelations) {
    Collections.sort(
        childRelations, (r1, r2) -> r1.getSkapad().compareTo(r2.getSkapad()) * -1); // Descending
    // order.
    Relations.FrontendRelations latestChildRelations = new Relations.FrontendRelations();
    latestChildRelations.setReplacedByIntyg(
        findRelationOfType(childRelations, RelationKod.ERSATT, true));
    latestChildRelations.setReplacedByUtkast(
        findRelationOfType(childRelations, RelationKod.ERSATT, false));
    latestChildRelations.setComplementedByIntyg(
        findRelationOfType(childRelations, RelationKod.KOMPLT, true));
    latestChildRelations.setComplementedByUtkast(
        findRelationOfType(childRelations, RelationKod.KOMPLT, false));
    latestChildRelations.setUtkastCopy(
        findRelationOfType(childRelations, RelationKod.KOPIA, false));
    return latestChildRelations;
  }

  private WebcertCertificateRelation findRelationOfType(
      List<WebcertCertificateRelation> relations, RelationKod relationKod, boolean signed) {
    for (WebcertCertificateRelation wcr : relations) {
      if (wcr.getRelationKod() == relationKod) {
        if ((signed && wcr.getStatus() == UtkastStatus.SIGNED)
            || (!signed
                && (wcr.getStatus() == UtkastStatus.DRAFT_INCOMPLETE
                    || wcr.getStatus() == UtkastStatus.DRAFT_COMPLETE))) {
  ...
  /**
   * Implementation detail: Spring JPA Repository @Query doesn't really work with
   * getSingleResult()-style queries, thus this method gets a list of "parentRelations" even though
   * there can never be more than one parent.
   */
  @Override
  public Optional<WebcertCertificateRelation> findParentRelation(String intygsId) {
    List<WebcertCertificateRelation> parentRelations =
        utkastRepoCustom.findParentRelation(intygsId);
  ...
  public List<WebcertCertificateRelation> findChildRelations(String intygsId) {
    return utkastRepoCustom.findChildRelations(intygsId).stream()
        .sorted((r1, r2) -> r2.getSkapad().compareTo(r1.getSkapad()))
        .collect(Collectors.toList());
  }

  @Override
  public Optional<WebcertCertificateRelation> getNewestRelationOfType(
      String intygsId, RelationKod relationKod, List<UtkastStatus> allowedStatuses) {
