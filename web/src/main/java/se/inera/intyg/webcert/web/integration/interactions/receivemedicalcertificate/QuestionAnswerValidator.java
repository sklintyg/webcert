...
public final class QuestionAnswerValidator {
  ...
  private static void validateLakarutlatande(
      List<String> messages, LakarutlatandeEnkelType lakarutlatande) {
  ...
  private static void validateVardgivare(List<String> messages, HosPersonalType hosPersonal) {
    if (!Constants.HSA_ID_OID.equals(
        hosPersonal.getEnhet().getVardgivare().getVardgivareId().getRoot())) {
      messages.add("Felaktig vardgivareid root");
    }
    if (Strings.isNullOrEmpty(
        hosPersonal.getEnhet().getVardgivare().getVardgivareId().getExtension())) {
