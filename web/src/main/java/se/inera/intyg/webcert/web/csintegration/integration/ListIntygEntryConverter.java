...
public class ListIntygEntryConverter {
  ...
  public ListIntygEntry convert(Certificate certificate) {
    ...
    listIntygEntry.setStatus(
        convertStatus(metadata.getStatus(), metadata.isValidForSign(), metadata.isSent()));
    ...
    if (certificate.getLinks() != null) {
      certificate
          .getLinks()
          .forEach(link -> listIntygEntry.getLinks().add(convertResourceLink(link)));
  ...
  private Relations convertRelations(CertificateRelations relations) {
    if (relations == null
        || relations.getChildren() == null
        || relations.getChildren().length == 0) {
      return new Relations();
    }

    final var webcertCertificateRelation =
        Stream.of(relations.getChildren())
    ...
    if (webcertCertificateRelation.getRelationKod() == RelationKod.ERSATT) {
      if (webcertCertificateRelation.getStatus() == UtkastStatus.SIGNED) {
        ...
        convertedRelations
            ...
            .setReplacedByUtkast(webcertCertificateRelation);
      }
    } else if (webcertCertificateRelation.getRelationKod() == RelationKod.KOMPLT) {
      if (webcertCertificateRelation.getStatus() == UtkastStatus.SIGNED) {
        convertedRelations
            .getLatestChildRelations()
            .setComplementedByIntyg(webcertCertificateRelation);
      } else {
        convertedRelations
            .getLatestChildRelations()
            .setComplementedByUtkast(webcertCertificateRelation);
      }
    }
