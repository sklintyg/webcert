...
public class DssSignatureService {
  ...
  private static final String NAMEID_FORMAT_ENTITY =
  ...
  private static final String REQUESTED_CERT_ATTRIBUTE_GIVEN_NAME_SAML_NAME =
  ...
  private static final String REQUESTED_CERT_ATTRIBUTE_SURNAME_SAML_NAME =
  ...
  private static final String REQUESTED_CERT_ATTRIBUTE_SERIAL_NUMBER_SAML_NAME =
  ...
  public DssSignatureService(
      ...
      WebCertUserService userService,
        ...
        .noneMatch(
            value -> {
              ...
              if (value.endsWith("*")) {
                ...
                return currentCareUnitHsaId
                    .toUpperCase()
                    .startsWith(wildcardRemovedSubstring.toUpperCase());
              } else {
  ...
  public DssSignRequestDTO createSignatureRequestDTO(SignaturBiljett sb) {
    ...
    String signRequest =
        dssSignMessageService.signSignRequest(createSignRequest(dateTimeNow, sb, transactionID));
    String base64EncodedSignRequest =
        Base64.getEncoder().encodeToString(signRequest.getBytes(StandardCharsets.UTF_8));
  ...
  private SignRequest createSignRequest(
      LocalDateTime dateTimeNow, SignaturBiljett sb, String transactionID) {
  ...
  public String createTransactionID() {
    ...
    return String.format(
        "%s-%s-%s-%s", formatIdField(customerId), formatIdField(applicationId), timestamp, uuid);
  }
  ...
  private JAXBElement<SignRequestExtensionType> createSignRequestExtension(
      String intygsId, LocalDateTime dateTimeNow) {
    ...
    final var idpUrl =
        dssSignMessageIdpProvider.get(userService.getUser().getIdentityProviderForSign());
    ...
    signRequester.setValue(
        dssClientEntityHostUrl
            + SignatureApiController.SIGNATUR_API_CONTEXT_PATH
            + SignatureApiController.SIGN_SERVICE_METADATA_PATH);
  ...
  private SignMessageType createSignMessage(String intygsId, String idpUrl) {
    ...
    String message =
        this.signMessage
            .replace("{intygsTyp}", intygsTyp)
            .replace(
                "{patientPnr}",
                patientPersonnummer != null ? patientPersonnummer.getPersonnummerWithDash() : "")
  ...
  private CertRequestPropertiesType createCertRequestProperties() {
    ...
    addMappedAttribute(
        ...
        REQUESTED_CERT_ATTRIBUTE_GIVEN_NAME_REF,
        REQUESTED_CERT_ATTRIBUTE_GIVEN_NAME,
        true,
        REQUESTED_CERT_ATTRIBUTE_GIVEN_NAME_SAML_NAME);
    addMappedAttribute(
        requestedAttributesType,
        REQUESTED_CERT_ATTRIBUTE_SURNAME_REF,
        REQUESTED_CERT_ATTRIBUTE_SURNAME,
        true,
        REQUESTED_CERT_ATTRIBUTE_SURNAME_SAML_NAME);
    addMappedAttribute(
        requestedAttributesType,
        REQUESTED_CERT_ATTRIBUTE_SERIAL_NUMBER_REF,
        REQUESTED_CERT_ATTRIBUTE_SERIAL_NUMBER,
        true,
        REQUESTED_CERT_ATTRIBUTE_SERIAL_NUMBER_SAML_NAME);
    addMappedAttribute(
        requestedAttributesType,
        REQUESTED_CERT_ATTRIBUTE_COMMON_NAME_REF,
        REQUESTED_CERT_ATTRIBUTE_COMMON_NAME,
        true,
        REQUESTED_CERT_ATTRIBUTE_COMMON_NAME_SAML_NAME);

    certRequestPropertiesType.setRequestedCertAttributes(requestedAttributesType);
    return certRequestPropertiesType;
  }

  private void addMappedAttribute(
      RequestedAttributesType requestedAttributesType,
      String ref,
      String name,
      boolean req,
      String attribute) {
    requestedAttributesType
        .getRequestedCertAttribute()
        .add(createMappedAttributeType(ref, name, req, attribute));
  }

  private MappedAttributeType createMappedAttributeType(
      String ref, String name, boolean req, String attribute) {
  ...
  public SignaturBiljett receiveSignResponse(String relayState, String eIdSignResponse) {
    ...
    try {
      ...
      if (RESULTMAJOR_SUCCESS.equals(resultMajor)) {
        ...
        var signTasks =
            (JAXBElement<SignTasksType>)
                signResponse.getSignatureObject().getOther().getAny().get(0);
        var signTaskDataType = signTasks.getValue().getSignTaskData().get(0);
        byte[] signature = signTaskDataType.getBase64Signature().getValue();

        var signResponseExtension =
            (JAXBElement<SignResponseExtensionType>)
                signResponse.getOptionalOutputs().getAny().get(0);
        var certificates =
            signResponseExtension.getValue().getSignatureCertificateChain().getX509Certificate();
        ...
        var resultMessage =
            result.getResultMessage() == null ? null : result.getResultMessage().getValue();

        SignaturBiljett signaturBiljett =
            redisTicketTracker.updateStatus(relayState, SignaturStatus.ERROR);
        monitoringLogService.logSignServiceErrorReceived(
            relayState, signaturBiljett.getIntygsId(), resultMajor, resultMinor, resultMessage);
      ...
      monitoringLogService.logSignResponseInvalid(
          ...
          "Could not unmarshal sign response: " + e.getMessage());
      return signaturBiljett;
    } catch (Exception e) {
      LOG.error(
          "Unexpected error occurred when handling sign response with transaction id '%s'"
              .formatted(relayState),
          e);
      SignaturBiljett signaturBiljett =
          redisTicketTracker.updateStatus(relayState, SignaturStatus.ERROR);
      monitoringLogService.logSignResponseInvalid(
          relayState, signaturBiljett.getIntygsId(), "Internal problem: " + e.getMessage());
      return signaturBiljett;
    }
  }
  ...
  private SignResponse unMarshallSignResponse(String eIdSignResponse) throws JAXBException {
    JAXBContext context =
        JAXBContext.newInstance(
            SignResponse.class, SignResponseExtensionType.class, SignTasksType.class);
    return (SignResponse)
        context
            .createUnmarshaller()
  ...
  public String findReturnErrorUrl(String intygsId, String ticketId) {
    ...
    if (utkastOptional.isPresent()) {
      ...
      return String.format(
          "%s/#/%s/%s/edit/%s/?error&ticket=%s",
          dssClientResponseHostUrl, intygsTyp, intygTypeVersion, intygsId, ticketId);
    } else {
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.INTERNAL_PROBLEM, "Can't find certificate to return to!");
    }
  }
  ...
  public String findReturnUrl(String intygsId) {
    ...
    if (utkastOptional.isPresent()) {
      ...
      return String.format(
          "%s/#/intyg/%s/%s/%s/?signed",
          dssClientResponseHostUrl, intygsTyp, intygTypeVersion, intygsId);
    } else {
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.INTERNAL_PROBLEM, "Can't find certificate to return to!");
    }
