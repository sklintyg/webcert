...
/**
 * This class is responsible for implementing GE-002, e.g. requirements on how to fetch patient
 * details for a given intygstyp.
 *
 * <p>It's over-complex and needs refactoring.
 *
...
public class PatientDetailsResolverImpl implements PatientDetailsResolver {
   ...
   * PUService} in certain consumer classes.
   */
  @Override
  ...
  @Override
  public Map<Personnummer, SekretessStatus> getSekretessStatusForList(
    ...
    List<Personnummer> distinctPersonnummerList =
        personnummerList.stream().distinct().collect(Collectors.toList());
    ...
    persons.forEach(
        (key, value) -> {
          if (value != null && value.getStatus() == PersonSvar.Status.FOUND) {
            sekretessStatusMap.put(
                key,
                value.getPerson().sekretessmarkering()
                    ? SekretessStatus.TRUE
                    : SekretessStatus.FALSE);
          } else {
  ...
  @Override
  public Map<Personnummer, PatientDetailsResolverResponse> getPersonStatusesForList(
      List<Personnummer> personnummerList) {
    ...
    persons.forEach(
        (key, value) -> {
          ...
          if (value != null && value.getStatus() == PersonSvar.Status.FOUND) {
            ...
            patientResponse.setProtectedPerson(
                value.getPerson().sekretessmarkering()
                    ? SekretessStatus.TRUE
                    : SekretessStatus.FALSE);
          } else {
  ...
  public boolean isPatientAddressChanged(Patient oldPatient, Patient newPatient) {
    return oldPatient != null
        && (newPatient == null
            || (oldPatient.getPostadress() != null
  ...
  public Patient resolvePatient(
      Personnummer personnummer, String intygsTyp, String intygsTypVersion) {
    WebCertUser user = null;
    if (webCertUserService.hasAuthenticationContext()) {
      user = webCertUserService.getUser();
    }

    // Make sure any external intygstyp representations (such as TSTRK1007) are mapped to our
    // internal types.
  ...
  private Patient resolvePatient(
      Personnummer personnummer, WebCertUser user, PatientDetailResolveOrder resolveOrder) {
    PersonSvar personSvar = getPersonSvar(personnummer);
    ...
    if (isPredecessorStrategy(resolveOrder)) {
      List<Utkast> utkastList =
          getPredecessor(personnummer, user, resolveOrder.getPredecessorType());
                ...
                .orElseThrow(
                    () ->
                        new IllegalStateException(
                            "List was > 0 but findFirst() returned no result."));
        try {
          ModuleApi moduleApi =
  ...
  private void resolvePatientAdressDetails(
      Patient patient,
      PatientDetailResolveOrder resolveOrder,
      PersonSvar personSvar,
      WebCertUser user,
      Utlatande predecessor) {
  ...
  private void resolvePatientOtherDetails(
      Patient patient,
      PatientDetailResolveOrder resolveOrder,
      PersonSvar personSvar,
      WebCertUser user,
    ...
    if (user.getOrigin().equals(UserOriginType.DJUPINTEGRATION.name())) {
      ...
      patient.setFullstandigtNamn(
          Joiner.on(' ')
              .skipNulls()
              .join(
                  parameters.getFornamn(), parameters.getMellannamn(), parameters.getEfternamn()));
      return true;
    }
    return false;
  }
  ...
  private List<Utkast> getPredecessor(
      Personnummer personnummer, WebCertUser user, String intygsTyp) {
    ...
    if (user.getOrigin().equals(UserOriginType.DJUPINTEGRATION.name())) {
      ...
      utkastList.addAll(
          utkastRepository.findDraftsByPatientAndEnhetAndStatus(
              personnummer.getPersonnummerWithDash(),
              Collections.singletonList(user.getValdVardenhet().getId()),
              UTKAST_STATUSES,
              Sets.newHashSet(intygsTyp)));
    }
    return utkastList;
