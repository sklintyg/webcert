...
public abstract class BaseXMLSignatureService extends BaseSignatureService {
  ...
  protected FinalizedCertificateSignature finalizeXMLDSigSignatureForCS(
      String x509certificate, SignaturBiljett biljett, byte[] rawSignature) {
    try {
      ...
      final var certificate =
          signCertificateService.sign(biljett.getIntygsId(), signatureXml, biljett.getVersion());
      biljett.setStatus(SignaturStatus.SIGNERAD);
  ...
  private void storeX509InSignatureType(
      String x509certificate, IntygXMLDSignature intygXmldSignature) {
  ...
  private void performBasicSignatureValidation(
      String x509certificate, Utkast utkast, IntygXMLDSignature intygXmldSignature) {
    String utkastXml =
        utkastModelToXMLConverter.utkastToXml(
            intygXmldSignature.getIntygJson(), utkast.getIntygsTyp());
    String finalXml =
        prepareSignatureService.encodeSignatureIntoSignedXml(
            intygXmldSignature.getSignatureType(), utkastXml);
    if (x509certificate != null && !x509certificate.isEmpty()) {
      boolean validationResult =
          xmldSigService.validateSignatureValidity(finalXml, false).isValid();
      if (!validationResult) {
        throw new WebCertServiceException(
            WebCertServiceErrorCodeEnum.INTERNAL_PROBLEM, "Signature is invalid.");
      }
    }
  }
  ...
  private void applySignature(
      byte[] rawSignature, IntygXMLDSignature intygXmldSignature, SignMethod signMethod) {
    ...
    switch (signMethod) {
      ...
      case GRP:
        throw new IllegalStateException(
            "We do not handle signatures as XMLDSig for GRP, "
                + "if you're here something has gone wrong.");
    }
    intygXmldSignature.getSignatureType().setSignatureValue(svt);
  }

  protected SignaturBiljett createAndPersistSignatureForXMLDSig(
      Utkast utkast, SignaturBiljett biljett, String signaturXml, WebCertUser user) {
    IntygXMLDSignature intygXMLDSignature = (IntygXMLDSignature) biljett.getIntygSignature();

    final String payloadJson = intygXMLDSignature.getIntygJson();

    final String signingXmlHash =
        Base64.getEncoder()
            .encodeToString(
                intygXMLDSignature
                    .getSignatureType()
    ...
    // Use the JSON from the DB temp storage. Convert it (again) into XML and do a digest on it. It
    // MUST
    ...
    TransformAndDigestResponse transformAndDigestResponse =
        prepareSignatureService.transformAndGenerateDigest(utkastXml, biljett.getIntygsId());

    checkDigests(
        utkast.getIntygsId(),
        signingXmlHash,
        new String(transformAndDigestResponse.getDigest(), StandardCharsets.UTF_8));
    checkVersion(utkast, biljett);

    // For WC 6.1, we want to store the following:
    // INTYG_DATA - the actual data we're going to send to Intygstjansten OR the <intyg> element
    // WITHOUT signature.
    // INTYG_HASH - the hash of the canonicalized <intyg>...</intyg> we've signed on.
    // SIGNATUR_DATA - the base64-encoded <intyg>...<Signature>...</Signature></intyg> containing
    // the canonicalized
