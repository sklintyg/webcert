...
public class CertificateToIntygInfoConverter {
  ...
  private final CertificateRelationsToIntygInfoEventsConverter
  ...
  public WcIntygInfo convert(Certificate certificate, List<Question> questions) {
    ...
    wcIntygInfo.setSentToRecipient(
        metadata.isSent() && metadata.getRecipient() != null
    ...
    final var createdEvent =
        new IntygInfoEvent(
            Source.WEBCERT, certificate.getMetadata().getCreated(), IntygInfoEventType.IS001);
    ...
    if (certificate.getMetadata().getSigned() != null) {
      signedEvent =
          new IntygInfoEvent(
              Source.WEBCERT, certificate.getMetadata().getSigned(), IntygInfoEventType.IS004);
    ...
    if (certificate.getMetadata().isSent() && certificate.getMetadata().getRecipient() != null) {
      sentEvent =
          new IntygInfoEvent(
              Source.WEBCERT,
              certificate.getMetadata().getRecipient().getSent(),
    ...
    if (certificate.getMetadata().getRevokedAt() != null) {
      revokedEvent =
          new IntygInfoEvent(
              Source.WEBCERT, certificate.getMetadata().getRevokedAt(), IntygInfoEventType.IS009);
    ...
    if (certificate.getMetadata().getReadyForSign() != null) {
      readyForSignEvent =
          new IntygInfoEvent(
              Source.WEBCERT,
              certificate.getMetadata().getReadyForSign(),
              IntygInfoEventType.IS018);
    }

    wcIntygInfo.setEvents(
        Streams.concat(notificationEvents.stream(), relationEvents.stream())
  ...
  private static List<Question> getComplementsAnswered(List<Question> questions) {
    return questions.stream()
        .filter(question -> question.getType() == QuestionType.COMPLEMENT && question.isHandled())
        .toList();
  }
  ...
  private static List<Question> getAdminQuestionsSent(List<Question> questions) {
    return questions.stream()
        .filter(
            question ->
                question.getType() != QuestionType.COMPLEMENT
