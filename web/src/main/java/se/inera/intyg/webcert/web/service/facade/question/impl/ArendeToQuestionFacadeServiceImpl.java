...
public class ArendeToQuestionFacadeServiceImpl implements GetQuestionsFacadeService {
  ...
  @Autowired
  public ArendeToQuestionFacadeServiceImpl(
  ...
  public List<Question> getQuestions(String certificateId) {
    ...
    final var questionList =
        getQuestionList(
            arendenInternal,
            complementsMap,
            answersByCertificate,
            answersMap,
            remindersMap,
            answersDraftMap);
  ...
  private List<CertificateRelation> getAnswersByCertificate(
      String certificateId, Map<String, Complement[]> complementsMap) {
  ...
  private Map<String, Complement[]> getComplementsMap(List<Arende> arendenInternal) {
    final var complementQuestions =
        arendenInternal.stream().filter(isComplementQuestion()).collect(Collectors.toList());
    ...
    return arendenInternal.stream()
        ...
        .collect(
            Collectors.groupingBy(
                Arende::getPaminnelseMeddelandeId,
                HashMap::new,
                Collectors.toCollection(ArrayList::new)));
  }

  private List<Question> getQuestionList(
      List<Arende> questions,
      Map<String, Complement[]> complementsMap,
      List<CertificateRelation> answersByCertificate,
      Map<String, Arende> answersMap,
      Map<String, List<Arende>> remindersMap,
  ...
  private CertificateRelation getAnsweredByCertificate(
      Arende question, List<CertificateRelation> answersByCertificate) {
