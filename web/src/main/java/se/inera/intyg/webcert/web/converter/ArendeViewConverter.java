...
public class ArendeViewConverter {
  ...
  private static String getThreadRootMessageId(Arende arende) {
    String referenceId =
        (arende.getSvarPaId() != null) ? arende.getSvarPaId() : arende.getPaminnelseMeddelandeId();
        ...
        .kompletteringar(
            ImmutableList.copyOf(
                convertToMedicinsktArendeView(
                    arende.getKomplettering(), arende.getIntygsId(), arende.getIntygTyp())))
        .svarPaId(arende.getSvarPaId())
        .build();
  }

  public ArendeConversationView convertToArendeConversationView(
      Arende fraga,
      Arende svar,
  ...
  /**
   * Creates a list of ArendeConversationView for the given list of messages (Arende), for the
   * specific intyg. All messages will get grouped together in pairs of question-answer where such
   * relations exist. If the specified intyg has been answered with another intyg, that information
   * will be added to the ArendeConversationView.
   *
  ...
  private ArendeConversationView createConversationViewFromArendeList(
      List<Arende> messagesInThread,
      List<AnsweredWithIntyg> kompltForIntyg,
      List<ArendeDraft> arendeDrafts) {
    Optional<Arende> fraga =
        messagesInThread.stream()
            .filter(a -> getArendeType(a) == ArendeType.FRAGA)
            .reduce(
                (element, otherElement) -> {
                  throw new IllegalArgumentException("More than 1 fraga found.");
                });
    Optional<Arende> svar =
        messagesInThread.stream()
    ...
    if (!svar.isPresent() && fraga.get().getAmne() == ArendeAmne.KOMPLT) {
      komplt =
          AnsweredWithIntygUtil.returnOldestKompltOlderThan(
              fraga.get().getTimestamp(), kompltForIntyg);
    }
    return convertToArendeConversationView(
        fraga.get(), svar.orElse(null), komplt, paminnelser, draftText);
  }

  private List<MedicinsktArendeView> convertToMedicinsktArendeView(
      List<MedicinsktArende> medicinskaArenden, String intygsId, String intygsTyp) {
    ...
    for (MedicinsktArende arende : medicinskaArenden) {
      ...
      MedicinsktArendeView view =
          MedicinsktArendeView.builder()
  ...
  private String getJsonPropertyHandle(
