...
public class ListCertificatesForCareWithQAResponderImpl
    implements ListCertificatesForCareWithQAResponderInterface {

  private final IntygService intygService;
  private final GetCertificatesWithQAFromCertificateService
      getCertificatesWithQAFromCertificateService;
  ...
  @Override
  @PerformanceLogging(
      eventAction = "list-certificates-for-care-with-qa",
      eventType = MdcLogConstants.EVENT_TYPE_ACCESS)
  public ListCertificatesForCareWithQAResponseType listCertificatesForCareWithQA(
      String s, ListCertificatesForCareWithQAType request) {
    ...
    log.info(
        "Started processing request: PersonId: '{}' - CareProviderId '{}' - UnitIds '{}'",
    ...
    try {
      final var notifications =
          notificationService.findNotifications(intygWithNotificationsRequest);
      final var listItemsFromCS = getCertificatesWithQAFromCertificateService.get(notifications);
      final var intygWithNotifications =
          intygService.listCertificatesForCareWithQA(
              notifications.stream()
                  .filter(
                      removeNotificationsRelatedToCertificatesFromCertificateService(
                          listItemsFromCS))
  ...
  private java.util.List<ListItem> buildListItemsForWC(
      java.util.List<IntygWithNotificationsResponse> intygWithNotifications) {
    ...
    for (IntygWithNotificationsResponse intygHolder : intygWithNotifications) {
      ...
      handelseList
          .getHandelse()
          .addAll(
              intygHolder.getNotifications().stream().map(HandelseFactory::toHandelse).toList());
  ...
  private static IntygWithNotificationsRequest getIntygWithNotificationsRequest(
      ListCertificatesForCareWithQAType request) {
    IntygWithNotificationsRequest.Builder builder =
        new IntygWithNotificationsRequest.Builder()
            .setPersonnummer(
                Personnummer.createPersonnummer(request.getPersonId().getExtension()).get());
