...
public class CertificateForPatientServiceImpl implements CertificateForPatientService {
  ...
  private List<ListIntygEntry> getCertificatesFromIT(Personnummer patientId, List<String> units) {
    ...
    final var certificatesFromIT =
        deserialize(
            certificatesForPatientCache.get(
                key,
                () -> {
                  final var certificates = intygService.listIntygFromIT(units, patientId);
                  LOG.debug(
                      "'{}' certificates from IntygService was put in cache for key '{}'",
                      certificates.size(),
                      key);
                  return objectMapper.writeValueAsString(certificates);
                }));
    LOG.debug(
        "IntygsService returned {} certificates from cache for key '{}'",
        certificatesFromIT.size(),
        key);
  ...
  private Map<String, FrontendRelations> getRelationsMap(List<Utkast> utkasts) {
    ...
    getRelevantRelations(utkasts)
        .forEach(
            relation -> {
              final var frontendRelations =
                  relationsMap.computeIfAbsent(
                      relation.getIntygsId(), id -> new FrontendRelations());
  ...
  private ListIntygEntry decorateWithRelation(
      ListIntygEntry entry, Map<String, FrontendRelations> relationsMap) {
    entry
        .getRelations()
        .setLatestChildRelations(
            relationsMap.getOrDefault(entry.getIntygId(), new FrontendRelations()));
    return entry;
  }
  ...
  private List<WebcertCertificateRelation> getRelevantRelations(List<Utkast> utkasts) {
    ...
    return utkasts.stream()
        .filter(
            utkast ->
                utkast.getRelationIntygsId() != null
                    && relevantRelationCodes.contains(utkast.getRelationKod())
  ...
  private static List<ListIntygEntry> merge(
      List<ListIntygEntry> certificatesFromWC, List<ListIntygEntry> certificatesFromIT) {
    return Stream.concat(
            certificatesFromIT.stream()
                .filter(fromIT -> keepITCertificate(certificatesFromWC, fromIT)),
            certificatesFromWC.stream()
                .filter(fromWC -> keepWCCertificate(certificatesFromIT, fromWC)))
        .sorted(comparing(ListIntygEntry::getLastUpdatedSigned, reverseOrder()))
        .collect(Collectors.toList());
  }

  private static boolean keepITCertificate(
      List<ListIntygEntry> certificatesFromWC, ListIntygEntry fromIT) {
    return certificatesFromWC.stream()
        .filter(
            fromWC ->
                matchingIds(fromIT, fromWC)
                    && !isCancelled(fromIT)
                    && (!isOnlySentInIT(fromIT, fromWC)
                        || isOnlySentInIT(fromIT, fromWC) && isCancelled(fromWC)))
        .findAny()
        .isEmpty();
  }

  private static boolean keepWCCertificate(
      List<ListIntygEntry> certificatesFromIT, ListIntygEntry fromWC) {
