...
public class MessageImportServiceImpl implements MessageImportService {
  ...
  private IntygContentHolder getCertificate(String certificateId) {
    try {
      ...
      LOG.error(
          "Could not find the certificate to import messages for. CertificateId: " + certificateId,
          ex);
      return null;
    }
  }
  ...
  private List<MessageFromIT> removeMessageToExclude(
      List<MessageFromIT> messages, String excludeMessageId) {
  ...
  private void processMessage(MessageFromIT messageFromIT, IntygContentHolder certificate) {
    if (isSendMessageToCare(messageFromIT.getMessageContent())) {
      ...
      LOG.error(
          "Couldn't process message. Unknown message format for messageId: "
              + messageFromIT.getMessageId());
    }
  }

  private void processOutgoingMessage(MessageFromIT messageFromIT, IntygContentHolder certificate) {
    final var messageToRecipient =
        convertXmlToSendMessageToRecipient(
            messageFromIT.getMessageContent(), messageFromIT.getMessageId());
    ...
    monitoringLogService.logMessageImported(
        savedMessage.getIntygsId(),
        savedMessage.getMeddelandeId(),
        certificate
            .getUtlatande()
            .getGrundData()
            .getSkapadAv()
            ...
            .getVardgivare()
            .getVardgivarid(),
        savedMessage.getEnhetId(),
        isMessageAnAnswer(savedMessage)
  ...
  private void processIncomingMessage(MessageFromIT messageFromIT, IntygContentHolder certificate) {
    final var messageToCare =
        convertXmlToSendMessageToCare(
            messageFromIT.getMessageContent(), messageFromIT.getMessageId());
    ...
    if (savedMessage.getStatus() == Status.PENDING_INTERNAL_ACTION
        && savedMessage.getAmne() == ArendeAmne.KOMPLT) {
      if (isComplemented(certificate)) {
        savedMessage.setSenasteHandelse(
            certificate
                .getRelations()
    ...
    monitoringLogService.logMessageImported(
        ...
        certificate
            ...
            .getVardgivarid(),
        savedMessage.getEnhetId(),
        isMessageAnAnswer(savedMessage) ? "SendMessageToCare - Answer" : "SendMessageToCare");
  }
  ...
  private boolean isMessageReminder(Arende message) {
    return message.getPaminnelseMeddelandeId() != null
        && !message.getPaminnelseMeddelandeId().isEmpty();
  }
  ...
  private Arende convertMessageToRecipient(
      SendMessageToRecipientType messageToRecipientType, IntygContentHolder certificate) {
    final var message = new Arende();

    message.setAmne(ArendeAmne.valueOf(messageToRecipientType.getAmne().getCode()));
    message.setEnhetId(
        messageToRecipientType.getSkickatAv().getEnhet().getEnhetsId().getExtension());
    ...
    message.setVardgivareName(
        messageToRecipientType.getSkickatAv().getEnhet().getVardgivare().getVardgivarnamn());
  ...
  private Arende convertMessageToCare(
      SendMessageToCareType messageToCare, IntygContentHolder certificate) {
    final var message = ArendeConverter.convert(messageToCare);

    ArendeConverter.decorateMessageFromCertificate(
        message, certificate.getUtlatande(), messageToCare.getSkickatTidpunkt());
  ...
  private SendMessageToCareType convertXmlToSendMessageToCare(String messageXml, String messageId) {
    try {
      ...
      return (SendMessageToCareType)
          ((JAXBElement) unmarshaller.unmarshal(new StringReader(messageXml))).getValue();
    } catch (Exception ex) {
  ...
  private SendMessageToRecipientType convertXmlToSendMessageToRecipient(
      String messageXml, String messageId) {
    try {
