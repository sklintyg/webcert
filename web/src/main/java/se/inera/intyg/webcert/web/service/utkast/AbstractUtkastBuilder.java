...
public abstract class AbstractUtkastBuilder<T extends AbstractCreateCopyRequest>
    implements CopyUtkastBuilder<T> {
   ...
   */
  @Override
  public UtkastBuilderResponse populateCopyUtkastFromSignedIntyg(
    ...
    IntygContentHolder signedIntygHolder =
        intygService.fetchIntygData(orignalIntygsId, originalIntygsTyp);

    ModuleApi orgModuleApi =
        moduleRegistry.getModuleApi(
            originalIntygsTyp, signedIntygHolder.getUtlatande().getTextVersion());
    ...
    se.inera.intyg.common.support.model.common.internal.Vardenhet vardenhet =
        grundData.getSkapadAv().getVardenhet();
    ...
    String draftCopyJson =
        getInternalModel(
            orgUtlatande, moduleApi, copyRequest, patientDetails, relation, newDraftCopyId);
    ...
    if (patientDetails != null) {
      ...
      se.inera.intyg.common.support.model.common.internal.Patient patient =
          signedIntygHolder.getUtlatande().getGrundData().getPatient();
  ...
  /**
   * Makes sure that the new draft gets a suitable version accoring to business rules for versioning
   * of intygtypes. see GE-012 Versionshantering av intygstyper see WC-F016
   */
  private void ensureCorrectNewVersion(T copyRequest, String originalVersion) {
    if (StringUtils.isEmpty(copyRequest.getTypVersion())) {
      String version =
          intygTextsService.getLatestVersionForSameMajorVersion(
              copyRequest.getTyp(), originalVersion);
      if (version == null) {
  ...
  public UtkastBuilderResponse populateCopyUtkastFromOrignalUtkast(
      T copyRequest, Person patientDetails, boolean addRelation)
      throws ModuleNotFoundException, ModuleException {
    ...
    ModuleApi orgModuleApi =
        moduleRegistry.getModuleApi(
            copyRequest.getOriginalIntygTyp(), orgUtkast.getIntygTypeVersion());
    Utlatande orgUtlatande;
    try {
    ...
    ModuleApi moduleApi =
        moduleRegistry.getModuleApi(copyRequest.getTyp(), copyRequest.getTypVersion());
  ...
  protected Utkast buildUtkastCopy(
      T copyRequest,
      String utkastId,
      String utkastTyp,
    ...
    // If source and target certificate types are different, the current copy action is for
    // prefilling of a draft,
    // and version should be set to 1 (instead of 0) to suppress the option to prefill the draft
    // once more.
  ...
  protected CreateDraftCopyHolder createModuleRequestForCopying(
      AbstractCreateCopyRequest copyRequest,
      Person person,
      Relation relation,
      String newDraftCopyId) {
    ...
    CreateDraftCopyHolder newDraftCopyHolder =
        new CreateDraftCopyHolder(newDraftCopyId, copyRequest.getHosPerson(), relation);
    ...
    if (person != null) {
      ...
      patient.setFullstandigtNamn(
          IntygConverterUtil.concatPatientName(
              patient.getFornamn(), patient.getMellannamn(), patient.getEfternamn()));
  ...
  private void populateUtkastWithVardenhetAndHoSPerson(
      Utkast utkast, AbstractCreateCopyRequest copyRequest) {
    ...
    VardpersonReferens creator =
        UpdateUserUtil.createVardpersonFromHosPerson(copyRequest.getHosPerson());
  ...
  private void populatePatientDetailsFromPatient(Utkast utkast, Patient patient) {
    ...
    if (patient.getFullstandigtNamn() != null
        && (utkast.getPatientFornamn() == null || utkast.getPatientEfternamn() == null)) {
  ...
  private void replacePatientPersonnummerWithNew(
      Utkast utkast, AbstractCreateCopyRequest copyRequest) {
    if (copyRequest.containsNyttPatientPersonnummer()) {
  ...
  private UtkastStatus validateDraft(ModuleApi moduleApi, String draftCopyJson)
      throws ModuleException {
    ValidateDraftResponse validationResponse =
        moduleApi.validateDraft(draftCopyJson, defaultTypeAheadProvider);
