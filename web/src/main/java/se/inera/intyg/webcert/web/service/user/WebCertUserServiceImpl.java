...
public class WebCertUserServiceImpl implements WebCertUserService, LoggedInWebcertUserService {
  ...
  public WebCertUser getUser() {
    if (SecurityContextHolder.getContext().getAuthentication().getPrincipal()
        instanceof WebCertUser webCertUser) {
      return webCertUser;
    }
    return null;
  }
  ...
  public void deleteUserPreferences() {
    WebCertUser user = getUser();
    String hsaId = user.getHsaId();
    Map<String, String> anvandarPreferences =
        anvandarPreferenceRepository.getAnvandarPreference(hsaId);
    int deleteCount = 0;
    for (Map.Entry<String, String> preference : anvandarPreferences.entrySet()) {
      AnvandarPreference toDelete =
          anvandarPreferenceRepository.findByHsaIdAndKey(hsaId, preference.getKey());
      if (toDelete != null) {
  ...
  @Override
  public boolean isAuthorizedForUnit(
  ...
  public boolean isAuthorizedForUnits(List<String> enhetsHsaIds) {
    WebCertUser user = getUser();
    return user != null
        && new HashSet<>(user.getIdsOfSelectedVardenhet()).containsAll(enhetsHsaIds);
  }
  ...
  public void scheduleSessionRemoval(HttpSession session) {
    ...
    log.debug(
        "Scheduled removal of session {} at {} milliseconds", session.getId(), timeToInvalidate);
  }
  ...
  private void updateUserRole(Role role) {
    ...
    getUser()
        .setAuthorities(AuthoritiesResolverUtil.toMap(role.getPrivileges(), Privilege::getName));
  }

  boolean checkIfAuthorizedForUnit(
      WebCertUser user, String vardgivarHsaId, String enhetsHsaId, boolean isReadOnlyOperation) {
    if (user == null) {
      return false;
    }
