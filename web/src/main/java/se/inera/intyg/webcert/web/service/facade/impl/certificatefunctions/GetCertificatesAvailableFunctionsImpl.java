...
public class GetCertificatesAvailableFunctionsImpl implements GetCertificatesAvailableFunctions {
  ...
  private static final String READY_FOR_SIGN_DESCRIPTION =
      "Utkastet markeras som klar för signering.";
  ...
  private static final String REVOKE_LOCKED_DRAFT_DESCRIPTION =
  ...
  public List<ResourceLinkDTO> get(Certificate certificate) {
    ...
    switch (certificate.getMetadata().getStatus()) {
      case UNSIGNED:
        availableFunctions.addAll(getAvailableFunctionsForDraft(certificate));
        break;
      case SIGNED:
        availableFunctions.addAll(getAvailableFunctionsForCertificate(certificate));
        break;
      case LOCKED:
        availableFunctions.addAll(getAvailableFunctionsForLockedDraft(certificate));
        break;
      case REVOKED:
        availableFunctions.addAll(getAvailableFunctionsForRevokedCertificate(certificate));
        break;
      default:
    }
  ...
  /**
   * Determine all resource links for certificate that is in the draft stage. A rough filtering
   * determines what should be added as available links. This is due to the fact that links may not
   * be filtered future on.
   *
  ...
  private ArrayList<ResourceLinkDTO> getAvailableFunctionsForDraft(Certificate certificate) {
    ...
    if (certificate.getMetadata().getType().equals(LUAENA_TYPE)
        && isDjupintegration()
        && patientOlderThanThirtyYearsAndTwoMonths(
    ...
    resourceLinks.add(
        ResourceLinkDTO.create(
            ResourceLinkTypeDTO.EDIT_CERTIFICATE, EDIT_NAME, EDIT_DESCRIPTION, true));

    resourceLinks.add(getPrintResourceLink(certificate));

    resourceLinks.add(
        ResourceLinkDTO.create(
            ResourceLinkTypeDTO.REMOVE_CERTIFICATE, REMOVE_NAME, REMOVE_DESCRIPTION, true));

    if (isMessagingAvailable(certificate) && isComplementingCertificate(certificate)) {
      resourceLinks.add(
          ResourceLinkDTO.create(
              ResourceLinkTypeDTO.QUESTIONS, QUESTIONS_NAME, QUESTIONS_DESCRIPTION, true));
    }

    if (!webCertUserService.getUser().isLakare()
        && !webCertUserService.getUser().isPrivatLakare()) {
      resourceLinks.add(CertificateForwardFunction.createResourceLinkForDraft());
    ...
    if (isLisjp(certificate)) {
      resourceLinks.add(ResourceLinkDTO.create(FMB, FMB_NAME, FMB_DESCRIPTION, true));
    }

    certificateSignConfirmationFunction
        .get(certificate, webCertUserService.getUser())
        .ifPresent(resourceLinks::add);

    displayPatientAddressInCertificate.get(certificate).ifPresent(resourceLinks::add);

    createCertificateFromCandidateFunction.get(certificate).ifPresent(resourceLinks::add);

    srsFunction
        .getSRSFullView(certificate, webCertUserService.getUser())
        .ifPresent(resourceLinks::add);

    certificateSignAndSendFunction.get(certificate).ifPresent(resourceLinks::add);
  ...
  private ArrayList<ResourceLinkDTO> getAvailableFunctionsForCertificate(Certificate certificate) {
    ...
    if (isReplaceCertificateAvailable(certificate)) {
      if (hasUnhandledComplement(certificate)) {
        ...
        resourceLinks.add(
            ResourceLinkDTO.create(
                ResourceLinkTypeDTO.REPLACE_CERTIFICATE, REPLACE_NAME, REPLACE_DESCRIPTION, true));
      }
    }
    ...
    if (CertificateRenewFunction.validate(
        certificate.getMetadata().getType(),
        certificate.getMetadata().getRelations(),
      ...
      resourceLinks.add(
          CertificateRenewFunction.createResourceLink(
              loggedInCareUnitId, savedCareUnitId, certificate.getMetadata().getType()));
    }
    ...
    if (isMessagingAvailable(certificate)
        && isSent(certificate)
        && !hasUnsignedComplement(certificate)) {
      resourceLinks.add(
          ResourceLinkDTO.create(
    ...
    createCertificateFromTemplateFunction
        .get(certificate, webCertUserService.getUser())
        .ifPresent(resourceLinks::add);

    showRelatedCertificateFunction
        .get(certificate, webCertUserService.getUser())
        .ifPresent(resourceLinks::add);

    sendCertificateFunction.get(certificate).ifPresent(resourceLinks::add);

    srsFunction
        .getSRSMinimizedView(certificate, webCertUserService.getUser())
        .ifPresent(resourceLinks::add);
  ...
  private ResourceLinkDTO getQuestionsResourceLink(Certificate certificate) {
    if (isSent(certificate)) {
      ...
      return ResourceLinkDTO.create(
          ResourceLinkTypeDTO.QUESTIONS_NOT_AVAILABLE, QUESTIONS_NAME, QUESTIONS_DESCRIPTION, true);
    }
  }
  ...
  private ArrayList<ResourceLinkDTO> getAvailableFunctionsForLockedDraft(Certificate certificate) {
    ...
    copyCertificateFunction.get(certificate).ifPresent(resourceLinks::add);
  ...
  private ArrayList<ResourceLinkDTO> getAvailableFunctionsForRevokedCertificate(
  ...
  private boolean isComplementingCertificate(Certificate certificate) {
    return certificate.getMetadata().getRelations() != null
  ...
  private boolean includesChildRelation(
      CertificateRelations relations, CertificateRelationType type, CertificateStatus status) {
  ...
  private boolean isMessagingAvailable(Certificate certificate) {
    return authoritiesHelper.isFeatureActive(
        AuthoritiesConstants.FEATURE_HANTERA_FRAGOR, certificate.getMetadata().getType());
  }
  ...
  private ResourceLinkDTO getPrintResourceLink(Certificate certificate) {
    if (!certificate.getMetadata().getPatient().isProtectedPerson()) {
      ...
      return ResourceLinkDTO.create(
          ResourceLinkTypeDTO.PRINT_CERTIFICATE,
          PRINT_NAME,
          certificate.getMetadata().getStatus() == UNSIGNED
              ? PRINT_DRAFT_DESCRIPTION
              : PRINT_CERTIFICATE_DESCRIPTION,
  ...
  private boolean hasBeenComplementedBySignedCertificate(Certificate certificate) {
    if (certificate.getMetadata().getRelations() != null) {
      return Arrays.stream(certificate.getMetadata().getRelations().getChildren())
          .anyMatch(
              relation ->
                  relation.getType().equals(COMPLEMENTED) && relation.getStatus() == SIGNED);
    }
    return false;
  }
  ...
  private boolean hasUnsignedComplement(Certificate certificate) {
    if (certificate.getMetadata().getRelations() != null) {
      return Arrays.stream(certificate.getMetadata().getRelations().getChildren())
          .anyMatch(
              relation ->
                  relation.getType().equals(COMPLEMENTED) && relation.getStatus() == UNSIGNED);
    }
    return false;
  }
  ...
  private boolean patientOlderThanThirtyYearsAndTwoMonths(String patientId) {
    final var birthDate =
        PatientToolkit.birthDate(Personnummer.createPersonnummer(patientId).orElseThrow());
