...
public class ResourceLinkHelperImpl implements ResourceLinkHelper {
  ...
  public void decorateIntygModuleWithValidActionLinks(
      List<IntygModuleDTO> intygModuleDTOList, Personnummer patient) {
    intygModuleDTOList.forEach(
        intygModuleDTO -> decorateIntygModuleWithValidActionLinks(intygModuleDTO, patient));
  }

  @Override
  public void decorateIntygModuleWithValidActionLinks(
      IntygModuleDTO intygModuleDTO, Personnummer patient) {
  ...
  @Override
  public void decorateUtkastWithValidActionLinks(
      DraftHolder draftHolder,
      String certificateType,
      String certificateTypeVersion,
      Vardenhet careUnit,
      Personnummer patient) {
    final var accessEvaluationParameters =
        AccessEvaluationParameters.create(
            certificateType, certificateTypeVersion, careUnit, patient, draftHolder.isTestIntyg());
  ...
  public void decorateIntygWithValidActionLinks(IntygContentHolder intygContentHolder) {
    ...
    final var careUnit =
        intygContentHolder.getUtlatande().getGrundData().getSkapadAv().getVardenhet();
    final var personId =
        intygContentHolder.getUtlatande().getGrundData().getPatient().getPersonId();
    final var accessEvaluationParameters =
        AccessEvaluationParameters.create(
            certificateType,
    ...
    if (certificateAccessServiceHelper.isAllowToCreateDraftFromSignedTemplate(
  ...
  @Override
  public void decorateIntygWithValidActionLinks(
      List<ListIntygEntry> listIntygEntryList, Personnummer patient) {
  ...
  @Override
  public void decorateIntygWithValidActionLinks(
      ListIntygEntry listIntygEntry, Personnummer patient) {
  ...
  @Override
  public void decorateArendeWithValidActionLinks(
      List<ArendeListItem> arendeListItems, Vardenhet careUnit) {
  ...
  private boolean isLocked(DraftHolder draftHolder) {
    return draftHolder.getStatus() != null
        && draftHolder.getStatus().equals(UtkastStatus.DRAFT_LOCKED);
  }

  private void decorateDraft(
      DraftHolder draftHolder, AccessEvaluationParameters accessEvaluationParameters) {
  ...
  private void decorateLockedDraft(
      DraftHolder draftHolder, AccessEvaluationParameters accessEvaluationParameters) {
  ...
  private List<ActionLink> getActionLinksForQuestions(
    ...
    if (certificateAccessServiceHelper.isAllowToSetComplementAsHandled(
        accessEvaluationParameters)) {
