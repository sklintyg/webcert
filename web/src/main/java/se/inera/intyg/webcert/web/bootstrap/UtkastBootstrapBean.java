...
public class UtkastBootstrapBean {
  ...
  private final List<ArendeAmne> arendeAmnen =
      Arrays.asList(ArendeAmne.AVSTMN, ArendeAmne.KONTKT, ArendeAmne.OVRIGT);
  private final List<Amne> fsAmnen =
    ...
    for (Resource resource : getResourceListing("classpath*:module-bootstrap-certificate/*.xml")) {
      ...
      try {
        ...
        if (utkastRepo.findById(utlatande.getId()).isPresent()) {
          LOG.info(
              "Bootstrapping certificate '{}' from module {} (version {}) skipped. Already in database.",
              filename,
              moduleName,
          ...
          LOG.info(
              "Bootstrapping certificate '{}' from module {} (version {})",
              filename,
              moduleName,
              intygMajorTypeVersion);
          final var status =
              filename.contains("locked") ? UtkastStatus.DRAFT_LOCKED : UtkastStatus.SIGNED;
  ...
  private void addArendeIfFKCertificate(Utlatande utlatande) {
    if (utlatande.getTyp().equals(Fk7263EntryPoint.MODULE_ID)) {
      ...
      fragaRepo.save(
          createFragaSvar(utlatande, FrageStallare.FORSAKRINGSKASSAN, false, false, "Test person"));
    }

    if (utlatande.getTyp().equals(LuaefsEntryPoint.MODULE_ID)
        || utlatande.getTyp().equals(LuaenaEntryPoint.MODULE_ID)
        || utlatande.getTyp().equals(LisjpEntryPoint.MODULE_ID)
        || utlatande.getTyp().equals(LuseEntryPoint.MODULE_ID)) {
  ...
  private void addSignatureIfSignedCertificate(
      UtkastStatus status, Utkast draft, Utlatande utlatande) {
  ...
  // INTYG-4086: An incredibly ugly hack to mitigate the fact that we're populating test-data using
  // the XML format
  ...
  private void setupArende(
      Utlatande utlatande, boolean komplettering, boolean paminnelse, FrageStallare fragestallare) {
    ArendeAmne amne;
    if (komplettering) {
  ...
  private String createArende(
      Utlatande utlatande,
      boolean komplettering,
      String paminnelseMeddelandeId,
      FrageStallare fragestallare,
      ArendeAmne amne) {
    ...
    arende.setVardgivareName(
        utlatande.getGrundData().getSkapadAv().getVardenhet().getVardgivare().getVardgivarnamn());
  ...
  private FragaSvar createFragaSvar(
      Utlatande utlatande, FrageStallare fragestallare, boolean komplettering, boolean paminnelse) {
  ...
  private FragaSvar createFragaSvar(
    ...
    fs.setIntygsReferens(
        new IntygsReferens(
            ...
            null,
            utlatande.getGrundData().getSigneringsdatum()));
    if (komplettering) {
  ...
  private Utkast createUtkast(Utlatande json, UtkastStatus status) throws JsonProcessingException {
    ...
    utkast.setVardgivarId(
        json.getGrundData().getSkapadAv().getVardenhet().getVardgivare().getVardgivarid());
    utkast.setVardgivarNamn(
        json.getGrundData().getSkapadAv().getVardenhet().getVardgivare().getVardgivarnamn());
