...
public class CreateDraftCertificateFromCS implements CreateDraftCertificate {
  ...
  public CreateDraftCertificateResponseType create(Intyg certificate, IntygUser user) {
    final var modelId =
        csIntegrationService.certificateExternalTypeExists(
            certificate.getTypAvIntyg().getCodeSystem(), certificate.getTypAvIntyg().getCode());

    if (modelId.isEmpty()) {
      log.debug(
          "Certificate type '{}' does not exist in certificate service",
          certificate.getTypAvIntyg().getCode());
      return null;
    }
    ...
    try {
      final var draftCertificateRequest =
          csIntegrationRequestFactory.createDraftCertificateRequest(
              modelId.get(), certificate, user);
      final var createdCertificate =
          csIntegrationService.createCertificate(draftCertificateRequest);
      ...
      publishCertificateAnalyticsMessage.publishEvent(
          draftCertificateRequest.getPrefillXml() == null
              ? certificateAnalyticsMessageFactory.draftCreated(
                  createdCertificate, loggedInWebcertUser)
  ...
  private Optional<CreateDraftCertificateResponseType> validatePUIntegration(Intyg certificate) {
    ...
    final var personId =
        Personnummer.createPersonnummer(personIdExtension)
            .orElseThrow(
                () ->
                    new WebCertServiceException(
                        WebCertServiceErrorCodeEnum.PU_PROBLEM,
                        String.format(
                            "Cannot create Personnummer object with invalid personId '%s'",
                            personIdExtension)));
