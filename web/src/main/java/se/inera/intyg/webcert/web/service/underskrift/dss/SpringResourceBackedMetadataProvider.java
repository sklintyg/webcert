...
public class SpringResourceBackedMetadataProvider extends AbstractReloadingMetadataResolver {
   ...
   * @throws ComponentInitializationException thrown if there is a problem retrieving information
   *     about the resource
   */
  ...
  protected byte[] fetchMetadata() throws ResolverException {
    try {
      final var metadataUpdateTime =
          ZonedDateTime.ofInstant(
              Instant.ofEpochSecond(metadataResource.lastModified()), ZoneId.systemDefault());
      log.debug("resource {} was last modified {}", getMetadataIdentifier(), metadataUpdateTime);

      if (getLastRefresh() == null
          || metadataUpdateTime.isAfter(
              ZonedDateTime.ofInstant(getLastRefresh(), ZoneId.systemDefault()))) {
