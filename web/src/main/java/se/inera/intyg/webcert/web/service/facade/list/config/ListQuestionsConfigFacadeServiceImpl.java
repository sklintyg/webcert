...
public class ListQuestionsConfigFacadeServiceImpl implements ListVariableConfigFacadeService {
  ...
  private static final String RESET_FILTER_TOOLTIP =
      "Återställ sökfilter för ej hanterade ärenden.";
  ...
  private Optional<ListFilterConfig> getFilterFromId(
      List<ListFilterConfig> currentFilters, String id) {
    return currentFilters.stream().filter((filter) -> filter.getId().equals(id)).findFirst();
  }
  ...
  private ListFilterConfig getSignedByFilter(String unit) {
    ...
    return ListFilterConfigFactory.createStaffSelect(
        "SIGNED_BY", "Signerat av", savedByList, defaultValue);
  }
  ...
  private ListFilterConfig getUnitStaticFilter() {
    return new ListFilterSelectConfig(
        "UNIT", webCertUserService.getUser().getValdVardenhet().getId(), Collections.emptyList());
  }
  ...
  private List<ListFilterConfigValue> getUnitList() {
    ...
    final var subUnits =
        hsaOrganizationsService.getVardenhet(loggedInUnit.getId()).getMottagningar();

    final var units = getUnits(loggedInUnit, subUnits);
    final var unitStatistics =
        calculateQuestionsForUnitService.calculate(user, units).getUnitStatistics();

    final var unitIdAndNamesMap =
        units.stream()
            .collect(Collectors.toMap(SelectableVardenhet::getId, SelectableVardenhet::getNamn));

    final var list =
        unitStatistics.entrySet().stream()
            .sorted(sortUnitFirstAndSubUnitsAlphabetical(loggedInUnit.getId(), unitIdAndNamesMap))
            .map(
                unit ->
                    getUnitSelectOption(
                        unit.getKey(), unit.getValue(), loggedInUnit.getId(), unitIdAndNamesMap))
            .collect(Collectors.toList());

    list.addFirst(
        ListFilterConfigValue.create(
            "", getShowAllText(loggedInUnit.getId(), unitStatistics), true));

    return list;
  }

  private static ArrayList<SelectableVardenhet> getUnits(
      SelectableVardenhet loggedInUnit, List<Mottagning> subUnits) {
  ...
  private String getShowAllText(String unitId, Map<String, UnitStatisticsDTO> statistics) {
    ...
    final var totalQuestions =
        unitStatistic.getQuestionsOnUnit() + unitStatistic.getQuestionsOnSubUnits();
    return "Visa alla (" + totalQuestions + ")";
  }

  private Comparator<Map.Entry<String, UnitStatisticsDTO>> sortUnitFirstAndSubUnitsAlphabetical(
      String unitId, Map<String, String> unitIdAndNamesMap) {
    return Comparator.comparing(
            (Map.Entry<String, UnitStatisticsDTO> entry) ->
                isNotMatchedUnit(entry.getKey(), unitId))
  ...
  private ListFilterConfigValue getUnitSelectOption(
      String unitId,
      UnitStatisticsDTO unit,
      String loggedInUnitId,
      Map<String, String> unitIdAndNamesMap) {
    return ListFilterConfigValue.create(
        unitId,
        getUnitText(unitId, unit.getQuestionsOnUnit(), loggedInUnitId, unitIdAndNamesMap),
        false);
  }
