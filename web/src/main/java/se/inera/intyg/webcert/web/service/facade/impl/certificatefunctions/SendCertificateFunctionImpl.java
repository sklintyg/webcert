...
public class SendCertificateFunctionImpl implements SendCertificateFunction {
  ...
  private static final String SEND_BODY_FK =
  ...
  private static final String SEND_BODY_LISJP =
      "<p>Om du går vidare kommer intyget skickas direkt till "
          + "Försäkringskassans system vilket ska göras i samråd med patienten.</p>"
  ...
  public Optional<ResourceLinkDTO> get(Certificate certificate) {
    if (!allowedCertificateTypes.contains(certificate.getMetadata().getType())
        ...
        || isWrongMajorVersion(certificate)) {
  ...
  private static boolean isWrongMajorVersion(Certificate certificate) {
    return (certificate.getMetadata().getType().equals(TsBasEntryPoint.MODULE_ID)
            || certificate.getMetadata().getType().equals(TsDiabetesEntryPoint.MODULE_ID))
  ...
  private Optional<ResourceLinkDTO> getResourceLinkDTOFkLisjp(Certificate certificate) {
    return Optional.of(
        ResourceLinkDTO.create(
            ...
            hasShortSickleavePeriod(certificate)
                ? SEND_BODY_SHORT_SICKLEAVE_PERIOD
                : SEND_BODY_LISJP,
            true));
  }
  ...
  private boolean includesChildRelation(CertificateRelations relations) {
    ...
    return Arrays.stream(relations.getChildren())
        .anyMatch(
            relation ->
                relation.getType().equals(CertificateRelationType.REPLACED)
                    && relation.getStatus().equals(CertificateStatus.SIGNED));
  }
  ...
  private boolean hasShortSickleavePeriod(Certificate certificate) {
    final var optionalSickLeavePeriod =
        certificate.getData().values().stream()
            .filter(
                dataElement ->
                    dataElement.getConfig().getType()
                        == CertificateDataConfigType.UE_CHECKBOX_DATE_RANGE_LIST)
            .findFirst();
    if (optionalSickLeavePeriod.isPresent()) {
      final var sickLeavePeriod =
          (CertificateDataValueDateRangeList) optionalSickLeavePeriod.get().getValue();
  ...
  private static ArrayList<CertificateDataValueDateRange> mergeConnectedDates(
      List<CertificateDataValueDateRange> dateRanges) {
  ...
  private static boolean hasOverlappingDates(
      CertificateDataValueDateRange date, CertificateDataValueDateRange dateRange) {
