...
public class CertificateTypeInfoModalService {
  ...
  public Optional<CertificateTypeInfoModal> get(String certificateType, Personnummer patientId) {
    ...
    final var previousCertificates =
        utkastService.checkIfPersonHasExistingIntyg(patientId, user, null, false);
    ...
    final var previousCertificate =
        getPreviousIntyg(certificateType, previousCertificateMap, previousDraftMap);

    if (previousCertificate.isEmpty() || previousCertificate.get().getLatestIntygsId() == null) {
      return Optional.empty();
    }

    final var modalProvider =
        CertificateTypeInfoModalProviderResolver.getModalProvider(certificateType);

    if (modalProvider == null) {
      return Optional.empty();
    }

    return modalProvider.create(info(previousCertificate.get(), certificateType));
  }

  private static Optional<PreviousIntyg> getPreviousIntyg(
  ...
  private PreviousCertificateInfo info(PreviousIntyg previousCertificate, String certificateType) {
    final var utkast =
        utkastService.getDraft(previousCertificate.getLatestIntygsId(), certificateType, false);
