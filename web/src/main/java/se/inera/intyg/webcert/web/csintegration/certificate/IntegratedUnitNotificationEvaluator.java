...
public class IntegratedUnitNotificationEvaluator {
  ...
  public boolean mailNotification(
    ...
    if (evaluateMailNotification(
        careProviderId, issuedOnUnitId, regionNotificationConfigs, issuingDate)) {
      log.info(
          "Certificate with id '{}' has been evaluated and should receive mail notification. CareProviderId: '{}',"
              + " IssuedOnUnitId: '{}', IssuingDate: '{}'",
          certificateId,
          careProviderId,
          issuedOnUnitId,
          issuingDate);
  ...
  private boolean evaluateMailNotification(
      String careProviderId,
      String issuedOnUnitId,
      List<RegionNotificationConfig> regionNotificationConfigs,
      LocalDateTime issuingDate) {
  ...
  private boolean evaluateMailNotification(
      ...
      LocalDateTime activateFrom,
      String unitId,
      String careProviderId,
      LocalDateTime issuingDate) {
    if ((careProviderIds != null && !careProviderIds.contains(careProviderId))
        && (unitIds != null && !unitIds.contains(unitId))) {
      return false;
    }
