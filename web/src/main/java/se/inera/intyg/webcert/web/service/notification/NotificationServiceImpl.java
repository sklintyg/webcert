...
public class NotificationServiceImpl implements NotificationService {
  ...
  public void sendNotificationForQuestionReceived(FragaSvar fragaSvar) {
    if (unitIsIntegrated(fragaSvar) && shouldNotRecieveNotificationByMail(fragaSvar)) {
      sendNotificationForQAs(
          ...
          NotificationEvent.NEW_QUESTION_FROM_RECIPIENT,
          fragaSvar.getSistaDatumForSvar(),
          ArendeAmne.fromAmne(fragaSvar.getAmne()).orElse(null));
    } else {
      sendNotificationForIncomingQuestionByMail(
          new MailNotification(
              fragaSvar.getInternReferens().toString(),
              fragaSvar.getIntygsReferens().getIntygsId(),
              Fk7263EntryPoint.MODULE_ID,
              fragaSvar.getVardperson().getEnhetsId(),
              fragaSvar.getVardperson().getEnhetsnamn(),
              fragaSvar.getVardperson().getHsaId()));
    }
  }

  private boolean unitIsIntegrated(FragaSvar fragaSvar) {
    return integreradeEnheterRegistry.isEnhetIntegrerad(
        fragaSvar.getVardperson().getEnhetsId(), Fk7263EntryPoint.MODULE_ID);
  }
  ...
  public void sendNotificationForAnswerRecieved(FragaSvar fragaSvar) {
    if (unitIsIntegrated(fragaSvar)) {
      sendNotificationForQAs(
          fragaSvar.getIntygsReferens().getIntygsId(), NotificationEvent.NEW_ANSWER_FROM_RECIPIENT);
    } else {
  ...
  @Override
  public void sendNotificationForQuestionReceived(
      Arende arende, String careProviderId, LocalDateTime issuingDate) {
    if (unitIsIntegrated(arende)
        && shouldNotRecieveNotificationByMail(arende, careProviderId, issuingDate)) {
      sendNotificationForQAs(
          arende.getIntygsId(),
          NotificationEvent.NEW_QUESTION_FROM_RECIPIENT,
          arende.getSistaDatumForSvar(),
          arende.getAmne());
    } else {
      sendNotificationForIncomingQuestionByMail(
          new MailNotification(
  ...
  @Override
  public void sendNotificationForAnswerRecieved(
      Arende arende, String careProviderId, LocalDateTime issuingDate) {
    if (unitIsIntegrated(arende)
        && shouldNotRecieveNotificationByMail(arende, careProviderId, issuingDate)) {
  ...
  private boolean shouldNotRecieveNotificationByMail(
      Arende arende, String careProviderId, LocalDateTime issuingDate) {
    return !integratedUnitNotificationEvaluator.mailNotification(
  ...
  @Override
  public void forwardInternalNotification(
      final String intygsId,
    ...
    try {
      ...
      NotificationMessage notificationMessage =
          notificationMessageFactory.createNotificationMessage(
              intygsId,
              intygstyp,
              careUnitId,
              json,
  ...
  private List<Handelse> findNotificationsBetweenTimestamp(IntygWithNotificationsRequest request) {
    if (request.shouldUseEnhetId()) {
      return handelseRepo.findByPersonnummerAndEnhetsIdInAndTimestampBetween(
          request.getPersonnummer().getPersonnummer(),
          request.getEnhetId(),
          request.getStartDate(),
          request.getEndDate());
    } else {
      return handelseRepo.findByPersonnummerAndVardgivarIdAndTimestampBetween(
          request.getPersonnummer().getPersonnummer(),
          request.getVardgivarId(),
          request.getStartDate(),
          request.getEndDate());
    }
  }
  ...
  private List<Handelse> findNotificationsAfterTimestamp(IntygWithNotificationsRequest request) {
    if (request.shouldUseEnhetId()) {
      return handelseRepo.findByPersonnummerAndEnhetsIdInAndTimestampAfter(
          request.getPersonnummer().getPersonnummer(),
          request.getEnhetId(),
          request.getStartDate());
    } else {
      return handelseRepo.findByPersonnummerAndVardgivarIdAndTimestampAfter(
          request.getPersonnummer().getPersonnummer(),
          request.getVardgivarId(),
          request.getStartDate());
    }
  ...
  private List<Handelse> findNotificationsBeforeTimestamp(IntygWithNotificationsRequest request) {
    if (request.shouldUseEnhetId()) {
      return handelseRepo.findByPersonnummerAndEnhetsIdInAndTimestampBefore(
          request.getPersonnummer().getPersonnummer(), request.getEnhetId(), request.getEndDate());
    } else {
      return handelseRepo.findByPersonnummerAndVardgivarIdAndTimestampBefore(
          request.getPersonnummer().getPersonnummer(),
          request.getVardgivarId(),
          request.getEndDate());
    }
  ...
  private List<Handelse> findNotificationsWithoutTimestamp(IntygWithNotificationsRequest request) {
    if (request.shouldUseEnhetId()) {
      return handelseRepo.findByPersonnummerAndEnhetsIdIn(
          request.getPersonnummer().getPersonnummer(), request.getEnhetId());
    } else {
      return handelseRepo.findByPersonnummerAndVardgivarId(
          request.getPersonnummer().getPersonnummer(), request.getVardgivarId());
    }
  }

  private void sendNotificationForQAs(
      String intygsId, NotificationEvent event, LocalDate date, ArendeAmne amne) {
    Utkast utkast = getUtkast(intygsId);
    if (utkast != null) {
  ...
  private void createAndSendNotification(
      Utkast utkast, HandelsekodEnum handelse, ArendeAmne amne, LocalDate sistaDatumForSvar) {

    Optional<SchemaVersion> version = sendNotificationStrategy.decideNotificationForIntyg(utkast);
    if (version.isEmpty()) {
      LOGGER.debug(
          "Schema version is not present. Notification message not sent for event {}", handelse);
      return;
    }
  ...
  private void createAndSendNotification(
    ...
    NotificationMessage notificationMessage =
        notificationMessageFactory.createNotificationMessage(
            utkast, handelse, version, reference, amneskod, sistaDatumForSvar);
  ...
  private void createAndSendNotification(
      IntygContentHolder certificate,
      HandelsekodEnum handelse,
      ArendeAmne amne,
      LocalDate sistaDatumForSvar) {

    final var optionalSchemaVersion =
        sendNotificationStrategy.decideNotificationForIntyg(certificate.getUtlatande());
    ...
    createAndSendNotification(
        certificate, handelse, amne, sistaDatumForSvar, optionalSchemaVersion.get(), hanteratAv);
  }
  ...
  private void createAndSendNotification(
    ...
    final var reference =
        referensService.getReferensForIntygsId(certificate.getUtlatande().getId());
  ...
  private void createAndSendNotificationForQAs(
      Utkast utkast, NotificationEvent event, ArendeAmne amne, LocalDate sistaDatumForSvar) {
    ...
    if (handelseKod == null) {
      LOGGER.debug(
          "Notification message not sent for event {} in version {}",
          event.name(),
          version.get().name());
      return;
    }
  ...
  private void createAndSendNotificationForQAs(
      String certificateId, NotificationEvent event, ArendeAmne amne, LocalDate sistaDatumForSvar) {
    ...
    if (handelseKod == null) {
      LOGGER.debug(
          "Notification message not sent for event {} in version {}",
          event.name(),
          optionalSchemaVersion.get().name());
      return;
    }

    createAndSendNotification(
        certificate, handelseKod, amne, sistaDatumForSvar, optionalSchemaVersion.get(), null);
  }
  ...
  public void send(
    ...
    LOGGER.debug(
        "Notification message generated and sent to aggregation queue: {}", notificationMessage);
    monitoringLog.logStatusUpdateQueued(
        notificationMessage.getIntygsId(),
        correlationId,
        notificationMessage.getLogiskAdress(),
        notificationMessage.getIntygsTyp(),
        intygTypeVersion,
        notificationMessage.getHandelse().name(),
  ...
  private String currentUserId() {
    ...
    return auth == null || !(auth.getPrincipal() instanceof WebCertUser)
        ? null
        : ((WebCertUser) auth.getPrincipal()).getHsaId();
  }
  ...
  private void sendNotificationForIncomingQuestionByMail(MailNotification mailNotification) {
    try {
      ...
      LOGGER.error(
          String.format(
              "Notification mail for question '%s' concerning certificate '%s' couldn't be sent to '%s' (%s)",
              mailNotification.getQaId(),
              mailNotification.getCertificateId(),
  ...
  private void sendNotificationForIncomingAnswerByMail(MailNotification mailNotification) {
    try {
      ...
      LOGGER.error(
          String.format(
              "Notification mail for answer '%s' concerning certificate '%s' couldn't be sent to '%s' (%s)",
              mailNotification.getQaId(),
              mailNotification.getCertificateId(),
              mailNotification.getCareUnitId(),
              mailNotification.getCareUnitName()),
