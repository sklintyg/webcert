...
public final class SendMessageToRecipientTypeBuilder {
  ...
  public static SendMessageToRecipientType build(
      Arende arende, WebCertUser skickatAv, String logiskAdressMottagare) {
    ...
    request.setPatientPersonId(
        InternalConverterUtil.getPersonId(createPnr(arende.getPatientPersonId())));
  ...
  private static Enhet buildEnhet(WebCertUser user) {
    ...
    if (sourceVardenhet != null && sourceVardenhet instanceof AbstractVardenhet) {
      ...
      enhet.setArbetsplatskod(
          InternalConverterUtil.getArbetsplatsKod(valdVardenhet.getArbetsplatskod()));
