...
public class StatModuleApiController extends AbstractApiController {
  ...
  public ResponseEntity<StatsResponse> getStatistics() {
    ...
    Map<String, Long> fragaSvarStatsMap =
        fragaSvarService.getNbrOfUnhandledFragaSvarForCareUnits(allUnitIds, intygsTyper);
    Map<String, Long> arendeStatsMap =
        arendeService.getNbrOfUnhandledArendenForCareUnits(allUnitIds, intygsTyper);

    Map<String, Long> mergedMap =
        StatisticsHelper.mergeArendeAndFragaSvarMaps(fragaSvarStatsMap, arendeStatsMap);
    Map<String, Long> intygStatsMap =
        intygDraftService.getNbrOfUnsignedDraftsByCareUnits(allUnitIds);
    ...
    long fragaSvarOnOtherUnitThanTheSelected =
        calcSumFromSelectedUnits(unitIdsOfNotSelected, mergedMap);
    statsResponse.setTotalNbrOfUnhandledFragaSvarOnOtherThanSelected(
        fragaSvarOnOtherUnitThanTheSelected);
    ...
    long unsignedDraftsOnOtherThanSelected =
        calcSumFromSelectedUnits(unitIdsOfNotSelected, intygStatsMap);
    statsResponse.setTotalNbrOfUnsignedDraftsOnOtherThanSelected(unsignedDraftsOnOtherThanSelected);

    long unsignedDraftsOnSelected =
        getSafeStatValueFromMap(user.getValdVardenhet().getId(), intygStatsMap);
  ...
  private void populateStatsResponseWithVardgivarStats(
      StatsResponse statsResponse,
      List<Vardgivare> vardgivare,
      Map<String, Long> intygStats,
    ...
    for (Vardgivare vg : vardgivare) {
      ...
      vgStats
          .getVardenheter()
          .addAll(
              createAndPopulateVardenheterWithStats(
                  vg.getVardenheter(), intygStats, fragaSvarStats));
      statsResponse.getVardgivare().add(vgStats);
    }
  }

  private List<VardenhetStats> createAndPopulateVardenheterWithStats(
      List<Vardenhet> vardenheter, Map<String, Long> intygStats, Map<String, Long> fragaSvarStats) {
  ...
  private void addStatsForMottagningar(
      Vardenhet vardenhet,
      List<VardenhetStats> veStatsList,
      Map<String, Long> intygStats,
      Map<String, Long> fragaSvarStats) {
