...
public class IntygRelationHelperImpl implements IntygRelationHelper {
  ...
  public void decorateIntygListWithRelations(List<ListIntygEntry> fullIntygItemList) {
    ...
    ListRelationsForCertificateResponseType response =
        getRelationsFromIntygstjanst(
            fullIntygItemList.stream()
                .map(ListIntygEntry::getIntygId)
                .collect(Collectors.toList()));

    if (response != null) {
      // Very ugly, iterate over both lists, find matches and create relation(s) on the
      // ListIntygEntries.
    ...
    // Finally, we need to take any relations present in Webcert (but not yet present in
    // Intygstjänsten) into
    // account.
    for (ListIntygEntry lie : fullIntygItemList) {
  ...
  private WebcertCertificateRelation createWebcertCertificateRelation(
      Relation r, boolean childRelation) {
    String intygsId =
        childRelation ? r.getFranIntygsId().getExtension() : r.getTillIntygsId().getExtension();
    return new WebcertCertificateRelation(
        intygsId,
        RelationKod.fromValue(r.getTyp().getCode()),
        r.getSkapad(),
        UtkastStatus.SIGNED,
        r.isFranIntygMakulerat());
  }
  ...
  private ListRelationsForCertificateResponseType getRelationsFromIntygstjanst(
      List<String> intygIds) {
    ...
    return listRelationsForCertificateResponderInterface.listRelationsForCertificate(
        logicalAddress, request);
  }

  private boolean firstSkapadLaterDateThanSecond(
      WebcertCertificateRelation first, WebcertCertificateRelation second) {
    if (first == null) {
      return false;
    }
    if (second == null) {
  ...
  private void mergeRelations(Relations startRelations, Relations augmentWith) {
    ...
    // Since only a single parent relation is possible to have, only add if there is none already
    // present.
    ...
    if (firstSkapadLaterDateThanSecond(
        augmentWith.getLatestChildRelations().getReplacedByIntyg(),
        startRelations.getLatestChildRelations().getReplacedByIntyg())) {
      startRelations
          .getLatestChildRelations()
          .setReplacedByIntyg(augmentWith.getLatestChildRelations().getReplacedByIntyg());
    }
    if (firstSkapadLaterDateThanSecond(
        augmentWith.getLatestChildRelations().getReplacedByUtkast(),
        startRelations.getLatestChildRelations().getReplacedByUtkast())) {
      startRelations
          .getLatestChildRelations()
          .setReplacedByUtkast(augmentWith.getLatestChildRelations().getReplacedByUtkast());
    }
    if (firstSkapadLaterDateThanSecond(
        augmentWith.getLatestChildRelations().getComplementedByIntyg(),
    ...
    if (firstSkapadLaterDateThanSecond(
        augmentWith.getLatestChildRelations().getComplementedByUtkast(),
