...
public interface UtkastService {
   ...
   *
   * <p>Can be used when the utkast need to be retrieved for another service but won't be returned
   * to the user.
   *
   * @param intygId utkast id
   * @param intygType utkast type
   ...
   */
  SaveDraftResponse updateDraftFromCandidate(
  ...
  /**
   * Fairly specialized method to check if a person has existing Intyg of the same type. Returns a
   * Map of either "Utkast" or "Intyg" to Map of String, Boolean, where String is the intyg type and
   * the Boolean indicates that a previous Intyg or Utkast of the same type exists within the same
   * caregiver.
   *
   ...
   */
  Map<String, Map<String, PreviousIntyg>> checkIfPersonHasExistingIntyg(
      Personnummer personnummer, IntygUser user, String currentDraftId);
   ...
   *
   * @return true or false value if draft has been created through a replacement of another
   *     certificate.
   */
