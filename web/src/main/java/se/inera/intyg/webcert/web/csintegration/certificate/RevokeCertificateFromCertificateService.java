...
public class RevokeCertificateFromCertificateService implements RevokeCertificateFacadeService {
  ...
  public Certificate revokeCertificate(String certificateId, String reason, String message) {
    ...
    final var revokedCertificate =
        csIntegrationService.revokeCertificate(
            certificateId, csIntegrationRequestFactory.revokeCertificateRequest(reason, message));
    ...
    monitorLog(
        certificate,
        revokedCertificate.getMetadata().getRevokedBy().getPersonId(),
        reason,
        certificateStatusBeforeRevoke);
    ...
    handleMessageNotificationForParentService.notify(
        revokedCertificate.getMetadata().getRelations());
  ...
  private void monitorLog(
    ...
    if (certificateStatusBeforeRevoke == CertificateStatus.LOCKED) {
      monitoringLogService.logUtkastRevoked(certificate.getMetadata().getId(), revokedBy, reason);
    }
