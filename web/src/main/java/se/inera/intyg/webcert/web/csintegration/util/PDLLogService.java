...
public class PDLLogService {
  ...
  public void logCreatedWithIntygUser(Certificate certificate, IntygUser user) {
    logService.logCreateIntyg(
        logRequestFactory.createLogRequestFromCertificate(certificate, additionalInfo()),
        new LogUser.Builder(
                user.getHsaId(), user.getValdVardenhet().getId(), user.getValdVardgivare().getId())
  ...
  public void logSent(Certificate certificate) {
    logService.logSendIntygToRecipient(
        logRequestFactory.createLogRequestFromCertificate(
            certificate, additionalInfoWithRecipient(certificate, additionalInfo())));
  }
  ...
  public void logCreateMessage(String personId, String certificateId) {
    logService.logCreateMessage(webCertUserService.getUser(), personId, certificateId);
  }
  ...
  private String additionalInfoWithRecipient(Certificate certificate, String additionalInfo) {
    final var recipientMessage =
        String.format(
            "Intyg skickat till mottagare %s", certificate.getMetadata().getRecipient().getName());
  ...
  public void logReadLevelTwo(Certificate response) {
    logService.logReadLevelTwo(
        webCertUserService.getUser(), response.getMetadata().getPatient().getPersonId().getId());
  }
}
