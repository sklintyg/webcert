...
public class ListFilterHelper {
  ...
  private static List<UtkastStatus> getDraftStatusListFromFilter(String status) {
    final var showAll =
        Arrays.asList(
            UtkastStatus.DRAFT_COMPLETE, UtkastStatus.DRAFT_LOCKED, UtkastStatus.DRAFT_INCOMPLETE);
  ...
  public static String convertOrderBy(ListFilter filter, ListType listType) {
    ...
    return listType == ListType.CERTIFICATES
        ? convertOrderBy(orderBy.getValue())
        : convertOrderByForQuestions(orderBy.getValue());
  }
