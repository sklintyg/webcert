...
/**
 * This class intefaces with io.swagger core classes for on-the-fly generation of Swagger .JSON
 * files for use by swagger-ui.
 *
 * <p>Derived from io.swagger.jaxrs.listing.ApiListingResource.
 *
 * <p>Created by eriklupander on 2015-11-09.
 */
// CHECKSTYLE:OFF LineLength
@Path("/")
@Api(
    value = "/services/swagger",
    description = "REST API för att generera swagger-beskrivning av våra REST API:er",
...
public class ApiScanner {
  ...
  /**
   * Returns a list of key->value pairs of REST APIs declared in swagger-cxf-servlet.xml. Primarily
   * used by our customized swagger-ui to list available endpoints.
   *
  ...
  private Set<Class<?>> resolveClasses(
      Application app, ServletConfig sc, ReflectiveJaxrsScanner scanner) {
    Set<Class<?>> classes;
  ...
  private String resolveBasePathForApi(String api) {
    if (basepathMap.containsKey(api)) {
      ...
      throw new IllegalArgumentException(
          "Cannot generate Swagger docs for API '"
              + api
              + "'. Its basepath must be mapped in swagger-cxf-servlet.xml");
    }
  }

  private Swagger process(
      Application app, ServletConfig sc, HttpHeaders headers, UriInfo uriInfo, String api) {
    Swagger swagger = (Swagger) context.getAttribute(SWAGGER_TOKEN + api);
    if (!apiInitialized.containsKey(SWAGGER_TOKEN + api)
        || !apiInitialized.get(SWAGGER_TOKEN + api)) {
