...
/**
 * Enum that defines different results when access is checked. If NO_PROBLEM is returned, the user
 * is considered having access. If any other code is returned, no access is given and the code
 * explains the reason for the restriction.
 */
public enum AccessResultCode {
  ...
  /**
   * No access, could not check if patient has sekretess or not due to unavailable
  ...
  /**
   * No access. Not allowed to handle patient with sekretess on another CareUnit than the user is
   * logged in to.
   */
