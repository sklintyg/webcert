 ...
 * Vardenhet -> Mottagning tree.
 *
 * <p>Created by eriklupander on 2017-09-27.
 */
public final class HoSPersonHelper {
  ...
  public static Vardenhet createVardenhetFromIntygUser(String enhetId, IntygUser user) {
    ...
    AbstractVardenhet enhet =
        HoSPersonHelper.findVardenhetEllerMottagning(user, enhetId)
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "User '"
                            + user.getHsaId()
                            + "' has no MIU for care unit '"
                            + enhetId
                            + "'"));
      ...
      Vardgivare hsaVardgivare =
          HoSPersonHelper.findVardgivare(user, hsaVardenhet.getVardgivareHsaId())
              .orElseThrow(
                  () ->
                      new IllegalStateException(
                          "Unable to find parent vårdgivare for vardenhet '" + enhetId + "'"));
    ...
    if (enhet instanceof Mottagning) {
      ...
      Vardgivare hsaVardgivare =
          HoSPersonHelper.findVardgivareForMottagning(user, m.getId())
              .orElseThrow(
                  () ->
                      new IllegalStateException(
                          "Unable to find parent vårdgivare for mottagning '" + enhetId + "'"));
    ...
    throw new IllegalArgumentException(
        "AbstractVardenhet instance passed to createVardenhetFromIntygUser was of unsupported type: "
            + enhet.getClass().getName());
  }
