...
public class CertificateListItemConverterImpl implements CertificateListItemConverter {
  ...
  private CertificateListItem convertListItem(ListIntygEntry listIntygEntry, ListType listType) {
    ...
    final var certificateListItemStatus =
        listType == ListType.CERTIFICATES
            ? getCertificateListItemStatus(listIntygEntry.getStatus().equals(SENT))
            : getCertificateListItemStatus(
                listIntygEntry.getStatus(), listIntygEntry.getRelations());
    final var patientListInfo = getPatientListInfo(listIntygEntry);
    final var convertedLinks =
        resourceLinkListHelper.get(listIntygEntry, certificateListItemStatus);
    ...
    listItem.addValue(
        ...
        listType == ListType.CERTIFICATES
            ? listIntygEntry.getSigned()
            : listIntygEntry.getLastUpdated());
    listItem.addValue(ListColumnType.PATIENT_ID, patientListInfo);
    listItem.addValue(
        listType == ListType.DRAFTS ? ListColumnType.SAVED_BY : ListColumnType.SAVED_SIGNED_BY,
  ...
  private CertificateListItem convertListItem(ArendeListItem entry) {
    ...
    if (isAllowedToForward(convertedLinks)) {
      listItem.addValue(
          ...
          getForwardedListInfo(
              entry.getEnhetsnamn(),
              entry.getVardgivarnamn(),
  ...
  private String getQuestionAction(ArendeListItem entry) {
    return ArendeServiceImpl.getAmneString(
        entry.getAmne(), entry.getStatus(), entry.isPaminnelse(), entry.getFragestallare());
  }
  ...
  private CertificateListItem convertListItem(CertificateListEntry entry) {
    ...
    listItem.addValue(
        ListColumnType.LINKS, resourceLinkListHelper.get(entry, certificateListItemStatus));
    return listItem;
  }
  ...
  private CertificateListItemStatus getCertificateListItemStatus(
      String status, Relations relations) {
  ...
  private boolean checkActiveRelation(
      WebcertCertificateRelation relationFromCertificate,
      WebcertCertificateRelation relationFromDraft) {
    return (relationFromCertificate != null && !relationFromCertificate.isMakulerat())
        || relationFromDraft != null;
  }
  ...
  private boolean isComplemented(Relations relations) {
    ...
    final var complementedByCertificate =
        relations.getLatestChildRelations().getComplementedByIntyg();
  ...
  private boolean isAllowedToForward(List<ResourceLinkDTO> links) {
    return links.stream()
        .anyMatch(
            link ->
                link.getType() == ResourceLinkTypeDTO.FORWARD_CERTIFICATE
                    || link.getType() == ResourceLinkTypeDTO.FORWARD_QUESTION);
  }
  ...
  private ForwardedListInfo getForwardedListInfo(ListIntygEntry entry) {
    ...
    return getForwardedListInfo(
        unit.getNamn(), careGiver.getNamn(), entry.isVidarebefordrad(), entry.getIntygType());
  }

  private ForwardedListInfo getForwardedListInfo(
      String unitName, String careGiverName, boolean isForwarded, String certificateType) {
    return new ForwardedListInfo(isForwarded, unitName, careGiverName, certificateType);
  }
}
