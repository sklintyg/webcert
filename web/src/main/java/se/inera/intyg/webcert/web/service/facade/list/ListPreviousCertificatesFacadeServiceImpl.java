...
@Service
public class ListPreviousCertificatesFacadeServiceImpl
    implements ListPreviousCertificatesFacadeService {
  ...
  private static final List<String> CURRENT_CERTIFICATES =
      Arrays.asList(
          CertificateListItemStatus.INCOMPLETE.getName(),
          CertificateListItemStatus.COMPLETE.getName(),
          CertificateListItemStatus.SIGNED.getName(),
          CertificateListItemStatus.SENT.getName());

  private static final List<String> MODIFIED_CERTIFICATES =
      Arrays.asList(
          CertificateListItemStatus.LOCKED.getName(),
          CertificateListItemStatus.REVOKED.getName(),
          CertificateListItemStatus.REPLACED.getName(),
          CertificateListItemStatus.COMPLEMENTED.getName());
  ...
  public ListInfo get(ListFilter filter) {
    ...
    final var listFromCertificateService =
        listCertificatesAggregator.listCertificatesForPatient(patientId.getOriginalPnr());
    final var mergedList =
        Stream.concat(filteredList.stream(), listFromCertificateService.stream())
  ...
  private List<ListIntygEntry> filterProtectedPatients(
      WebCertUser user, Personnummer patientId, List<ListIntygEntry> list) {
        ...
        .orElseThrow(
            () ->
                new WebCertServiceException(
                    ...
                    String.format(
                        "Cannot create Personnummer object with invalid personId %s", patientId)));
  }
  ...
  private List<String> getUnits() {
    ...
    if (units.isEmpty()) {
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.INVALID_STATE, "Current user has no assignments");
    }
    return units;
  }

  private List<CertificateListItem> filterListOnStatus(
      ListFilter filter, List<CertificateListItem> list) {
  ...
  private List<CertificateListItem> performStatusFiltering(
      List<CertificateListItem> list, List<String> wantedStatuses) {
    return list.stream()
        .filter(item -> wantedStatuses.contains((String) item.getValue("STATUS")))
        .collect(Collectors.toList());
  }

  private boolean doesFilterMatchList(
      ListFilterRadioValue statusFilter, FilterStatusType statusType) {
