 ...
 * WebCertServiceException are logged as warnings. All other runtime exceptions thrown are treated
 * as errors and logged as such.
 *
 * @author nikpet
 */
public class WebcertRestExceptionHandler implements ExceptionMapper<RuntimeException> {
  ...
  private Response handleWebCertServiceException(WebCertServiceException wcse) {
    ...
    WebcertRestExceptionResponse exceptionResponse =
        new WebcertRestExceptionResponse(wcse.getErrorCode(), wcse.getMessage());
    return Response.status(Status.INTERNAL_SERVER_ERROR)
        .entity(exceptionResponse)
        .type(MediaType.APPLICATION_JSON)
        .build();
  ...
  private Response handleRuntimeException(RuntimeException re) {
    ...
    WebcertRestExceptionResponse exceptionResponse =
        new WebcertRestExceptionResponse(
            WebCertServiceErrorCodeEnum.UNKNOWN_INTERNAL_PROBLEM, re.getMessage());
    return Response.status(Status.INTERNAL_SERVER_ERROR)
        .entity(exceptionResponse)
        .type(MediaType.APPLICATION_JSON)
        .build();
