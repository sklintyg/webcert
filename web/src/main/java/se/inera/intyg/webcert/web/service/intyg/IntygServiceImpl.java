...
public class IntygServiceImpl implements IntygService {
  ...
  private static void copyOldAddressToNewPatientData(
      Patient oldPatientData, Patient newPatientData) {
  ...
  @PostConstruct
  public void init() {
    sekretessmarkeringStartDatum =
        LocalDateTime.parse(sekretessmarkeringProdDate, DateTimeFormatter.ISO_DATE_TIME);
  }
  ...
  @Override
  public IntygContentHolder fetchIntygData(
  ...
  @Override
  public IntygContentHolder fetchIntygDataForInternalUse(
      String certificateId, boolean includeRelations) {
    return fetchIntygData(certificateId, null, includeRelations, false, false);
  }

  @Override
  public IntygContentHolder fetchIntygDataforCandidate(
   ...
   * @param relations If the relations between intyg should be populated. This can be expensive
   *     (several database operations). Use sparsely.
   ...
   */
  private IntygContentHolder fetchIntygData(
      String intygsId,
      String intygsTyp,
      boolean relations,
      boolean pdlLogging,
    ...
    if (pdlLogging) {
      LogRequest logRequest =
          logRequestFactory.createLogRequestFromUtlatande(
              intygsData.getUtlatande(), checkSjf(intygsData));
  ...
  @Override
  public Pair<List<ListIntygEntry>, Boolean> listIntyg(
      List<String> enhetId, Personnummer personnummer) {
    ...
    try {
      ListCertificatesForCareResponseType response =
          listCertificateService.listCertificatesForCare(logicalAddress, request);
      List<ListIntygEntry> fullIntygItemList =
          intygConverter.convertIntygToListIntygEntries(
              response.getIntygsLista().getIntyg(), webcertCerts);
    ...
    // If intygstjansten was unavailable, we return whatever certificates we can find and clearly
    // inform
  ...
  public List<ListIntygEntry> listIntygFromIT(List<String> enhetId, Personnummer personnummer) {
    ...
    final var fullIntygItemList =
        response.getIntygsLista().getIntyg().stream()
  ...
  private List<ListIntygEntry> filterByIntygTypeForUser(
      List<ListIntygEntry> fullIntygItemList, SekretessStatus sekretessmarkering) {
    ...
    Set<String> intygsTyper =
        (sekretessmarkering == SekretessStatus.TRUE
                || sekretessmarkering == SekretessStatus.UNDEFINED)
            ? filterAllowedForSekretessMarkering(base)
            : base;

    // The user is only granted access to view intyg of intygstyper that are in the set.
    return fullIntygItemList.stream()
        .filter(i -> intygsTyper.contains(i.getIntygType()))
        .collect(Collectors.toList());
  }

  /**
   * Adds any IntygItems found in Webcert for this patient not present in the list from
   * intygstjansten.
   */
  private void addDraftsToListForIntygNotSavedInIntygstjansten(
      List<ListIntygEntry> fullIntygItemList, List<ListIntygEntry> webcertIntygItems) {
            ...
            .filter(
                wcIntyg ->
                    fullIntygItemList.stream()
                        .noneMatch(
                            itIntyg -> itIntyg.getIntygId().equalsIgnoreCase(wcIntyg.getIntygId())))
            .collect(Collectors.toList()));
  }
  ...
  private List<ListIntygEntry> getIntygFromWebcert(
      List<String> enhetId, Personnummer personnummer) {
    ...
    List<Utkast> drafts =
        utkastRepository.findDraftsByPatientAndEnhetAndStatus(
            DaoUtil.formatPnrForPersistence(personnummer), enhetId, statuses, intygsTyper);
    return buildIntygItemListFromDrafts(drafts);
  }

  private Set<String> filterAllowedForSekretessMarkering(Set<String> base) {
    Set<String> allowedForSekretess =
        authoritiesHelper.getIntygstyperAllowedForSekretessmarkering();
    return base.stream().filter(allowedForSekretess::contains).collect(Collectors.toSet());
  }
  ...
  public IntygPdf fetchIntygAsPdf(String intygsId, String intygsTyp, boolean isEmployerCopy) {
    try {
      ...
      boolean revoked =
          intyg.getStatuses() != null
              && intyg.getStatuses().stream()
                  .anyMatch(s -> s.getType().equals(CertificateState.CANCELLED));
      if (revoked) {
        throw new WebCertServiceException(
            WebCertServiceErrorCodeEnum.INVALID_STATE, "Can't print revoked certificate.");
      }
      ...
      IntygPdf intygPdf =
          moduleFacade.convertFromInternalToPdfDocument(
              intygsTyp, intyg.getContents(), intyg.getStatuses(), utkastStatus, isEmployerCopy);
  ...
  public IntygServiceResult storeIntyg(Utkast utkast) {
    ...
    try {
      certificateSenderService.storeCertificate(
          utkast.getIntygsId(), utkast.getIntygsTyp(), utkast.getModel());
      return IntygServiceResult.OK;
    } catch (CertificateSenderException cse) {
      LOG.error(
          String.format("Could not put certificate store message on queue: %s", cse.getMessage()),
          cse);
  ...
  @Override
  public IntygServiceResult sendIntyg(
      String intygsId, String typ, String recipient, boolean delay) {

    final Optional<Utkast> optionalUtkast =
        Optional.ofNullable(utkastRepository.findById(intygsId).orElse(null));

    final Utlatande utlatande =
        optionalUtkast
            .map(
                utkast ->
                    moduleFacade.getUtlatandeFromInternalModel(
                        utkast.getIntygsTyp(), utkast.getModel(), utkast.getSkapad()))
    ...
    if (optionalUtkast.isPresent()) {
      ...
      try {
        ...
        certificate =
            moduleFacade.getCertificate(intygsId, typ, intygTypeInfo.getIntygTypeVersion());
      } catch (IntygModuleFacadeException e) {
        throw new WebCertServiceException(WebCertServiceErrorCodeEnum.MODULE_PROBLEM, e);
      }
    ...
    // WC-US-SM-001 - vi får ej skicka FK-intyg för sekretessmarkerad patient som innehåller
    // personuppgifter.
    verifyNoExposureOfSekretessmarkeradPatient(utlatande);

    SendIntygConfiguration sendConfig =
        new SendIntygConfiguration(recipient, webCertUserService.getUser());
    ...
    LogRequest logRequest =
        logRequestFactory.createLogRequestFromUtlatande(
            utlatande, sendConfig.getPatientConsentMessage());
  ...
  // Kontrollera om det signerade intyget i intygstjänsten har namn- eller adressuppgifter. Om så är
  // fallet,
  ...
  private void verifyNoExposureOfSekretessmarkeradPatient(Utlatande intyg) {
    ...
    SekretessStatus sekretessStatus =
        patientDetailsResolver.getSekretessStatus(intyg.getGrundData().getPatient().getPersonId());
    ...
    if (Fk7263EntryPoint.MODULE_ID.equalsIgnoreCase(intyg.getTyp())
      ...
      if (intyg.getGrundData().getSigneringsdatum().isBefore(sekretessmarkeringStartDatum)) {
        throw new WebCertServiceException(
            WebCertServiceErrorCodeEnum.CERTIFICATE_TYPE_SEKRETESSMARKERING_HAS_PUDATA,
            "Cannot send certificate for sekretessmarkerad patient having existing name or address.");
      }
    }
  }

  private void verifyNotReplacedBySignedIntyg(
      final String intygsId, final IntygOperation operation) {
    final Optional<WebcertCertificateRelation> replacedByRelation =
        certificateRelationService.getNewestRelationOfType(
            intygsId, RelationKod.ERSATT, Arrays.asList(UtkastStatus.SIGNED));
    if (replacedByRelation.isPresent() && !replacedByRelation.get().isMakulerat()) {
      String errorString =
          String.format(
              "Cannot %s certificate '%s', the certificate is replaced by certificate '%s'",
              operation.getValue(), intygsId, replacedByRelation.get().getIntygsId());
      LOG.debug(errorString);
      throw new WebCertServiceException(WebCertServiceErrorCodeEnum.INVALID_STATE, errorString);
    }
  }
   ...
   */
  @Override
  public IntygServiceResult revokeIntyg(
      String intygsId, String intygsTyp, String revokeMessage, String reason) {
    ...
    if (intyg.isRevoked()) {
      ...
      throw new WebCertServiceException(
          WebCertServiceErrorCodeEnum.INVALID_STATE, "Certificate is already revoked");
    }

    try {
      certificateSenderService.revokeCertificate(
          intygsId,
          moduleFacade.getRevokeCertificateRequest(
              intygsTyp,
              intyg.getUtlatande(),
              IntygConverterUtil.buildHosPersonalFromWebCertUser(
  ...
  /**
   * Check if signed certificate is a completion, in that case, send to recipient and close pending
   * completion QA / Arende as handled.
   *
   * <p>Check if signed certificate should be sent directly to default recipient for this intygstyp.
   *
   * <p>Note that the send operation uses the "delay" boolean to allow the signing operation some
   * time to complete in intygstjansten.
   */
  @Override
  public void handleAfterSigned(Utkast utkast) {
    List<RelationKod> shouldCloseCompletionCodes =
        Lists.newArrayList(RelationKod.KOMPLT, RelationKod.ERSATT, RelationKod.FRLANG);
    boolean shouldCloseCompletions =
        shouldCloseCompletionCodes.stream().anyMatch(it -> it == utkast.getRelationKod());
    boolean isSigneraSkickaDirekt =
        authoritiesHelper.isFeatureActive(
            AuthoritiesConstants.FEATURE_SIGNERA_SKICKA_DIREKT, utkast.getIntygsTyp());
    if (isNotATestIntyg(utkast)
        && (isSigneraSkickaDirekt || utkast.getRelationKod() == RelationKod.KOMPLT)) {
      try {
        LOG.info("Send intyg '{}' directly to recipient", utkast.getIntygsId());
        sendIntyg(
            utkast.getIntygsId(),
            utkast.getIntygsTyp(),
            moduleRegistry.getModuleEntryPoint(utkast.getIntygsTyp()).getDefaultRecipient(),
            true);
      } catch (ModuleNotFoundException e) {
        throw new WebCertServiceException(
            WebCertServiceErrorCodeEnum.MODULE_PROBLEM,
            "Could not send intyg directly to recipient",
            e);
      }
    }
  ...
  /**
   * Is considered a test intyg if the intyg is tagged with test intyg flag. Or if it was created
   * without the flag, it will still be considered a test intyg if the patient currently holds the
   * testIndicator-flag.
   */
  private boolean isNotATestIntyg(Utkast utkast) {
    return !(utkast.isTestIntyg()
        || patientDetailsResolver.isTestIndicator(utkast.getPatientPersonnummer()));
  }
  ...
  @Override
  public List<IntygWithNotificationsResponse> listCertificatesForCareWithQA(
  ...
  /**
   * When no draft is found in WC but there are notifications in the database, it could be of two
   * reasons. Either the certificate wasn't issued in WC (and will be found in IT) or it was removed
   * before it got signed. This method checks the latter based on what notifications exists for the
   * draft/certificate.
   *
  ...
  private HashMap<String, List<Handelse>> getNotificationCertificateIdHash(
      List<Handelse> allNotifications) {
  ...
  private IntygWithNotificationsResponse getIntygWithNotificationsResponse(
      Utkast draft, List<Handelse> notifications) {
    try {
      final var ref = referensService.getReferensForIntygsId(draft.getIntygsId());
      final ModuleApi api =
          moduleRegistry.getModuleApi(draft.getIntygsTyp(), draft.getIntygTypeVersion());
      final Intyg intyg = api.getIntygFromUtlatande(api.getUtlatandeFromJson(draft.getModel()));
      final Pair<ArendeCount, ArendeCount> arenden =
          fragorOchSvarCreator.createArenden(draft.getIntygsId(), draft.getIntygsTyp());
  ...
  private IntygWithNotificationsResponse getIntygWithNotificationsResponse(
      IntygContentHolder certificate, List<Handelse> notifications) {
    try {
      ...
      final Pair<ArendeCount, ArendeCount> arenden =
          fragorOchSvarCreator.createArenden(certificate.getUtlatande().getId(), certificateType);
  ...
  private IntygTypeInfo getIntygTypeInfoFromIT(String intygsId) {
    ...
    try {
      final GetCertificateTypeInfoResponseType certificateTypeInfo =
          getCertificateTypeInfoService.getCertificateTypeInfo(logicalAddress, requestType);
      return new IntygTypeInfo(
          intygsId, certificateTypeInfo.getTyp().getCode(), certificateTypeInfo.getTypVersion());
  ...
  protected IntygServiceResult sendIntygToCertificateSender(
      SendIntygConfiguration sendConfig, Utlatande intyg, boolean delay) {

    String intygsId = intyg.getId();
    String recipient = sendConfig.getRecipient();
    String intygsTyp = intyg.getTyp();
    HoSPersonal skickatAv =
        IntygConverterUtil.buildHosPersonalFromWebCertUser(webCertUserService.getUser(), null);
    ...
    try {
      ...
      certificateSenderService.sendCertificate(
          intygsId,
          intyg.getGrundData().getPatient().getPersonId(),
          objectMapper.writeValueAsString(skickatAv),
          recipient,
          delay);
  ...
  /**
   * Builds a IntygContentHolder by first trying to get the Intyg from intygstjansten. If not found
   * or the Intygstjanst couldn't be reached, the local Utkast - if available - will be used
   * instead.
   *
   * <p>Note that even when found, we check if we need to decorate the response with data from the
   * utkast in order to mitigate async send states. (E.g. a send may be in resend due to 3rd party
   * issues, in that case decorate with data about sent state from the Utkast)
   *
   * <p>The data will be updated with current patient data from PU unless otherwise stated in the
   * module api.
   */
  private IntygContentHolder getIntygData(String intygId, String typ, boolean relations) {
    ...
    try {
      ...
      CertificateResponse certificate =
          moduleFacade.getCertificate(intygId, intygType, intygTypeVersion);
      String internalIntygJsonModel = certificate.getInternalModel();

      final Personnummer personId =
      ...
      // Since an FK-intyg never will have anything other than personId, try to fetch all using
      // ruleset
      Patient newPatientData =
          patientDetailsResolver.resolvePatient(personId, intygType, intygTypeVersion);
      ...
      try {
        utlatande =
            moduleRegistry
                .getModuleApi(intygType, intygTypeVersion)
                .getUtlatandeFromJson(internalIntygJsonModel);
        patientNameChanged =
            patientDetailsResolver.isPatientNamedChanged(
                utlatande.getGrundData().getPatient(), newPatientData);
        patientAddressChanged =
            patientDetailsResolver.isPatientAddressChanged(
                utlatande.getGrundData().getPatient(), newPatientData);
      } catch (IOException e) {
        LOG.error(
      ...
      internalIntygJsonModel =
          moduleApi.updateBeforeViewing(
              internalIntygJsonModel, newPatientData, certificate.getMetaData().getSignDate());
      ...
      return IntygContentHolder.builder()
          ...
          .latestMajorTextVersion(
              intygTextsService.isLatestMajorVersion(intygType, intygTypeVersion))
          .build();

    } catch (IntygModuleFacadeException me) {
      // It's possible the Intygstjanst hasn't received the Intyg yet, look for it locally before
      // rethrowing
      // exception
      Utkast utkast = utkastRepository.findByIntygsIdAndIntygsTyp(intygId, intygType);
      if (utkast == null) {
    ...
    try {
      ...
      Patient newPatientData =
          patientDetailsResolver.resolvePatient(
              utkast.getPatientPersonnummer(), utkast.getIntygsTyp(), utkast.getIntygTypeVersion());
      ...
      // INTYG-7449, INTYG-7529: Update patient data before (will not be done on intyg that store
      // address data)
      String internalIntygJsonModel =
          moduleRegistry
              .getModuleApi(utkast.getIntygsTyp(), utkast.getIntygTypeVersion())
              .updateBeforeViewing(utkast.getModel(), newPatientData, utkast.getSkapad());

      utlatande =
          moduleFacade.getUtlatandeFromInternalModel(
              utkast.getIntygsTyp(), internalIntygJsonModel, utkast.getSkapad());
      List<Status> statuses = IntygConverterUtil.buildStatusesFromUtkast(utkast);
      Relations certificateRelations =
          certificateRelationService.getRelations(utkast.getIntygsId());

      final SekretessStatus sekretessStatus =
          patientDetailsResolver.getSekretessStatus(newPatientData.getPersonId());
      if (SekretessStatus.UNDEFINED.equals(sekretessStatus)) {
        throw new WebCertServiceException(
            WebCertServiceErrorCodeEnum.PU_PROBLEM,
            "Sekretesstatus could not be fetched from the PU service");
      ...
      boolean patientNameChanged =
          patientDetailsResolver.isPatientNamedChanged(
              utlatande.getGrundData().getPatient(), newPatientData);
      boolean patientAddressChanged =
          patientDetailsResolver.isPatientAddressChanged(
              utlatande.getGrundData().getPatient(), newPatientData);
      ...
      return IntygContentHolder.builder()
          ...
          .latestMajorTextVersion(
              intygTextsService.isLatestMajorVersion(
                  utlatande.getTyp(), utlatande.getTextVersion()))
          .build();
  ...
  /**
   * Send a notification message to stakeholders informing that a question related to a revoked
  ...
  private IntygServiceResult whenSuccessfulRevoke(Utlatande intyg, String reason) {
    ...
    certificateEventService.createCertificateEvent(
        intygsId, webCertUserService.getUser().getHsaId(), EventCode.MAKULERAT, reason);
  ...
  private void handleComplementedParent(String intygsId) {
    ...
    if (isParentCompleted(relationsOfChild)
        && isLatestChildCompletionAndMatchIntygsId(
            relationsOfChild.getParent().getIntygsId(), intygsId)) {
  ...
  private boolean isLatestChildCompletionAndMatchIntygsId(
      String intygsIdOfParent, String intygsIdToMatch) {
    ...
    return relationsOfParent != null
        ...
        && relationsOfParent.getLatestChildRelations().getComplementedByIntyg().getRelationKod()
            == RelationKod.KOMPLT
        && relationsOfParent
            .getLatestChildRelations()
            .getComplementedByIntyg()
            .getIntygsId()
            .equals(intygsIdToMatch);
  }

  private void markUtkastWithSendDateAndRecipient(
      final Utkast foundUtkast, String intygsId, String recipient) {

    final Utkast utkast =
        (foundUtkast != null) ? foundUtkast : utkastRepository.findById(intygsId).orElse(null);
