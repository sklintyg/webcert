...
public class UtkastCandidateServiceImpl {
  ...
  public Optional<UtkastCandidateMetaData> getCandidateMetaData(
    ...
    try {
      ...
      if (candidate.isPresent()) {
        ...
        // True if the enhet or vardenhet of the currently logged in user is same vardenhet or a
        // subunit
        ...
        metaData =
            new UtkastCandidateMetaData.Builder()
                .with(
                    builder -> {
        ...
        logService.logReadIntyg(
            logRequestFactory.createLogRequestFromUtkast(utkast, isCoherentJournaling),
            LogUtil.getLogUser(getUser()));
      }
      ...
      throw new WebCertServiceException(
          ...
          "Failed to lookup a candidate to copy certificate information from",
          e);
    }
  }
  ...
  private Optional<Utkast> filterLisjpCandidates(
      List<Utkast> candidates, GetCopyFromCriteria copyFromCriteria) {
    ...
    return candidates.stream()
        ...
        .filter(
            candidate ->
                filterOnMajorVersion(
                    candidate.getIntygTypeVersion(), copyFromCriteria.getIntygTypeMajorVersion()))
        .filter(candidate -> webCertUserService.isUserAllowedAccessToUnit(candidate.getEnhetsId()))
        .sorted(
            Comparator.comparing(
                u -> u.getSignatur().getSigneringsDatum(), Comparator.reverseOrder()))
        .findFirst();
  }

  private Optional<Utkast> filterDbCandidates(
      List<Utkast> candidates, GetCopyFromCriteria copyFromCriteria) {
    ...
    return candidates.stream()
        ...
        .filter(
            candidate -> getUser().getValdVardgivare().getId().equals(candidate.getVardgivarId()))
        .sorted(
            Comparator.comparing(
                u -> u.getSignatur().getSigneringsDatum(), Comparator.reverseOrder()))
        .findFirst();
  ...
  private boolean filterOnMajorVersion(String intygTypeVersion, String intygTypeMajorVersion) {
    return !Strings.isNullOrEmpty(intygTypeVersion)
        && !Strings.isNullOrEmpty(intygTypeMajorVersion)
