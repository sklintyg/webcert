...
/**
 * This is a bit of a hack to mitigate states when an intyg is on a resend queue due to a 3rd party
 * being unavailable so IT cannot finish a send/revoke properly. Since the Intyg in IT won't be
 * marked as sent/revoked until the operation has successfully finished, the intyg state shown in
 * Webcert GUI will be as NOT sent/revoked, even though we have performed that operation and
 * indicated in the GUI that we have sent or revoked it.
 *
 * <p>Thus, we decorate the returned information with revoked/sent state from the Utkast if
 * possible.
 */
@Component
public class UtkastIntygDecoratorImpl implements UtkastIntygDecorator {
  ...
  public void decorateWithUtkastStatus(CertificateResponse certificate) {
    ...
    Utkast utkast =
        utkastRepository.findById(certificate.getMetaData().getCertificateId()).orElse(null);
    ...
    if (utkast.getAterkalladDatum() != null && !isRevoked) {
      ...
      Status revokedStatus =
          new Status(CertificateState.CANCELLED, "HSVARD", utkast.getAterkalladDatum());
