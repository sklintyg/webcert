...
public class CertificateSignAndSendFunctionImpl implements CertificateSignAndSendFunction {
  ...
  public Optional<ResourceLinkDTO> get(Certificate certificate) {
    ...
    return Optional.of(
        ResourceLinkDTO.create(
            ResourceLinkTypeDTO.SIGN_CERTIFICATE, SIGN_NAME, SIGN_DESCRIPTION, true));
  }
  ...
  private boolean shouldBeSentToForsakringskassan(String certificateType) {
    return Fk7263EntryPoint.MODULE_ID.equals(certificateType)
        || LuseEntryPoint.MODULE_ID.equals(certificateType)
        || LisjpEntryPoint.MODULE_ID.equals(certificateType)
        || LuaefsEntryPoint.MODULE_ID.equals(certificateType)
        || LuaenaEntryPoint.MODULE_ID.equals(certificateType);
  }

  private boolean shouldBeSentToTransportstyrelsen(String certificateType) {
    return TsBasEntryPoint.MODULE_ID.equals(certificateType)
        || TsDiabetesEntryPoint.MODULE_ID.equals(certificateType);
  }
  ...
  private boolean isSignedAndSendDirectly(Certificate certificate) {
    return (authoritiesHelper.isFeatureActive(
        ...
        && !certificate.getMetadata().getPatient().isTestIndicated();
  }
