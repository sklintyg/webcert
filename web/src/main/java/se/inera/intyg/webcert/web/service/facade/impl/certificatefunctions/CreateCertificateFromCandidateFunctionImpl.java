...
@Component
public class CreateCertificateFromCandidateFunctionImpl
    implements CreateCertificateFromCandidateFunction {
  ...
  public Optional<ResourceLinkDTO> get(Certificate certificate) {
    ...
    if (!candidateMetadata.get().getSameVardenhet()
        && DbModuleEntryPoint.MODULE_ID.equalsIgnoreCase(candidateMetadata.get().getIntygType())) {
      return Optional.of(
  ...
  private boolean missingSupportToCreateFromCandidate(Certificate certificate) {
    return !(isFirstVersion(certificate)
        && noParentRelations(certificate)
        && isCertificateTypeSupported(certificate));
  }
  ...
  private String getBody(UtkastCandidateMetaData candidateMetaData) {
    ...
    throw new IllegalArgumentException(
        String.format(
            "Candidate certificate of type '%s' is not supported",
