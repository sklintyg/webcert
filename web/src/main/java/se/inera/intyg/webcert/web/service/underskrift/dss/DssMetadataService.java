...
/**
 * This service initializes metadata needed in the DSS (Digital Signing Service) integration It
 * depends on OpenSaml and that its bootstrapping is done prior to the instantiation of this
 * service, which in Webcert is done in the SAML configuration.
 */
@Service
@Slf4j
...
public class DssMetadataService {
  ...
  @Getter private KeyStore dssKeyStore;
  @Getter private KeySelector dssKeySelector;
  ...
  public String getClientMetadataAsString() {
    try {
      ...
      final var marshaller =
          Objects.requireNonNull(getMarshallerFactory().getMarshaller(clientEntityDescriptor));
      ...
      TransformerFactory.newInstance()
          .newTransformer()
          .transform(new DOMSource(metadataXml), metadata);
  ...
  protected void initDssMetadata() {
    try {
      dssServiceMetadataProvider =
          new SpringResourceBackedMetadataProvider(new Timer(true), dssServiceMetadataResource);
  ...
  protected void initClientMetadata() {
    try {
      ...
      final var privateKey =
          (PrivateKey) keyStoreSiths.getKey(keystoreAlias, keystorePassword.toCharArray());
      final var certificate = (X509Certificate) keyStoreSiths.getCertificate(keystoreAlias);

      final var assertionCOnsumerService =
          dssClientResponseHostUrl + SIGNATUR_API_CONTEXT_PATH + SIGN_SERVICE_RESPONSE_PATH;
      ...
      spssoDescriptor
          ...
          .add(createAssertionConsumerService(assertionCOnsumerService));
      ...
      final var entityId =
          dssClientEntityHostUrl + SIGNATUR_API_CONTEXT_PATH + SIGN_SERVICE_METADATA_PATH;
  ...
  private KeyDescriptor getKeyDescriptor(UsageType usageType, X509Certificate certificate)
  ...
  /**
   * Extracts the URL to use in SignRequest form. This URL should be used in the POST action of the
   * form.
   *
   * <p>The source of this URL could either come from property or metadata
   *
  ...
  public String getDssActionUrl() {
    ...
    if (StringUtils.hasText(actionUrlProperty)) {
      ...
      AssertionConsumerService assertionConsumerService =
          dssSpSsoDescriptor.getDefaultAssertionConsumerService();
  ...
  private void initDssKeyStore(SPSSODescriptor spSSODescriptor) {
    ...
    try {
      ...
      for (final var keyDescriptor : Objects.requireNonNull(keyDescriptors)) {
        ...
        for (final var x509Data : x509Datas) {
          ...
          for (final var x509Certificate : x509Certificates) {
            ...
            final var decodedX509Cert =
                Base64.getMimeDecoder().decode(Objects.requireNonNull(x509Certificate).getValue());
      ...
      log.error(
          "Unable to load DSS kyeStore from metadata resource: {}", dssServiceMetadataResource);
