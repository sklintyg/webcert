...
public class SrsServiceImpl implements SrsService {
  ...
  public SrsResponse getSrs(
    ...
    LOG.debug(
        "getSrs(user: [not logged], certificateId: {}, personalIdentificationNumber: [not logged], diagnosisCode: {},"
    ...
    // If we are in a draft, the main diagnosis might not yet have been persisted on the draft,
    // instead we use
    ...
    SrsResponse response =
        srsInfraService.getSrs(
            user,
            createPnr(personalIdentificationNumber),
            extensionChain,
            filter,
            answers,
            daysIntoSickLeave);
  ...
  @Override
  public Samtyckesstatus getConsent(String personalIdentityNumber, String careUnitHsaId)
      throws InvalidPersonNummerException {
  ...
  @Override
  public ResultCodeEnum setConsent(
      String personalIdentificationNumber, String careUnitHsaId, boolean consent)
      throws InvalidPersonNummerException {
    LOG.debug(
        "setConsent(personalIdentityNumber: [not logged], careUnitHsaId: {}, consent: {})",
        careUnitHsaId,
        consent);
  ...
  public ResultCodeEnum setOwnOpinion(
      ...
      String opinion) {
    LOG.debug(
        "setOwnOpinion(personalIdentityNumber: [not logged], careGiverHsaId: {}, careUnitHsaId: {}, "
            + "certificateId: {}, diagnosisCode: {})",
        careGiverHsaId,
        careUnitHsaId,
        certificateId,
        diagnosisCode);
  ...
  /**
   * Decorates the response with descriptions of the diagnosis codes that already are in the
   * response.
   */
  private void decorateWithDiagnosisDescription(SrsResponse response) {
            ...
            p -> {
              if (!Strings.isNullOrEmpty(p.getDiagnosisCode())) {
                DiagnosResponse diagnosResponse =
                    diagnosService.getDiagnosisByCode(
                        p.getDiagnosisCode(), Diagnoskodverk.ICD_10_SE);
                if (diagnosResponse.getResultat() == DiagnosResponseType.OK
                    && diagnosResponse.getDiagnoser() != null
    ...
    if (!Strings.isNullOrEmpty(response.getAtgarderDiagnosisCode())) {
      DiagnosResponse diagnosResponse =
          diagnosService.getDiagnosisByCode(
              response.getAtgarderDiagnosisCode(), Diagnoskodverk.ICD_10_SE);
      if (diagnosResponse.getResultat() == DiagnosResponseType.OK
          && diagnosResponse.getDiagnoser() != null
          && !diagnosResponse.getDiagnoser().isEmpty()) {
        response.setAtgarderDiagnosisDescription(
            diagnosResponse.getDiagnoser().get(0).getBeskrivning());
      }
    }
    if (!Strings.isNullOrEmpty(response.getStatistikDiagnosisCode())) {
      DiagnosResponse diagnosResponse =
          diagnosService.getDiagnosisByCode(
              response.getStatistikDiagnosisCode(), Diagnoskodverk.ICD_10_SE);
      if (diagnosResponse.getResultat() == DiagnosResponseType.OK
          && diagnosResponse.getDiagnoser() != null
          && !diagnosResponse.getDiagnoser().isEmpty()) {
        response.setStatistikDiagnosisDescription(
            diagnosResponse.getDiagnoser().get(0).getBeskrivning());
      }
    }
  }
  ...
  /**
   * Uses incoming parameters to constructs a response data filter to control which parts should be
   * included in the response from the SRS service (UtdataFilter).
   *
   ...
   * @return A filter object to control the response from the SRS service in accordance with the
   *     given parameters
   */
  private Utdatafilter buildResponseFilter(
      boolean addPrediction, boolean addMeasures, boolean addStatistics) {
