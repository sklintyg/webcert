...
public interface SrsService {
  ...
  /**
   * Returns recommended measures (SW: åtgärder), sick leave statistics and calculate risk
   * predictions based on diagnosis, personal data, regional data and answers to questions.
   *
   * <p>Takes input about the patient and answers to prediction questions to predict a personal risk
   * for a long sick leave (i.e. a sick leave longer than 90 days), also returns the general risk of
   * the population for the given diagnosis. Can also respond with recommended measures and sick
   * leave statistics for the given diagnosis.
   *
   * <p>Historic predictions for the user and diagnosis combination will always be included in the
   * response.
   *
   * <p>Performs PDL logging when required
   *
   * @param user The webcert user
   * @param certificateId The id of the certificate (SW: intygId) connected to the prediction
   * @param personalIdentificationNumber A Swedish personal identification number (SW: personnummer)
   *     on form YYYYMMDDNNNN identifying the subject of the risk prediction
   * @param diagnosisCode The main diagnosis of the current certificate
   * @param performRiskPrediction true to perform a new risk prediction and add to the response, any
   *     historic predictions on the same person and diagnosis are always in the response.
   ...
   * @param daysIntoSickLeave Number of days into the current sick leave, used as input to to the
   *     prediction
   * @return An object containing the requested response.
   * @throws InvalidPersonNummerException If the format of the personal identification number is
   *     incorrect
   ...
   */
  List<SrsQuestion> getQuestions(String diagnosisCode, String modelVersion)
      throws IllegalArgumentException;

  /**
   * Returns the consent status for SRS for a given person at a given care unit. The status can be
   * YES if the person has consented, NO if the person has actively declined the consent or NONE if
   * the person yet haven't answered regarding consent for SRS on the given care unit
   *
   * @param personalIdentityNumber A Swedish personal identification number (SW: personnummer) of
   *     the consenting person on form YYYYMMDDNNNN
   ...
   */
  Samtyckesstatus getConsent(String personalIdentityNumber, String careUnitHsaId)
   ...
   *
   * @param personalIdentificationNumber A Swedish personal identification number (SW: personnummer)
   *     of the prediction subject person on form YYYYMMDDNNNN
   * @param careUnitHsaId HSA-Id of the care unit
   * @param consent true if the person consents, false if the person actively declines the consent
   * @return Result of the update operation
   * @throws InvalidPersonNummerException If the personal identification number was incorrect
   */
  ResultCodeEnum setConsent(
      String personalIdentificationNumber, String careUnitHsaId, boolean consent)
      throws InvalidPersonNummerException;
   ...
   * @param personalIdentificationNumber A Swedish personal identification number (SW: personnummer)
   *     of the prediction subject person on form YYYYMMDDNNNN
   * @param careGiverHsaId HSA-id of the care giver where the practitioner currently works, we don't
   *     store the actual practitioner
