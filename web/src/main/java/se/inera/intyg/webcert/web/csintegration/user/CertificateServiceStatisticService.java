...
public class CertificateServiceStatisticService {
  ...
  public void add(
      UserStatisticsDTO statisticsFromWC,
      List<String> unitIds,
      WebCertUser user,
      boolean maxCommissionsExceeded) {

    final var statisticsFromCS =
        csIntegrationService.getStatistics(
            csIntegrationRequestFactory.getStatisticsRequest(unitIds));

    if (user.getValdVardenhet() != null) {
      statisticsFromWC.addNbrOfDraftsOnSelectedUnit(
          getNbrOfDraftsOnSelectedUnit(user, statisticsFromCS));
      statisticsFromWC.addNbrOfUnhandledQuestionsOnSelectedUnit(
          getNbrOfUnhandledQuestionsOnSelectedUnit(user, statisticsFromCS));
    ...
    if (!maxCommissionsExceeded) {
      final var careUnitsWithRelatedSubUnits =
          getAvailableCareUnitsWithSubUnits(user.getVardgivare());
      final var unitStatisticsDTO =
          buildUnitStatisticsFromCS(statisticsFromCS, careUnitsWithRelatedSubUnits);
      statisticsFromWC.mergeUnitStatistics(unitStatisticsDTO);
    }
  }

  private static Map<String, UnitStatisticsDTO> buildUnitStatisticsFromCS(
            ...
            Collectors.toMap(
                ...
                statisticForUnit -> {
                  ...
                  if (careUnitsWithRelatedSubUnits.containsKey(statisticForUnit.getKey())) {
                    final var subUnits =
                        careUnitsWithRelatedSubUnits.get(statisticForUnit.getKey());
                    subUnits.forEach(
                        unit -> {
                          if (!statisticsFromCS.containsKey(unit)) {
                            return;
                          }
                          unitStatistics.addQuestionsOnSubUnits(
                              statisticsFromCS.get(unit).getUnhandledMessageCount());
                          unitStatistics.addDraftsOnSubUnits(
                              statisticsFromCS.get(unit).getDraftCount());
                        });
                  }
  ...
  private static Map<String, List<String>> getAvailableCareUnitsWithSubUnits(
      List<Vardgivare> careProviders) {
  ...
  private static long getTotalDraftsAndUnhandledQuestionsOnOtherUnits(
      WebCertUser user, Map<String, StatisticsForUnitDTO> statisticsFromCS) {
    return statisticsFromCS.keySet().stream()
        .filter(unitId -> !user.getIdsOfSelectedVardenhet().contains(unitId))
        .mapToLong(
            unitId ->
                statisticsFromCS.get(unitId).getDraftCount()
                    + statisticsFromCS.get(unitId).getUnhandledMessageCount())
        .sum();
  }

  private static int getNbrOfUnhandledQuestionsOnSelectedUnit(
      WebCertUser user, Map<String, StatisticsForUnitDTO> statisticsFromCS) {
    return statisticsFromCS.keySet().stream()
  ...
  private static int getNbrOfDraftsOnSelectedUnit(
      WebCertUser user, Map<String, StatisticsForUnitDTO> statisticsFromCS) {
    return statisticsFromCS.keySet().stream()
        .filter(unitId -> user.getValdVardenhet().getId().equals(unitId))
        .findFirst()
