...
public class IntygToCertificateConverterImpl implements IntygToCertificateConverter {
  ...
  private Certificate convertToCertificate(IntygContentHolder certificate) {
    ...
    certificateToReturn
        .getMetadata()
        .setCreated(certificate.getUtlatande().getGrundData().getSigneringsdatum());
    ...
    certificateToReturn
        .getMetadata()
        .setCareProvider(getCareProvider(certificate.getUtlatande().getGrundData().getSkapadAv()));
    ...
    certificateToReturn.getMetadata().setStatus(getStatus(certificate.getStatuses()));
  ...
  private Unit getCareUnit(Vardenhet unit) {
    try {
      final var careUnitId =
          hsatkOrganizationService.getHealthCareUnit(unit.getEnhetsid()).getHealthCareUnitHsaId();
      final var careUnit =
      ...
      return Unit.builder().unitId(unit.getEnhetsid()).unitName(unit.getEnhetsnamn()).build();
    }
  }
