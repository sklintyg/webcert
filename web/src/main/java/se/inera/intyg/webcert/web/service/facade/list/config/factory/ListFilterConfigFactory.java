...
import se.inera.intyg.webcert.web.service.facade.list.config.dto.ListFilterDateConfig;
...
public class ListFilterConfigFactory {
  ...
  public static ListFilterDateRangeConfig signedDateRange() {
    ...
    return new ListFilterDateRangeConfig(
        "SIGNED",
        "Signeringsdatum",
        toDate(),
        fromDateWithLimits(null, min, min),
        true,
        SIGNED_DESCRIPTION);
  }
  ...
  public static ListFilterSelectConfig forwardedSelect() {
    return new ListFilterSelectConfig(
        ...
        List.of(
            ...
            ListFilterConfigValue.create(
                ForwardedType.NOT_FORWARDED.toString(),
                ForwardedType.NOT_FORWARDED.getName(),
                false)));
  }
  ...
  public static ListFilterSelectConfig draftStatusSelect() {
    return new ListFilterSelectConfig(
        ...
        List.of(
            ListFilterConfigValue.create(
                CertificateListItemStatus.SHOW_ALL.toString(),
                CertificateListItemStatus.SHOW_ALL.getName(),
                true),
            ListFilterConfigValue.create(
                CertificateListItemStatus.INCOMPLETE.toString(), "Uppgifter saknas", false),
            ListFilterConfigValue.create(
                CertificateListItemStatus.COMPLETE.toString(), "Kan signeras", false),
            ListFilterConfigValue.create(
                CertificateListItemStatus.LOCKED.toString(), "Låsta", false)));
  }
  ...
  public static ListFilterSelectConfig createStaffSelect(
    ...
    final var convertedSavedByList =
        staffInfo.stream()
            .map((info) -> convertStaffInfoIntoSelectFilter(info, defaultHsaId))
            .collect(Collectors.toList());
    convertedSavedByList.add(
        0, ListFilterConfigValue.create("SHOW_ALL", "Visa alla", defaultHsaId.equals("SHOW_ALL")));
  ...
  private static ListFilterConfigValue convertStaffInfoIntoSelectFilter(
    ...
    return ListFilterConfigValue.create(
        staffListInfo.getHsaId(),
        staffListInfo.getName(),
        defaultHsaId.equals(staffListInfo.getHsaId()));
  }

  public static ListFilterRadioConfig certificateStatusRadio() {
    return new ListFilterRadioConfig(
        "STATUS",
        "",
        List.of(
            ListFilterConfigValue.create(
                FilterStatusType.CURRENT_CERTIFICATES.toString(),
                FilterStatusType.CURRENT_CERTIFICATES.getName(),
                true),
            ListFilterConfigValue.create(
                FilterStatusType.MODIFIED_CERTIFICATES.toString(),
                FilterStatusType.MODIFIED_CERTIFICATES.getName(),
                false),
            ListFilterConfigValue.create(
                FilterStatusType.ALL_CERTIFICATES.toString(),
                FilterStatusType.ALL_CERTIFICATES.getName(),
                false)));
  }
