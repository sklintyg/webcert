...
public class QuestionStatusFilter {
  ...
  public Boolean validate(ArendeListItem question, QuestionStatusType statusToFilterOn) {
    switch (statusToFilterOn) {
      ...
      case COMPLEMENT:
        return isQuestionAnsweredByComplement(question)
            && isQuestionWaitingOnActionFromCare(question);
      case ANSWER:
  ...
  private static boolean isQuestionAnsweredByText(final ArendeListItem question) {
    return List.of(ArendeAmne.KONTKT.name(), ArendeAmne.OVRIGT.name(), ArendeAmne.AVSTMN.name())
        .contains(question.getAmne());
  }
