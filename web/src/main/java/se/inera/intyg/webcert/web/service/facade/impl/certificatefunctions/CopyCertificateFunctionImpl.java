...
public class CopyCertificateFunctionImpl implements CopyCertificateFunction {
  ...
  private static final String COPY_DESCRIPTION =
      "Skapar en redigerbar kopia av utkastet på den enheten du är inloggad på.";

  private static final String COPY_DESCRIPTION_BODY =
      "<p>"
  ...
  private static final String COPY_DESCRIPTION_BODY_WITH_MESSAGE =
  ...
  public Optional<ResourceLinkDTO> get(Certificate certificate) {
    ...
    final var patientId =
        Personnummer.createPersonnummer(
            ...
            .orElseThrow();
    final var message =
        certificateTypeMessageService.get(certificate.getMetadata().getType(), patientId);
    ...
    if (isCopyCertificateContinueAvailable(certificate)) {
      return Optional.of(
          ResourceLinkDTO.create(
              ResourceLinkTypeDTO.COPY_CERTIFICATE_CONTINUE, COPY_NAME, COPY_DESCRIPTION, true));
    }
    return Optional.empty();
  }

  private Optional<ResourceLinkDTO> getResourceLineWithMessage(
      CertificateMessage message, String certificateType) {
    ...
    return Optional.of(
        ResourceLinkDTO.create(
            ResourceLinkTypeDTO.COPY_CERTIFICATE, COPY_NAME, message.getMessage(), false));
  }

  private boolean isResourceLinkEnabled(
      CertificateMessageType messageType, String certificateType) {
    return (CERTIFICATE_ON_DIFFERENT_CARE_PROVIDER.equals(messageType)
            && DoiModuleEntryPoint.MODULE_ID.equals(certificateType))
