...
public class WebSecurityConfig {
  ...
  public RelyingPartyRegistrationRepository relyingPartyRegistrationRepository()
    ...
    final var appPrivateKey =
        (PrivateKey) keyStoreSiths.getKey(keyAlias, keyStorePassword.toCharArray());
    final var appCertificate = (X509Certificate) keyStoreSiths.getCertificate(keyAlias);

    final var registrationEleg =
        buildRegistration(
            samlIdpMetadataLocationEleg,
            REGISTRATION_ID_ELEG,
            samlEntityIdEleg,
            assertionConsumerServiceLocationEleg,
            appPrivateKey,
            appCertificate);

    final var registrationSiths =
        buildRegistration(
            samlIdpMetadataLocationSiths,
            REGISTRATION_ID_SITHS,
            samlEntityIdSiths,
            assertionConsumerServiceLocationSiths,
            appPrivateKey,
            appCertificate);

    final var registrationSithsNormal =
        buildRegistration(
            samlIdpMetadataLocationSiths,
            REGISTRATION_ID_SITHS_NORMAL,
            samlEntityIdSithsNormal,
            assertionConsumerServiceLocationSithsNormal,
            appPrivateKey,
            appCertificate);

    return new InMemoryRelyingPartyRegistrationRepository(
        List.of(registrationSiths, registrationEleg, registrationSithsNormal));
  }
  ...
  public SecurityFilterChain filterChain(
    ...
    http.authorizeHttpRequests(
        ...
        .requestCache(cacheConfigurer -> cacheConfigurer.requestCache(requestCache))
        .exceptionHandling(
            exceptionConfigurer ->
                exceptionConfigurer
        ...
        context -> {
          ...
          context
              ...
              .setRequestedAuthnContext(
                  buildRequestedAuthnContext(context.getRelyingPartyRegistration()));
        });
    ...
    logoutRequestResolver.setParametersConsumer(
        parameters -> {
          ...
          final var sessionIndex =
              new MySessionIndex(
                  SAML_2_0_PROTOCOL, ELEMENT_LOCAL_NAME_SESSION_INDEX, NAMESPACE_PREFIX_SAML2P);
          ...
          if (registrationId.equals(REGISTRATION_ID_SITHS)
              || registrationId.equals(REGISTRATION_ID_SITHS_NORMAL)) {
            final var issuerValue =
                METADATA_LOCATION_STRING_TEMPLATE.formatted(webcertDomainName, registrationId);
  ...
  private RelyingPartyRegistration buildRegistration(
      ...
      String entityId,
      String assertionConsumerLocation,
      PrivateKey key,
    ...
    return RelyingPartyRegistrations.fromMetadataLocation(metadataLocation)
        ...
        .signingX509Credentials(
            signing -> signing.add(Saml2X509Credential.signing(key, certificate)))
        .build();
  }
  ...
  private Saml2AuthenticationToken sithsSaml2AuthenticationToken(
    ...
    final var identityProviderForSign =
        getAttribute(authentication, ATTRIBUTE_IDENTITY_PROVIDER_FOR_SIGN);
    final var principal =
        webcertUserDetailsService.buildUserPrincipal(personId, authMethod, identityProviderForSign);
  ...
  private Saml2AuthenticationToken elegSaml2AuthenticationToken(
    ...
    final var principal =
        elegWebCertUserDetailsService.buildUserPrincipal(personId, authScheme, authMethod);
    final var saml2AuthenticationToken = new Saml2AuthenticationToken(principal, authentication);
    saml2AuthenticationToken.setAuthenticated(true);
    return saml2AuthenticationToken;
  }
