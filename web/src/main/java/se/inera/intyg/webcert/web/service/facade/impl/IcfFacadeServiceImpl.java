...
public class IcfFacadeServiceImpl implements IcfFacadeService {
  ...
  private IcfResponse getIcfData(IcfRequestDTO request) {
    return icfService.findIcfInformationByIcd10Koder(
        Icd10KoderRequest.of(
            request.getIcd10Code(0), request.getIcd10Code(1), request.getIcd10Code(2)));
  }
  ...
  private IcfResponseDTO convert(IcfResponse response) {
    ...
    result.setActivityLimitation(
        AvailableIcfCodes.builder()
            .commonCodes(getActivityLimitationCommonCodes(response))
            .uniqueCodes(getActivityLimitationUniqueCodes(response))
            .build());

    result.setDisability(
        AvailableIcfCodes.builder()
            .commonCodes(getDisabilityCommonCodes(response))
  ...
  private IcfCodeCollection getCommonCodes(IcfKoder icfKoder) {
    ...
    return IcfCodeCollection.builder()
        .icd10Codes(
            icfKoder.getIcd10Koder().stream()
                .sorted(Collections.reverseOrder())
                .map(this::getIcdCode)
                .collect(Collectors.toList()))
        .icfCodes(getIcfCodeList(icfKoder.getIcfKoder()))
        .build();
  }
  ...
  private List<IcfCodeCollection> getDisabilityUniqueCodes(IcfResponse response) {
    ...
    return response.getUnika().stream()
        .map(
            icfDiagnoskodResponse ->
                IcfCodeCollection.builder()
                    .icd10Codes(List.of(getIcdCode(icfDiagnoskodResponse.getIcd10Kod())))
                    .icfCodes(
                        icfDiagnoskodResponse.getFunktionsNedsattningsKoder() != null
                            ? getIcfCodeList(
                                icfDiagnoskodResponse.getFunktionsNedsattningsKoder().getIcfKoder())
        ...
        .map(
            icfDiagnoskodResponse ->
                IcfCodeCollection.builder()
                    .icd10Codes(List.of(getIcdCode(icfDiagnoskodResponse.getIcd10Kod())))
                    .icfCodes(
                        icfDiagnoskodResponse.getAktivitetsBegransningsKoder() != null
                            ? getIcfCodeList(
                                icfDiagnoskodResponse
  ...
  private Icd10Code getIcdCode(String icd10Code) {
    return Icd10Code.builder().title(getDiagnosisTitle(icd10Code)).code(icd10Code).build();
  }
