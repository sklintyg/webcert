...
public final class AuthoritiesHelperUtil {
  ...
  public static final String DRAFT_FROM_SAME_CARE_PROVIDER_AND_UNIT_EXISTS =
      "Det finns ett utkast på %s för detta personnummer. Du kan inte skapa ett nytt utkast men kan däremot välja att fortsätta med det befintliga utkastet.";
  public static final String DRAFT_FROM_SAME_CARE_PROVIDER_ON_OTHER_UNIT_EXISTS =
      "Det finns ett utkast på %s för detta personnummer på annan vårdenhet. Du kan inte skapa ett nytt utkast men kan däremot välja att fortsätta med det befintliga utkastet.";
  public static final String DRAFT_FROM_OTHER_CARE_PROVIDER_EXISTS =
      "Det finns ett utkast på %1$s för detta personnummer hos annan vårdgivare. Senast skapade %1$s är det som gäller. Om du fortsätter och lämnar in %1$set så blir det därför detta %1$s som gäller.";
  public static final String CERTIFICATE_FROM_SAME_CARE_PROVIDER_AND_UNIT_EXISTS =
      "Det finns ett signerat %1$s för detta personnummer. Du kan inte skapa ett nytt %1$s men kan däremot välja att ersätta det befintliga %1$set.";
  public static final String CERTIFICATE_FROM_SAME_CARE_PROVIDER_ON_OTHER_UNIT_EXISTS =
      "Det finns ett signerat %1$s för detta personnummer på annan vårdenhet. Du kan inte skapa ett nytt %1$s men kan däremot välja att ersätta det befintliga %1$set.";
  public static final String CERTIFICATE_FROM_OTHER_CARE_PROVIDER_EXISTS =
      "Det finns ett signerat %1$s för detta personnummer hos annan vårdgivare. Det är inte möjligt att skapa ett nytt %1$s.";
  public static final String CERTIFICATE_FROM_OTHER_CARE_PROVIDER_EXISTS_OVERRIDE =
      "Det finns ett signerat %1$s för detta personnummer hos annan vårdgivare. Senast skapade %1$s är det som gäller. Om du fortsätter och lämnar in %1$set så blir det därför detta %1$s som gäller.";

  // CHECKSTYLE:ON LineLength

  private AuthoritiesHelperUtil() {}

  public static boolean mayNotCreateUtkastForSekretessMarkerad(
      SekretessStatus sekretessStatus, IntygUser user, String intygsTyp) {
  ...
  public static ValidateDraftCreationResponse performUniqueAndModuleValidation(
    ...
    return getPrimaryValidationMessage(
        certificateValidation, draftValidation, certificateTypeSpecificValidation);
  }
  ...
  private static ValidateDraftCreationResponse validateUniqueCertificate(
    ...
    PreviousIntyg previousCertificate =
        previousDraftAndCertificates.get(CERTIFICATE).get(certificateType);

    if (previousCertificate != null
        && (authoritiesValidator
                .given(user, certificateType)
      ...
      if (previousCertificate.isSameVardgivare()) {
        ...
        if (authoritiesValidator
            ...
            .isVerified()) {
          return createResponse(
              CERTIFICATE_FROM_OTHER_CARE_PROVIDER_EXISTS, ResultCodeType.ERROR, certificateType);
        } else if (authoritiesValidator
            .given(user, certificateType)
            .features(AuthoritiesConstants.FEATURE_UNIKT_INTYG_INOM_VG)
            .isVerified()) {
          return createResponse(
    ...
    if (previousDraft != null
      ...
      if (previousDraft.isSameVardgivare()) {
        if (previousDraft.isSameEnhet()) {
          return createResponse(
              DRAFT_FROM_SAME_CARE_PROVIDER_AND_UNIT_EXISTS, ResultCodeType.ERROR, certificateType);
        } else {
          return createResponse(
        ...
        return createResponse(
            DRAFT_FROM_OTHER_CARE_PROVIDER_EXISTS, ResultCodeType.INFO, certificateType);
      }
    }
  ...
  private static ValidateDraftCreationResponse createResponse(
      String message, ResultCodeType resultCode, String certificateType) {
    String messageWithCertificateName =
        String.format(
            message, KvIntygstyp.getDisplayNameFromCodeValue(certificateType).toLowerCase());
