...
import se.inera.intyg.webcert.infra.security.authorities.validation.AuthExpectationSpecification;
...
/**
 * Implementation used to evaluate access criterias. Set the criterias that will be considered and
 * then call evaluate(). Make sure to set basic information as user, certificateType, careUnit and
 * patient first.
 */
public final class AccessServiceEvaluation {
  ...
  private AccessServiceEvaluation(
   ...
   */
  public static AccessServiceEvaluation create(
      @NotNull WebCertUserService webCertUserService,
    ...
    return new AccessServiceEvaluation(
        webCertUserService, patientDetailsResolver, utkastService, intygTextsService);
  }

  /**
   * Specifies which user and certificate type this evaluation will be done for. If called more than
   * once, the old values will be overridden.
   *
  ...
  /**
   * Add a privilege to consider IF the addPrivilege is true. This method can be called multiple
   * times.
   *
   * @param privilege Privilege to consider
   ...
   * @return AccessServiceEvaluation
   */
  public AccessServiceEvaluation privilegeIf(
      @NotNull String privilege, @NotNull boolean addPrivilege) {
    if (addPrivilege) {
      this.privileges.add(privilege);
    }
    return this;
  ...
  /**
   * Add a feature to block access if activer IF the addBlockFeature is true. This method can be
   * called multiple times.
   *
   * @param blockFeature feature to block access
   * @param addFeature Only add feature if true.
   */
  public AccessServiceEvaluation blockFeatureIf(
      @NotNull String blockFeature, @NotNull boolean addFeature) {
    if (addFeature) {
      this.blockFeatures.add(blockFeature);
    }
    return this;
  ...
  public AccessServiceEvaluation checkLatestCertificateTypeVersionIf(
      @NotNull String certificateTypeVersion, @NotNull boolean addCheck) {
    if (addCheck) {
  ...
  public AccessServiceEvaluation checkLatestCertificateTypeVersion(
      @NotNull String certificateTypeVersion) {
  ...
  /**
   * Consider if patient is deceased when evaluating. If called more than once, the old values will
   * be overridden.
   *
   * @param allowForSameUnit Allow handling (if all other criterias are met) when user is on the
   *     same unit.
   * @return AccessServiceEvaluation
   */
  ...
  /**
   * Set certificate types that should be excluded from checkPatientDeceased. This means that the
   * certificate types will be allowed. This method can be called multiple times.
   *
  ...
  /**
   * Set certificate types that should always be invalid when checkPatientDeceased. This means that
   * the certificate types will never be allowed if patient is deceased. This method can be called
   * multiple times.
   *
  ...
  /**
   * Consider if parameter inactiveUnit when evaluating. If called more than once, the old values
   * will be overridden.
   *
   * @param allowForSameUnit Allow handling (if all other criterias are met) when user is on the
   *     same unit.
  ...
  /**
   * Set certificate types that should be excluded from checkInactiveCareUnit. This means that the
   * certificate types will be allowed. This method can be called multiple times.
   *
   * @param certificateType Certificate types to exclude
   * @return AccessServiceEvaluation
  ...
  /**
   * Consider if patient sekretess when evaluating. If called more than once, the old values will be
   * overridden.
   *
   * @return AccessServiceEvaluation
   */
  ...
  /**
   * Consider if parameter renewOk when evaluating. If called more than once, the old values will be
   * overridden.
   *
   * @param allowForSameUnit Allow handling (if all other criterias are met) when user is on the
   *     same unit.
  ...
  /**
   * Set certificate types that should be excluded from checkRenew. This means that the certificate
   * types will be allowed. This method can be called multiple times.
   *
   * @param certificateType Certificate types to exclude
   * @return AccessServiceEvaluation
   */
  ...
  /**
   * Consider unique draft/certificate rules when evaluating. If called more than once, the old
  ...
  /**
   * Consider logged in unit when evaluating. If called more than once, the old values will be
   * overridden.
   *
  ...
  /**
   * Set certificate types that should be excluded from checkUnit. This means that the certificate
   * types will be allowed. This method can be called multiple times.
   *
   * @param certificateType Certificate types to exclude
   * @return AccessServiceEvaluation
  ...
  public AccessResult evaluate() {
    ...
    if (checkPatientDeceased
        && !excludeDeceasedCertificateTypes.contains(certificateType)
        && accessResult.isEmpty()) {
      accessResult =
          isDeceasedRuleValid(
              user,
              certificateType,
              careUnit.getEnhetsid(),
              patient,
              allowDeceasedForSameUnit,
              invalidDeceasedCertificateTypes);
    }

    if (checkPatientTestIndicator && accessResult.isEmpty()) {
      accessResult =
          isPatientTestIndicated(patient, careUnit.getEnhetsid(), allowTestIndicatorForSameUnit);
    }
    ...
    if (checkInactiveCareUnit
        && !excludeInactiveCertificateTypes.contains(certificateType)
        && accessResult.isEmpty()) {
      accessResult =
          isInactiveUnitRuleValid(user, careUnit.getEnhetsid(), allowInactiveForSameUnit);
    }

    if (checkRenew
        && !excludeRenewCertificateTypes.contains(certificateType)
        && accessResult.isEmpty()) {
    ...
    if (checkUnit
        && !excludeUnitCertificateTypes.contains(certificateType)
        && accessResult.isEmpty()) {
    ...
    if (checkUnique && accessResult.isEmpty()) {
      accessResult =
          isUniqueUtkastRuleValid(
              certificateType, user, patient, checkUniqueOnlyCertificate, certificateId);
    }
  ...
  private Optional<AccessResult> isSubscriptionRuleValid(WebCertUser user) {
    ...
    return Optional.of(
        AccessResult.create(
            AccessResultCode.MISSING_SUBSCRIPTION,
            createMessage("Action is blocked due to missing subscription")));
  }

  private Optional<AccessResult> isLatestMajorVersionRuleValid(
      WebCertUser user, String certificateType, String certificateTypeVersion) {
    final var feature =
        getFeature(user, AuthoritiesConstants.FEATURE_INACTIVATE_PREVIOUS_MAJOR_VERSION);
    ...
    return Optional.of(
        AccessResult.create(
            AccessResultCode.NOT_LATEST_MAJOR_VERSION,
            createMessage(String.format("Feature %s is active and blocks the action", feature))));
  }

  private Optional<AccessResult> isCertificateTypeInactive(
      WebCertUser user, String certificateType) {
    ...
    return Optional.of(
        AccessResult.create(
            AccessResultCode.INACTIVE_CERTIFICATE_TYPE,
            createMessage(
                String.format(
                    "Feature %s is active and blocks the action for certificate type %s",
                    feature, certificateType))));
  }
  ...
  private Optional<AccessResult> isBlockedRuleValid(WebCertUser user, List<String> blockFeatures) {
    for (String blockFeature : blockFeatures) {
      ...
      if (isFeatureActive(feature)) {
        return Optional.of(
            AccessResult.create(
                AccessResultCode.AUTHORIZATION_BLOCKED,
                createMessage(
                    String.format("Feature %s is active and blocks authorization", blockFeature))));
      }
    }
    return Optional.empty();
  }
  ...
  private Optional<AccessResult> isAuthorized(
      String intygsTyp, WebCertUser user, List<String> features, List<String> privilege) {
    final AuthExpectationSpecification authExpectationSpecification =
        getAuthExpectationSpecification(intygsTyp, user, features, privilege);
    ...
    return Optional.of(
        AccessResult.create(
            AccessResultCode.AUTHORIZATION_VALIDATION, authExpectationSpecification.error()));
  }

  private AuthExpectationSpecification getAuthExpectationSpecification(
      String intygsTyp, WebCertUser user, List<String> features, List<String> privileges) {
  ...
  private Optional<AccessResult> isDeceasedRuleValid(
      WebCertUser user,
      String intygsTyp,
      String enhetsId,
      Personnummer personnummer,
      boolean allowDeceasedForSameUnit,
      List<String> invalidDeceasedCertificateTypes) {
    ...
    if (patientDetailsResolver.isAvliden(personnummer)) {
      ...
      if (invalidDeceasedCertificateTypes.contains(intygsTyp)) {
        return Optional.of(
            AccessResult.create(AccessResultCode.DECEASED_PATIENT, createMessage(errorMessage)));
      }
  ...
  private Optional<AccessResult> isPatientTestIndicated(
      Personnummer patient, String unitId, boolean allowForSameUnit) {
    if (patientDetailsResolver.isTestIndicator(patient)
        && (isUserLoggedInOnDifferentUnit(unitId) || !allowForSameUnit)) {
      return Optional.of(
          AccessResult.create(
              AccessResultCode.TEST_INDICATED_PATIENT,
  ...
  private Optional<AccessResult> isTestCertificate(boolean isTestCertificate) {
    if (isTestCertificate) {
      return Optional.of(
          AccessResult.create(
              AccessResultCode.TEST_CERTIFICATE,
              createMessage("Certificate is flagged as test certificate")));
    }
    return Optional.empty();
  }

  private Optional<AccessResult> isInactiveUnitRuleValid(
      WebCertUser user, String enhetsId, boolean allowInactiveForSameUnit) {
    if (user.getParameters() != null && user.getParameters().isInactiveUnit()) {
      if (isUserLoggedInOnDifferentUnit(enhetsId) || !allowInactiveForSameUnit) {
        return Optional.of(
            AccessResult.create(
                AccessResultCode.INACTIVE_UNIT, createMessage("Parameter inactive unit")));
      }
    }
    return Optional.empty();
  }

  private Optional<AccessResult> isRenewRuleValid(
      WebCertUser user, String enhetsId, boolean allowRenewForSameUnit) {
    if (user.getParameters() != null
        && !user.getParameters().isFornyaOk()
        && (isUserLoggedInOnDifferentUnit(enhetsId) || !allowRenewForSameUnit)) {
      return Optional.of(
          AccessResult.create(
              AccessResultCode.RENEW_FALSE, createMessage("Parameter renewOK is false")));
    }

    return Optional.empty();
  }

  private Optional<AccessResult> isSekretessRuleValid(
      String intygsTyp, String enhetsId, WebCertUser user, Personnummer personnummer) {
    final SekretessStatus sekretessStatus = patientDetailsResolver.getSekretessStatus(personnummer);
    if (SekretessStatus.UNDEFINED.equals(sekretessStatus)) {
      return Optional.of(
          AccessResult.create(
              AccessResultCode.PU_PROBLEM,
              createMessage("PU-Service not available to resolve sekretess")));
    }

    if (SekretessStatus.TRUE.equals(sekretessStatus)) {
      final Optional<AccessResult> accessResult =
          isAuthorized(
              intygsTyp,
              user,
              Collections.emptyList(),
      ...
      if (accessResult.isPresent()) {
        return Optional.of(
            AccessResult.create(
                AccessResultCode.AUTHORIZATION_SEKRETESS,
                createMessage(
      ...
      if (isUserLoggedInOnDifferentUnit(enhetsId)) {
        return Optional.of(
            AccessResult.create(
                AccessResultCode.AUTHORIZATION_SEKRETESS_UNIT,
                createMessage(
  ...
  private Optional<AccessResult> isUnitRuleValid(
      Vardenhet vardenhet, WebCertUser user, boolean allowSJF, boolean isReadOnlyOperation) {
    ...
    if (user.getOrigin().equals(UserOriginType.DJUPINTEGRATION.name())) {
      ...
      if (isReadOnlyOperation
          && vardgivarId != null
          && !user.getValdVardgivare().getId().equals(vardgivarId)) {
        return Optional.of(
            AccessResult.create(
                AccessResultCode.AUTHORIZATION_DIFFERENT_UNIT, createMessage(errorMessage)));
      }
    }

    if (!webCertUserService.isAuthorizedForUnit(
        vardenhet.getVardgivare().getVardgivarid(), vardenhet.getEnhetsid(), isReadOnlyOperation)) {
      return Optional.of(
          AccessResult.create(
              AccessResultCode.AUTHORIZATION_DIFFERENT_UNIT, createMessage(errorMessage)));
    }
  ...
  private Optional<AccessResult> isUniqueUtkastRuleValid(
      String intygsTyp,
      WebCertUser user,
      Personnummer personnummer,
    ...
    if (isAnyUniqueFeatureEnabled(intygsTyp, user)) {
      final Map<String, Map<String, PreviousIntyg>> intygstypToStringToBoolean =
      ...
      if (!onlyCertificate && utkastExists != null && utkastExists.isSameVardgivare()) {
        ...
        if (intygExists != null && !utkastService.isDraftCreatedFromReplacement(certificateId)) {
          if (isUniqueFeatureEnabled(intygsTyp, user)) {
            return Optional.of(
                AccessResult.create(
                    AccessResultCode.UNIQUE_CERTIFICATE,
  ...
  private boolean isUniqueIntygFeatureEnabled(String intygsTyp, WebCertUser user) {
    final AuthoritiesValidator authoritiesValidator = new AuthoritiesValidator();
    return authoritiesValidator
        .given(user, intygsTyp)
        .features(AuthoritiesConstants.FEATURE_UNIKT_INTYG_INOM_VG)
        .isVerified();
  }
  ...
  private boolean isUniqueUtkastFeatureEnabled(String intygsTyp, WebCertUser user) {
    ...
    return authoritiesValidator
        .given(user, intygsTyp)
        .features(AuthoritiesConstants.FEATURE_UNIKT_UTKAST_INOM_VG)
        .isVerified();
  }
  ...
  private boolean isMissingSubscriptionWhenRequired(WebCertUser webCertUser) {
    final var isFristaendeWebcertUser =
        webCertUser.getOrigin().equals(UserOriginType.NORMAL.name());
    final var subscriptionInfo = webCertUser.getSubscriptionInfo();
    final var isSubscriptionRequired =
    ...
    if (isFristaendeWebcertUser && isSubscriptionRequired) {
      ...
      return selectedCareProvider != null
          && missingSubscriptions.contains(selectedCareProvider.getId());
    }
