...
public class CreateDraftCertificateValidatorImpl extends BaseCreateDraftCertificateValidator
  ...
  private void validatePatient(ResultValidator errors, Patient patient) {
    ...
    validatePatient(
        errors, Collections.singletonList(patient.getFornamn()), patient.getEfternamn(), personId);
  }

  private void validateSkapadAv(ResultValidator errors, HosPersonal skapadAv) {
    String personalId =
        skapadAv.getPersonalId() == null ? "" : skapadAv.getPersonalId().getExtension();
  ...
  private void validateSekretessmarkeringOchIntygsTyp(
      ResultValidator errors, Personnummer personnummer, TypAvIntyg typAvIntyg, IntygUser user) {
