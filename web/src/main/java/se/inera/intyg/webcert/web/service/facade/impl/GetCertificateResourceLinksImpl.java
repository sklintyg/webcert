...
public class GetCertificateResourceLinksImpl implements GetCertificateResourceLinks {
   ...
   * things like certificate data, user, integration, other existing certificates etc.
   *
  ...
  /**
   * To support certificates returned from certificate-service we need to consider if the
   * certificate already contains links. If the certificate do, then convert those links and return.
   * If not evaluate as normal with available functions and access functions.
   */
  ...
  private Map<ResourceLinkTypeDTO, AccessCheck> getFunctionsForDraft() {
    ...
    functions.put(
        ...
        (accessEvaluationParameters, certificate) ->
            draftAccessServiceHelper.isAllowToSign(
                accessEvaluationParameters, certificate.getMetadata().getId()));
  ...
  private interface AccessCheck {
    ...
    boolean hasAccess(
        AccessEvaluationParameters accessEvaluationParameters, Certificate certificate);
  }
}
