...
public class MailNotificationServiceImpl implements MailNotificationService {
  ...
  private static final String INCOMING_QUESTION_SUBJECT =
      "Försäkringskassan har ställt en fråga angående ett intyg";
  ...
  @Nullable private final PrivatePractitionerService privatePractitionerService;
  ...
  public void sendMailForIncomingQuestion(MailNotification mailNotification) {
    ...
    final var employeeHsaName =
        employeeNameService.getEmployeeHsaName(mailNotification.getSignedByHsaId());
    ...
    if (recipient == null) {
      ...
      try {
        ...
        sendNotificationMailToEnhet(
            type,
            mailNotification,
            INCOMING_QUESTION_SUBJECT,
  ...
  private void sendNotificationMailToEnhet(
      String type,
    ...
    try {
      // if recipient unit does not have a mail address configured, we try to lookup the unit's
      // parent
  ...
  private String mailBodyForFraga(
      MailNotificationEnhet unit, MailNotification mailNotification, String employeeName) {
    return "<p>"
        + "Försäkringskassan har ställt en fråga på ett intyg utfärdat av <b>%s</b> på <b>%s</b>."
            .formatted(employeeName, unit.getName())
        + "<br><a href=\""
        + intygsUrl(mailNotification)
        + "\">Läs och besvara frågan i Webcert</a></p><p>OBS! Sätt i ditt SITHS-kort innan du klickar på länken.</p>";
  }

  private String mailBodyForSvar(
      MailNotificationEnhet unit, MailNotification mailNotification, String employeeName) {
    return "<p>"
        + "Det har kommit ett svar från Försäkringskassan på en fråga som <b>%s</b> på <b>%s</b> har ställt."
            .formatted(employeeName, unit.getName())
        + " har ställt"
        + ".<br><a href=\""
        + intygsUrl(mailNotification)
  ...
  private void sendAdminMailAboutMissingEmailAddress(
      MailNotificationEnhet unit, MailNotification mailNotification) throws MessagingException {
    ...
    body.append(
        "<p> Enhetens id och namn är <b>%s %s</b>.".formatted(unit.getHsaId(), unit.getName()));
  ...
  private boolean isPrivatePractitionerEnhet(String careUnitId) {
    return careUnitId != null
        && careUnitId.toUpperCase(Locale.ENGLISH).startsWith(PRIVATE_PRACTITIONER_HSAID_PREFIX);
  }
  ...
  private MailNotificationEnhet retrieveDataFromHsa(String careUnitId) {
    try {
      ...
      return new MailNotificationEnhet(
          enhetData.getId(), enhetData.getNamn(), enhetData.getEpost());
    } catch (WebServiceException ex) {
      log.error(
          String.format(
              "Failed to contact HSA to get HSA Id '%s' : '%s'", careUnitId, ex.getMessage()),
          ex);
      return null;
    }
  }
  ...
  private MailNotificationEnhet getMailNotificationEnhetFromPPS(String hsaId) {
    ...
    try {
      ...
      if (privatePractitioner != null) {
        return new MailNotificationEnhet(
            hsaId, privatePractitioner.getCareUnitName(), privatePractitioner.getEmail());
      }
    } catch (Exception e) {
  ...
  public String intygsUrl(MailNotification mailNotification) {
    String url =
        webCertHostUrl
            + "/webcert/web/user/"
            + resolvePathSegment(
                mailNotification.getCareUnitId(), mailNotification.getCertificateId())
            + "/";
  ...
  private String resolvePathSegment(String enhetsId, String intygsId) {
    ...
    // Assume that the receiving user should have UTHOPP role if the certificate was not registered
    // through webcert
