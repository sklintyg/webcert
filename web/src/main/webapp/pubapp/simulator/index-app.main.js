...
.controller('IndexController', ['$scope', '$http', function($scope, $http) {
  ...
  $scope.deleteAllArendenOnUnit = function() {
    if ($scope.selectedEnhet !== '' && window.confirm(
        'Är du verkligen helt säker på att du vill radera alla ärenden på ' + $scope.selectedEnhet + ' ur databasen?')) {
      $http({
        method: 'DELETE',
  ...
  $scope.sendQuestion = function(q) {
    ...
    $http({
      ...
      if (response.status === 200) {
        ...
        $scope.resultat =
            '"Servern svarade med HTTP ' + response.status + ' ' + response.statusText + '. Det betyder att någonting gick fel.';
      }
