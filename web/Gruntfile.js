...
module.exports = function(grunt) {
  ...
  var VENDOR_DEPS = [
    ...
    SRC_DIR + '../bower_components/highcharts/modules/accessibility.js',
  ...
  grunt.file.write(DEST_DIR + 'app-deps.json', JSON.stringify(webcert.map(function(file) {
    return '/app/' + file;
  ...
  grunt.registerTask('generateModuleDeps', function() {
    ...
    grunt.file.write(DEST_DIR + 'app-deps.json', JSON.stringify(files.map(function(file) {
      return '/app/' + file;
    }).concat('/app/templates.js'), null, 4));
    ...
    Object.keys(modules).forEach(function(moduleName) {
      ...
      grunt.file.write(__dirname + module.dest + '/webcert/module-deps.json', JSON.stringify(files.map(function(file) {
  ...
  grunt.registerTask('default',
      ['bower', 'injector:sass', 'wiredep', 'ngtemplates:webcert', 'concat', 'ngAnnotate', 'uglify', 'sass:dist', 'postcss']);
