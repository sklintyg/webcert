...
public class CertificateAnalyticsMessageFactory {
  ...
  public CertificateAnalyticsMessage draftCreated(
      Certificate certificate, LoggedInWebcertUser loggedInWebcertUser) {
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(DRAFT_CREATED, loggedInWebcertUser))
        .certificate(createCertificate(certificate))
        .build();
  }

  public CertificateAnalyticsMessage draftCreated(
      Utkast utkast, LoggedInWebcertUser loggedInWebcertUser) {
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(DRAFT_CREATED, loggedInWebcertUser))
        .certificate(createCertificate(utkast))
        .build();
  }
  ...
  public CertificateAnalyticsMessage draftCreatedWithPrefill(
      Certificate certificate, LoggedInWebcertUser loggedInWebcertUser) {
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(DRAFT_CREATED_WITH_PREFILL, loggedInWebcertUser))
        .certificate(createCertificate(certificate))
        .build();
  }
  ...
  public CertificateAnalyticsMessage certificateSent(Certificate certificate, String recipientId) {
    return create(certificate, CERTIFICATE_SENT)
        .recipient(AnalyticsRecipient.builder().id(recipientId).build())
        .build();
  }
  ...
  public CertificateAnalyticsMessage receivedMessage(
      Certificate certificate, IncomingMessageRequestDTO incomingMessageRequest) {
  ...
  private CertificateAnalyticsMessageBuilder create(
    ...
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(type))
        .certificate(createCertificate(certificate));
  }

  private CertificateAnalyticsMessageBuilder create(
      Utkast utkast, CertificateAnalyticsMessageType type) {
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(type))
        .certificate(createCertificate(utkast));
  }

  private CertificateAnalyticsMessage create(
      Utlatande utlatande, CertificateAnalyticsMessageType type) {
    return CertificateAnalyticsMessage.builder()
        .event(createAnalyticsEvent(type))
        .certificate(
            AnalyticsCertificate.builder()
                ...
                .patientId(
                    utlatande.getGrundData().getPatient().getPersonId().getPersonnummerWithDash())
  ...
  private AnalyticsEvent createAnalyticsEvent(
      CertificateAnalyticsMessageType type, LoggedInWebcertUser loggedInWebcertUser) {
  ...
  private AnalyticsCertificate createCertificate(Certificate certificate) {
    return AnalyticsCertificate.builder()
        ...
        .parent(createCertificateAnalyticsRelation(certificate))
        .build();
  }
  ...
  private AnalyticsCertificate createCertificate(Utkast utkast) {
    return AnalyticsCertificate.builder()
        ...
        .parent(createCertificateAnalyticsRelation(utkast))
        .build();
  }
  ...
  private static CertificateAnalyticsMessageType messageTypeForIncomingMessage(
      IncomingMessageRequestDTO incomingMessageRequest) {
  ...
  private static boolean isAnswer(IncomingMessageRequestDTO incomingMessageRequest) {
    return incomingMessageRequest.getAnswerMessageId() != null
        && !incomingMessageRequest.getAnswerMessageId().isEmpty();
  }

  private static boolean isAnswer(Question question) {
    return question.getAnswer() != null
        && question.getAnswer().getId() != null
        && !question.getAnswer().getId().isEmpty();
  }
