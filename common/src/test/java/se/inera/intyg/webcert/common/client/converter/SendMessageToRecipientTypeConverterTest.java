...
class SendMessageToRecipientTypeConverterTest {
  ...
  void convertTest() throws JAXBException {
    ...
    assertEquals(
        ...
        requestRes.getPatientPersonId().getExtension());
    assertEquals(
        request.getSkickatAv().getPersonalId().getExtension(),
        requestRes.getSkickatAv().getPersonalId().getExtension());
  }
  ...
  private SendMessageToRecipientType buildSendMessageToRecipientType() {
    ...
    res.getSkickatAv()
        ...
        .getVardgivare()
        .getVardgivareId()
        .setExtension("TSTNMT2321000156-1002");
