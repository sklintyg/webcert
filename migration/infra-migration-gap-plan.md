...
`se.inera.intyg.webcert.infra.*`, and unused classes cleaned up. However, several items called for in
the incremental plan were not fully addressed during that work. This plan identifies and orders those
remaining gaps.
...
2. Remove `<import resource="classpath:basic-cache-config.xml"/>` from `webcert-config.xml` (line 36)
...
2. `IaCacheConfiguration` is already a `@Configuration` class. Add `@Profile` annotations directly
   ...
   - The nested profile logic in XML is complex — simplify using Spring's profile expression syntax:
     `@Profile({"qa", "prod", "caching-enabled"})`

3. Remove `<import resource="classpath:ia-services-config.xml"/>` from `webcert-config.xml` (line 37)
