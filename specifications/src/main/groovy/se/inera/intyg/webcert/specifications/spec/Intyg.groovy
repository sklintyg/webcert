...
import se.inera.intyg.webcert.specifications.spec.util.base.RestClientFixture
...
public class Intyg extends RestClientFixture {
    ...
    private certificateJson() {
        def doc = document()
		def stateList = [[state:"RECEIVED", target:"HSVARD", timestamp:utfärdat + "T12:00:00.000"]]
        if (skickat)
			stateList << [state:"SENT", target:"FKASSA", timestamp:utfärdat + "T12:00:10.000"]
		if (rättat)
			stateList << [state:"CANCELLED", target:"HSVARD", timestamp:utfärdat + "T13:00:00.000"]
