...
public interface FragaSvarRepositoryCustom extends FragaSvarFilteredRepositoryCustom {
   ...
   * matching one of the supplied list of id's. Is also discards any entity with {@link
   * se.inera.intyg.webcert.persistence.fragasvar.model.Status.CLOSED}. The result is NOT ordered.
   *
  ...
  /**
   * Should return a count of {@link FragaSvar} entities in the repository that has an enhetsId
   * matching one of the supplied list of id's. Is also discards any entity with {@link
   * se.inera.intyg.webcert.persistence.fragasvar.model.Status.CLOSED}.
   *
   ...
   */
  @Query(
      "SELECT count(fs) FROM FragaSvar fs WHERE fs.vardperson.enhetsId IN (:idList) AND fs.status <> 'CLOSED'")
  Long countUnhandledForEnhetsIds(@Param("idList") List<String> enhetsIds);

  /**
   * Should return a list that contains GroupableItems for the given parameters for later
   * aggregation into number of items per unit.
   *
   * @param enhetsIds List of hsa unit id's that should match the counted fraga svar entities.
   * @return A list that contains an array with fragasvar represented by id, enhets id and
   *     personnummer.
   */
  @Query(
      "SELECT new se.inera.intyg.webcert.common.model.GroupableItem(fs.internReferens, fs.vardperson.enhetsId, "
  ...
  List<GroupableItem> getUnhandledWithEnhetIdsAndIntygstyper(
      @Param("idList") List<String> enhetsIds, @Param("intygsTyper") Set<String> intygsTyper);

  /**
   * Returns a list of all unique hsaId and name (of vardperson who signed the certificate the
   * FragaSvar is linked to) where matches the supplied id.
   *
   * @return A list of Object[] where the first [0] value is the HsaId and the second [1] is the
   *     name
   */
  @Query(
      "SELECT DISTINCT fs.vardperson.hsaId, fs.vardperson.namn FROM FragaSvar fs WHERE fs.vardperson.enhetsId IN (:idList) "
  ...
  @Query(
      "SELECT NEW se.inera.intyg.webcert.persistence.fragasvar.model.FragaSvarStatus(fs.internReferens, fs.frageStallare, "
  ...
  /**
   * Should return a list of {@link FragaSvar} entities in the repository related to the specified
   * intygsId.
   *
   * @return A list of {@link FragaSvar} matching the search criteria. If no entities are found,
   *     this method returns an empty list.
