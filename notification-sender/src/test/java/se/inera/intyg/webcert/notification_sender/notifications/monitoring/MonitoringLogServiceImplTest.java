...
class MonitoringLogServiceImplTest {
  ...
  private static final LocalDateTime NEXT_REDELIEVERY_TIME =
      LocalDateTime.of(2021, 2, 21, 16, 1, 23, 123456789);
  ...
  void shouldLogNotificationDeliverySuccess() {
    logService.logStatusUpdateForCareStatusSuccess(
        EVENT_ID, EVENT_TYPE, CERTIFICATE_ID, CORRELATION_ID, LOGICAL_ADDRESS, DELIVERY_ATTEMPT);
    verifyLog(
        "STATUS_UPDATE_RESULT_SUCCESS Status update for care success on "
            + "delivery attempt '"
            + DELIVERY_ATTEMPT
            + "' for event ["
            + "certificateId: '"
  ...
  void shouldLogNotificationDeliveryResend() {
    logService.logStatusUpdateForCareStatusResend(
        ...
        ERROR_MESSAGE,
        DELIVERY_ATTEMPT,
        NEXT_REDELIEVERY_TIME);
    verifyLog(
        "STATUS_UPDATE_RESULT_RESEND Status update for care failure on "
            + "delivery attempt '"
            + DELIVERY_ATTEMPT
            + "' for event ["
            + "certificateId: '"
  ...
  void shouldLogNotificationDeliveryFailure() {
    logService.logStatusUpdateForCareStatusFailure(
        ...
        DELIVERY_ATTEMPT);
    verifyLog(
        "STATUS_UPDATE_RESULT_FAILURE Status update for care failure on "
            + "delivery attempt '"
            + DELIVERY_ATTEMPT
            + "' for event ["
            + "certificateId: '"
