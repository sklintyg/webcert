...
public class NotificationWSSenderTest {
  ...
  @InjectMocks private NotificationWSSender notificationWSSender;
  ...
  public void shallUpdateHandleByIfUserIdHeaderIsSet() {
    ...
    doReturn(mock(CertificateStatusUpdateForCareResponseType.class))
        .when(statusUpdateForCareClient)
        .certificateStatusUpdateForCare(LOGICAL_ADDRESS, statusUpdateMock);

    notificationWSSender.sendStatusUpdate(
        statusUpdateMock, CERTIFICATE_ID, LOGICAL_ADDRESS, USER_ID, CORRELATION_ID);

    verify(statusUpdateMock).setHanteratAv(argumentCaptor.capture());
    final var actualUserId =
        argumentCaptor.getValue() != null ? argumentCaptor.getValue().getExtension() : null;
  ...
  public void shallLeaveHandleByUnchangedIfUserIdHeaderIsMissing() {
    ...
    notificationWSSender.sendStatusUpdate(
        statusUpdateMock, CERTIFICATE_ID, LOGICAL_ADDRESS, null, CORRELATION_ID);
  ...
  public void
    ...
    doReturn(certificateStatusUpdateForCareResponseType)
        .when(statusUpdateForCareClient)
        .certificateStatusUpdateForCare(LOGICAL_ADDRESS, statusUpdateMock);
  ...
  public void shallSendNotificationResultMessageWhenCertificateStatusUpdateForCareReturns() {
    ...
    doReturn(mock(CertificateStatusUpdateForCareResponseType.class))
        .when(statusUpdateForCareClient)
        .certificateStatusUpdateForCare(LOGICAL_ADDRESS, statusUpdateMock);
    doReturn(expectedResultMessage)
        .when(notificationResultMessageCreator)
        .createResultMessage(statusUpdateMock, CORRELATION_ID);
