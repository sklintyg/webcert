...
public class CertificateRouteBuilder extends RouteBuilder {
  ...
  public void configure() {
    ...
    from(receiveCertificateTransferUri)
        ...
        .handled(true)
        .to("direct:certPermanentErrorHandlerEndpoint")
        .end()
        ...
        .to("bean:certificateStoreProcessor")
        .stop()
        .when(header(Constants.MESSAGE_TYPE).isEqualTo(Constants.SEND_MESSAGE))
        .to("bean:certificateSendProcessor")
        .stop()
        .when(header(Constants.MESSAGE_TYPE).isEqualTo(Constants.REVOKE_MESSAGE))
        .to("bean:certificateRevokeProcessor")
        .stop()
        .when(header(Constants.MESSAGE_TYPE).isEqualTo(Constants.SEND_MESSAGE_TO_RECIPIENT))
        .to("bean:sendMessageToRecipientProcessor")
        .stop()
        .when(
            header(Constants.MESSAGE_TYPE).isEqualTo(Constants.REGISTER_APPROVED_RECEIVERS_MESSAGE))
        .to("bean:registerApprovedReceiversProcessor")
        .stop()
        .otherwise()
        .log(
            LoggingLevel.ERROR,
            LOG,
            simple("Unknown message type: ${in.headers.MESSAGE_TYPE}").toString())
        .stop();
    ...
    from("direct:certPermanentErrorHandlerEndpoint")
        ...
        .log(
            ...
            LOG,
            simple(
                    "Permanent exception for intygs-id: ${header[INTYGS_ID]}, of message type ${header[MESSAGE_TYPE]}, "
    ...
    from("direct:certTemporaryErrorHandlerEndpoint")
        ...
        .log(
            ...
            LOG,
            simple(
                    "Temporary exception for intygs-id: ${header[INTYGS_ID]}, of message type ${header[MESSAGE_TYPE]}, "
                        + "with message: ${exception.message}\n ${exception.stacktrace}")
                .toString())
        .otherwise()
        .log(
