...
public final class NotificationRedeliveryUtil {
  ...
  public static Handelse getEventV3(
      HandelsekodEnum eventType,
      LocalDateTime eventTime,
      ArendeAmne topic,
      LocalDate lastDateForReply) {
    ...
    if (HandelsekodEnum.NYFRFM == eventType) {
      eventV3.setAmne(
          topic != null ? AmneskodCreator.create(topic.name(), topic.getDescription()) : null);
  ...
  public static HosPersonal getHosPersonal(
      Vardgivare careProvider, Vardenhet careUnit, PersonInformation personInfo) {
    ...
    unitV3.setArbetsplatskod(
        getIIType(
            new ArbetsplatsKod(),
            Strings.nullToEmpty(careUnit.getArbetsplatskod()).trim().isEmpty()
                ? TEMPORARY_ARBETSPLATSKOD
                : careUnit.getArbetsplatskod(),
            ARBETSPLATS_KOD_OID));
    ...
    hosPersonal.setFullstandigtNamn(
        personInfo.getGivenName() + " " + personInfo.getMiddleAndSurName());
  ...
  public static Patient getPatient(String personnummer) {
    ...
    patientV3.setPersonId(
        getIIType(
            new PersonId(),
            personnummer,
            SamordningsnummerValidator.isSamordningsNummer(personalId)
                ? SAMORDNING_ID_OID
                : PERSON_ID_OID));
