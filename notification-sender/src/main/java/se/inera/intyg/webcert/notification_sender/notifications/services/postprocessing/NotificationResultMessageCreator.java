...
public class NotificationResultMessageCreator {
  ...
  public NotificationResultMessage createFailureMessage(
    ...
    final var moduleEntryPoint =
        moduleRegistry.getModuleEntryPoint(notificationMessage.getIntygsTyp());
    final var certificateTypeExternalId = moduleEntryPoint.getExternalId();

    final var event =
        createEvent(notificationMessage, utlatande, userId, certificateTypeExternalId);
  ...
  public NotificationResultMessage createFailureMessage(
      Handelse event, NotificationRedelivery redelivery, Exception exception) {
  ...
  public NotificationResultMessage createResultMessage(
      CertificateStatusUpdateForCareType statusUpdate, String correlationId) {
  ...
  public void addToResultMessage(
  ...
  private void addStatusUpdateToResultMessage(
      NotificationResultMessage resultMessage, CertificateStatusUpdateForCareType statusUpdate) {
  ...
  private void addResultTypeToResultMessage(
      NotificationResultMessage resultMessage, ResultType resultType) {
    final var notificationResultType = new NotificationResultType();

    if (resultType.getResultCode() != null) {
      final var notificationResult =
          NotificationResultTypeEnum.fromValue(resultType.getResultCode().value());
      notificationResultType.setNotificationResult(notificationResult);
    }

    if (resultType.getErrorId() != null) {
      final var notificationErrorType =
          NotificationErrorTypeEnum.fromValue(resultType.getErrorId().value());
  ...
  private void addResultTypeToResultMessage(
      NotificationResultMessage resultMessage, Exception exception) {
    final var notificationResultType = new NotificationResultType();
  ...
  private Handelse createEvent(CertificateStatusUpdateForCareType statusUpdate) {
    ...
    event.setCertificateIssuer(
        statusUpdate.getIntyg().getSkapadAv().getPersonalId().getExtension());
  ...
  private NotificationResultType createResultType(
      Exception exception, NotificationResultTypeEnum resultTypeEnum) {
    final var notificationResultType = new NotificationResultType();
  ...
  private Handelse createEvent(
    ...
    event.setAmne(
        notificationMessage.getAmne() != null
            ? ArendeAmne.valueOf(notificationMessage.getAmne().getCode())
            : null);
