...
function checkFMB(fmbDiagnos, intyg) {
  ...
  if (fmbDiagnos.overliggandeTxt) {
    promiseArray.push(
        expect(element(by.id('fmb_heading_BESLUTSUNDERLAG_TEXTUELLT')).getText()).to.eventually.contain('Vägledning för sjukskrivning'));
  ...
  if (fmbDiagnos.generellInfo) {
    promiseArray.push(
        expect(element(by.id('fmb_heading_GENERELL_INFO')).getText()).to.eventually.contain('Försäkringsmedicinsk information'));
    //console.log("generellInfo: " + element(by.id('fmb_heading_GENERELL_INFO')).getText());
  }

  if (fmbDiagnos.symptomPrognosBehandling) {
    promiseArray.push(
        expect(element(by.id('fmb_heading_SYMPTOM_PROGNOS_BEHANDLING')).getText()).to.eventually.contain('Symtom, prognos, behandling'));
...
*/
    if (fmbDiagnos.vagledningForSjukskrivning) {
      promiseArray.push(expect(element(by.id('fmb_bullet_BESLUTSUNDERLAG_TEXTUELLT_0')).getText()).to.eventually.contain(
          fmbDiagnos.vagledningForSjukskrivning));
      //console.log("vagledningForSjukskrivning: " + element(by.id('fmb_bullet_BESLUTSUNDERLAG_TEXTUELLT_0')).getText());
    }

    if (fmbDiagnos.funktionsnedsattning) {
      promiseArray.push(
          expect(element(by.id('fmb_text_FUNKTIONSNEDSATTNING')).getText()).to.eventually.contain(fmbDiagnos.funktionsnedsattning));
    ...
    if (fmbDiagnos.aktivitetsbegransning) {
      promiseArray.push(
          expect(element(by.id('fmb_text_AKTIVITETSBEGRANSNING')).getText()).to.eventually.contain(fmbDiagnos.aktivitetsbegransning));
      //console.log("aktivitetsbegransning: " + element(by.id('fmb_text_AKTIVITETSBEGRANSNING')).getAttribute());
    }

    if (fmbDiagnos.symptomPrognosBehandling) {
      promiseArray.push(expect(element(by.id('fmb_text_SYMPTOM_PROGNOS_BEHANDLING')).getText()).to.eventually.contain(
          fmbDiagnos.symptomPrognosBehandling));
