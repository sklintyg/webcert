...
When(/^jag ska kunna svara med textmeddelande/, function() {
  ...
  var svaraPaKomplettering = kompletteringsFraga.element(by.cssContainingText('.btn-default', 'Kan inte komplettera')).typeKeys(
...
Given(/^Försäkringskassan (?:har ställt|ställer) en "([^"]*)" fråga om intyget$/, function(amne, callback) {
  ...
  body = soapMessageBodies.SendMessageToCare(this.user, this.patient, this.intyg, 'Begär ' + amne, testdataHelper.generateTestGuid(),
      amneCode);
  logger.silly(body);
