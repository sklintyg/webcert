...
function getNotificationEntries(intygsId, value, numEvents, intyg) {
  ...
  var selectStatement = 'SELECT utlatandeExtension, handelseKod,antalFragor,antalHanteradeFragor,antalSvar,antalHanteradeSvar, '
      + handelseTidName;
...
Then(/^ska statusuppdateringen visa frågor (\d+), hanterade frågor (\d+),antal svar (\d+), hanterade svar (\d+)$/,
    function(fragor, hanFragor, svar, hanSvar) {
