...
var Ag7804Utkast = AgBaseUtkast._extend({
    ...
    angeDiagnos: function(diagnosData) {
        ...
        if (diagnosData.onskarFormedlaDiagnos) {
            ...
            for (var i = 0; i < diagnosData.rows.length; i++) {
                ...
                promiseArr.push(pageHelpers.moveAndSendKeys(row.kod, diagnosData.rows[i].kod).then(browser.sleep(1000)).then(that.sendEnterToElement(row.kod)));
            }
        } else {
    ...
    angeKonsekvenser: function(data) {
        ...
        if (data.aktivitetsbegransning) {
            promisesArr.push(pageHelpers.moveAndSendKeys(this.konsekvenser.aktivitetsbegransning, data.aktivitetsbegransning, data.aktivitetsbegransning).then(function() {
